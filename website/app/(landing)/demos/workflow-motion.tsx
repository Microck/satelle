...
export class DemoCursor extends React.Component<CursorProps, { from: Point; to: Point }> {
  ...
  render() {
    ...
    return <span className="sw-cursor" ref={node => { this.node = node; }} aria-hidden="true" style={{ visibility: this.props.visible && this.props.target ? 'visible' : 'hidden', transform: `translate(${Math.round(from.x + (to.x - from.x) * q)}px, ${Math.round(from.y + (to.y - from.y) * q)}px)` }}>
