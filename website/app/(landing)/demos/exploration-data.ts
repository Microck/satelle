/** Scripted UI illustrations. No account, browser, Host, or MCP request is made.
 * Product/API and vendor UI sources: website/DEMO-EXPLORATIONS.md. */
...
export const SITES = {
    qa: 'github.com/Microck/satelle',
    analytics: 'analytics.google.com/analytics/web/',
    drive: 'drive.google.com/drive/my-drive',
    slack: 'slack.com',
};
export const PROMPTS = {
    qa: 'QA the Satelle repository on github.com. Check README navigation, issue search, and the empty state. Do not submit anything.',
...
export const CHATGPT_INTEGRATION = {
    status: 'concept',
    note: 'ChatGPT desktop concept. A compatible remote MCP bridge is required and is not included in Satelle.',
};
export const DEMOS = [
    { id: 'qa', number: '01', title: 'Test the website, not just the code.', lead: 'Walk through GitHub in the browser. Check the paths a person takes.', cta: 'Run a website check', href: '/docs/tutorial/first-session', label: 'Website QA', surface: 'github' },
    { id: 'chat', number: '02', title: 'Your computers, one conversation.', lead: 'A ChatGPT desktop concept: choose a Host, then ask for the task.', cta: 'Review connection requirements', href: '/demo-explorations#chatgpt-requirements', label: 'ChatGPT desktop', surface: 'chatgpt' },
...
export const TIMELINES: Record<DemoId, readonly Beat[]> = {
    qa: [
        { label: 'Open GitHub in the Host browser', ms: 3200 },
        { label: 'Check the README link', ms: 1600, target: 'qa-readme' },
        { label: 'Open the Issues tab', ms: 1500, target: 'qa-issues' },
        { label: 'Search for an intentionally unmatched term', ms: 2300, target: 'qa-search' },
        { label: 'Check the empty-state message', ms: 1600, target: 'qa-submit' },
        { label: 'Review the example QA notes', ms: 2200 },
    ],
    chat: [
        { label: 'Ask which Hosts are configured', ms: 1900 },
        { label: 'Read the local Host configuration', ms: 1500 },
        { label: 'Show configured Hosts, not a readiness verdict', ms: 2000 },
        { label: 'Ask for the Slack profile-photo task', ms: 2800 },
        { label: 'Review the proposed action', ms: 1800 },
        { label: 'Confirm the concept task', ms: 1500, target: 'chat-confirm' },
        { label: 'Show hypothetical task admission', ms: 2200 },
    ],
    transfer: [
...
export function sample(beats: readonly Beat[], elapsed: number) {
    ...
    let step = 0;
    let local = safe;
    while (step < beats.length - 1 && local >= beats[step].ms)
        local -= beats[step++].ms;
...
export function typed(text: string, frame: Frame, step: number, ms = 1800) {
    if (frame.step < step)
        return '';
    if (frame.step > step || frame.done)
        return text;
    return text.slice(0, Math.floor(text.length * Math.min(1, frame.local / ms)));
}
/** All visual tweening uses the player's finite clock, including window handoff. */
