...
| Restore current Session state after Client reconnect | Satelle Host Daemon state plus stable `thread/read` metadata, and `thread/resume` when the adapter must reopen the stored thread | partial | Read with `includeTurns: false` because 0.144.0 rejects the legacy full-history path for paginated threads. `notLoaded`, `idle`, or `systemError` proves that no upstream runtime owns the unresolved Turn, which closes recovery as failed without claiming task success. `active` does not identify its Turn, so Satelle retains recovery as unknown. |
