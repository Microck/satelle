---
title: Generated CLI and schema reference
description: Binary-derived command help and public schema versions for Satelle 0.1.12.
---

This page describes **Satelle 0.1.12**. The documentation contract regenerates
the command sections from native binary and JavaScript launcher help and derives schema identifiers from
public Satelle result and event schema sources. CI rejects hand-edited or stale output.

## Public schema versions

| Public schema source | Production source | Serialized schema version |
| --- | --- | --- |
| `BATCH_REQUEST_SCHEMA_VERSION` | `crates/satelle/src/automation.rs` | `satelle.batch.request.v1` |
| `BATCH_RESULT_SCHEMA_VERSION` | `crates/satelle/src/automation.rs` | `satelle.batch.result.v1` |
| `BATCH_SUMMARY_SCHEMA_VERSION` | `crates/satelle/src/automation.rs` | `satelle.batch.summary.v1` |
| `CONFIG_REPAIR_SCHEMA_VERSION` | `crates/satelle/src/output.rs` | `satelle.config.repair.v1` |
| `DoctorEventSchemaVersion` | `crates/satelle/src/core/mod.rs` | `satelle.doctor.events.v1` |
| `DoctorSchemaVersion` | `crates/satelle/src/core/mod.rs` | `satelle.doctor.v1` |
| `ERROR_SCHEMA_VERSION` | `crates/satelle/src/error-output.rs` | `satelle.error.v1` |
| `EVENT_SCHEMA_VERSION` | `crates/satelle/src/core/events.rs` | `satelle.events.v2` |
| `HostSessionsSchemaVersion` | `crates/satelle/src/core/mod.rs` | `satelle.host.sessions.v1` |
| `HOST_UPDATE_PLAIN_SCHEMA_VERSION` | `crates/satelle/src/output.rs` | `satelle.host.update.plain.v1` |
| `LogEntrySchema` | `crates/satelle/src/host/log-page.rs` | `satelle.logs.entry.v2` |
| `NOTIFY_WEBHOOK_SCHEMA_VERSION` | `crates/satelle/src/automation.rs` | `satelle.notify.webhook.v1` |
| `REPL_TRANSCRIPT_SCHEMA_VERSION` | `crates/satelle/src/automation.rs` | `satelle.repl.transcript.v1` |
| `SessionResultSchemaVersion` | `crates/satelle/src/output.rs` | `satelle.run.v2` |
| `SetupSchemaVersion` | `crates/satelle/src/core/mod.rs` | `satelle.setup.v2` |
| `SessionResultSchemaVersion` | `crates/satelle/src/output.rs` | `satelle.status.v2` |
| `SessionResultSchemaVersion` | `crates/satelle/src/output.rs` | `satelle.steer.v2` |
| `StopSchemaVersion` | `crates/satelle/src/core/mod.rs` | `satelle.stop.v1` |
| `SUPPORT_BUNDLE_SCHEMA_VERSION` | `crates/satelle/src/support.rs` | `satelle.support.bundle.v1` |
| `TELEMETRY_STATUS_SCHEMA_VERSION` | `crates/satelle/src/core/telemetry.rs` | `satelle.telemetry.status.v1` |
| `WATCH_CHANGE_SCHEMA_VERSION` | `crates/satelle/src/automation.rs` | `satelle.watch.change.v1` |

## `satelle`

```text
Satelle sends command uplinks to a visible remote host and returns telemetry from the native computer-use runtime.

Usage: satelle [OPTIONS] <COMMAND>

Commands:
  batch        Execute versioned NDJSON command requests with bounded concurrency
  repl         Run repeated Satelle commands against one selected Host
  watch        Emit versioned NDJSON records when selected Satelle state changes
  notify       Deliver selected Satelle state changes to a user-owned webhook
  completions
  setup
  repair
  doctor
  config
  paths
  host
  desktop
  telemetry    Inspect opt-in Controller or Host telemetry state
  self
  run
  steer
  action
  queue
  status
  stop
  session
  logs
  skills
  mcp
  support
  help         Print this message or the help of the given subcommand(s)

Options:
      --no-color
          Disable colored human output

      --log-verbosity <LEVEL>
          Set diagnostic log verbosity: off, info, debug, or trace

          [possible values: off, info, debug, trace]

      --profile <NAME>
          Apply a named user-level configuration profile

      --error-format <FORMAT>
          Format diagnostics as human-readable text or JSON

          [env: SATELLE_ERROR_FORMAT=]
          [possible values: human, json]

  -h, --help
          Print help (see a summary with '-h')

  -V, --version
          Print version
```

## `satelle native repair`

```text
Usage: satelle native repair [--dry-run]

Reinstall the exact native package through its package manager.
Local repair records an exact optional dependency in the owning project.

  --dry-run   Show the owner and repair command without changing files
  --no-color  Use plain output
  -h, --help  Show this help
```

## `satelle batch`

```text
Execute versioned NDJSON command requests with bounded concurrency

Usage: satelle batch [OPTIONS] --input <PATH_OR_DASH>

Options:
      --input <PATH_OR_DASH>       NDJSON request file, or - for standard input
      --no-color                   Disable colored human output
      --concurrency <CONCURRENCY>  Maximum commands executed at once [default: 4]
      --log-verbosity <LEVEL>      Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --profile <NAME>             Apply a named user-level configuration profile
      --error-format <FORMAT>      Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                       Print help
```

## `satelle repl`

```text
Run repeated Satelle commands against one selected Host

Usage: satelle repl [OPTIONS]

Options:
      --host <HOST>            Select the Host used by every command in this REPL
      --no-color               Disable colored human output
      --export <PATH>          Write one redacted versioned NDJSON record per executed command
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --include-prompts        Preserve inline run and steer prompts in the exported transcript
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                   Print help
```

## `satelle watch`

```text
Emit versioned NDJSON records when selected Satelle state changes

Usage: satelle watch [OPTIONS] <TARGET>

Arguments:
  <TARGET>  [possible values: sessions, logs, doctor, host]

Options:
      --host <HOST>
          Read state from this configured Host
      --no-color
          Disable colored human output
      --log-verbosity <LEVEL>
          Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --session <SESSION>
          Include only log entries for this Session
      --interval <INTERVAL>
          Delay between successful polls, such as 500ms or 2s [default: 2s]
      --profile <NAME>
          Apply a named user-level configuration profile
      --error-format <FORMAT>
          Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
      --reconnect-attempts <RECONNECT_ATTEMPTS>
          Consecutive failed polls allowed before the watch stops [default: 8]
  -h, --help
          Print help
```

## `satelle notify`

```text
Deliver selected Satelle state changes to a user-owned webhook

Usage: satelle notify [OPTIONS] --watch <WATCH> --webhook <WEBHOOK>

Options:
      --no-color
          Disable colored human output
      --watch <WATCH>
          Satelle state category to watch [possible values: sessions, logs, doctor, host]
      --log-verbosity <LEVEL>
          Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --webhook <WEBHOOK>
          User-level webhook notifier alias
      --host <HOST>
          Read state from this configured Host
      --profile <NAME>
          Apply a named user-level configuration profile
      --error-format <FORMAT>
          Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
      --session <SESSION>
          Include only log entries for this Session
      --interval <INTERVAL>
          Delay between successful polls, such as 500ms or 2s [default: 2s]
      --reconnect-attempts <RECONNECT_ATTEMPTS>
          Consecutive failed polls allowed before the notification watch stops [default: 8]
      --dry-run
          Print webhook payloads without making network requests
  -h, --help
          Print help
```

## `satelle completions`

```text
Usage: satelle completions [OPTIONS] [SHELL]

Arguments:
  [SHELL]  [possible values: bash, zsh, fish, powershell]

Options:
      --no-color                Disable colored human output
      --output-dir <DIRECTORY>  Install the completion script in DIRECTORY; detect the shell when omitted
      --log-verbosity <LEVEL>   Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --update-profile <FILE>   Activate the installed script through one managed block in FILE
      --profile <NAME>          Apply a named user-level configuration profile
      --error-format <FORMAT>   Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                    Print help
```

## `satelle setup`

```text
Usage: satelle setup [OPTIONS]

Options:
      --host <HOST>


      --no-color
          Disable colored human output

      --dry-run


      --log-verbosity <LEVEL>
          Set diagnostic log verbosity: off, info, debug, or trace

          [possible values: off, info, debug, trace]

      --profile <NAME>
          Apply a named user-level configuration profile

      --verify
          Run live native Computer Use readiness checks and any required provider smoke test after setup

      --error-format <FORMAT>
          Format diagnostics as human-readable text or JSON

          [env: SATELLE_ERROR_FORMAT=]
          [possible values: human, json]

      --on-demand


      --persistent


      --component <COMPONENT>
          [possible values: transport, host, codex, computer-use, desktop, provider-auth, all]

      --daemon-home <DAEMON_HOME>


      --daemon-config-file <DAEMON_CONFIG_FILE>


      --daemon-state-dir <DAEMON_STATE_DIR>


      --daemon-cache-dir <DAEMON_CACHE_DIR>


      --daemon-log-dir <DAEMON_LOG_DIR>


      --yes
          Accept ordinary setup mutations without an interactive confirmation

      --no-input


      --expected-host-id <HOST_ID>
          Require this exact Host identity during first-time SSH trust

      --raw-subprocess-output
          Capture selected redacted setup subprocess output for this invocation

      --output <PATH>
          Write the raw diagnostic artifact to this local path

      --format <FORMAT>
          [possible values: human, json, compact-json, toon, markdown]

      --json
          Alias for --format json

  -h, --help
          Print help (see a summary with '-h')

Agent-safe noninteractive provider auth flow:
  1. Configure host-resolved Secret Source descriptors in user-level host config.
  2. Run satelle setup --no-input --json to get a stable plan.
  3. Treat missing raw provider secret material as required human input, not as an agent-handled value.
```

## `satelle repair`

```text
Usage: satelle repair [OPTIONS]

Options:
      --host <HOST>
      --no-color               Disable colored human output
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --run <RUN_ID>
      --dry-run
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
      --yes                    Accept ordinary repair mutations without an interactive confirmation
      --no-input
      --raw-subprocess-output  Capture selected redacted repair subprocess output for this invocation
      --output <PATH>          Write the raw diagnostic artifact to this local path
      --format <FORMAT>        [possible values: human, json, compact-json, toon, markdown]
      --json                   Alias for --format json
  -h, --help                   Print help
```

## `satelle doctor`

```text
Usage: satelle doctor [OPTIONS]

Options:
      --host <HOST>
      --no-color               Disable colored human output
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --scope <SCOPE>
      --profile <NAME>         Apply a named user-level configuration profile
      --refresh
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
      --timeout <TIMEOUT>
      --serial-probes
      --events
      --fix
      --dry-run
      --yes
      --no-input
      --quiet
      --format <FORMAT>        [possible values: human, json, compact-json, toon, markdown]
      --json                   Alias for --format json
  -h, --help                   Print help
```

## `satelle config`

```text
Usage: satelle config [OPTIONS] <COMMAND>

Commands:
  check
  explain
  repair
  help     Print this message or the help of the given subcommand(s)

Options:
      --no-color               Disable colored human output
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                   Print help
```

## `satelle paths`

```text
Usage: satelle paths [OPTIONS]

Options:
      --host <HOST>
      --no-color               Disable colored human output
      --format <FORMAT>        [possible values: human, json, compact-json, toon, markdown]
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --json                   Alias for --format json
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                   Print help
```

## `satelle host`

```text
Usage: satelle host [OPTIONS] <COMMAND>

Commands:
  start
  trust     Authenticate a direct Host and pin its stable identity in user configuration
  status
  stop
  restart
  update
  cleanup
  sessions
  storage
  store
  help      Print this message or the help of the given subcommand(s)

Options:
      --no-color               Disable colored human output
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                   Print help
```

## `satelle desktop`

```text
Usage: satelle desktop [OPTIONS] <COMMAND>

Commands:
  snapshot
  help      Print this message or the help of the given subcommand(s)

Options:
      --no-color               Disable colored human output
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                   Print help
```

## `satelle telemetry`

```text
Inspect opt-in Controller or Host telemetry state

Usage: satelle telemetry [OPTIONS] <COMMAND>

Commands:
  status  Report policy, endpoint origin, queue use, delivery outcome, and exclusions
  help    Print this message or the help of the given subcommand(s)

Options:
      --no-color               Disable colored human output
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                   Print help
```

## `satelle self`

```text
Usage: satelle self [OPTIONS] <COMMAND>

Commands:
  update
  help    Print this message or the help of the given subcommand(s)

Options:
      --no-color               Disable colored human output
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                   Print help
```

## `satelle run`

```text
Usage: satelle run [OPTIONS] [PROMPT_OR_DASH]

Arguments:
  [PROMPT_OR_DASH]  Prompt text or '-' for stdin. Prompt arguments may be retained by shell history or visible in local process metadata; use stdin or --prompt-file for sensitive input

Options:
      --host <HOST>

      --no-color
          Disable colored human output
      --desktop-binding <ALIAS>

      --log-verbosity <LEVEL>
          Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --model <ALIAS>
          Model alias; must resolve with the effective provider alias as one exact Host Provider Binding pair
      --profile <NAME>
          Apply a named user-level configuration profile
      --error-format <FORMAT>
          Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
      --provider <ALIAS>
          Provider alias; must resolve with the effective model alias as one exact Host Provider Binding pair
      --detach

      --queue
          Persist this Turn until the Host can admit it
      --detach-on-interrupt
          Leave an admitted Turn running when the attached command is interrupted
      --quiet

      --verbose

      --experimental-provider-computer-use
          Experimental: attempt non-OpenAI provider Computer Use; behavior may not work correctly and still requires a live provider Computer Use smoke test
      --refresh-provider-smoke-test
          Refresh the cached provider Computer Use smoke-test result before execution; this does not enable experimental provider Computer Use
      --yolo
          Enable YOLO for this Turn with approval policy never and sandbox policy danger-full-access; native app or operating-system prompts remain human decisions
      --no-yolo
          Disable YOLO mode for this command
      --relay-native-actions
          Relay supported native Computer Use action requests for explicit allow or deny decisions
      --events <EVENTS>
          [default: auto] [possible values: auto, human, json, none]
      --prompt-file <PROMPT_FILE>
          Read prompt text from a local file; recommended for sensitive prompt input
      --image <LOCAL_PATH>
          Attach a local PNG or JPEG image (maximum 2, 5 MiB each, 10 MiB total); accepted media types depend on Host capabilities
      --remote-image <HOST_PATH>
          Attach an absolute PNG or JPEG path on the selected remote Host; shares the limits of --image
      --timeout <DURATION>
          Shorten this Turn's execution timeout (for example 30s, 5m, or 1h; maximum 24h)
      --raw-protocol
          Capture this prospective Turn's redacted Codex app-server JSON protocol
      --output <PATH>
          Write the raw diagnostic artifact to this local path
      --no-input
          Disable raw diagnostic consent input
      --yes
          Confirm this invocation's raw diagnostic warning in noninteractive mode
      --record <MODE>
          Record this prospective Turn as events, transcript, screenshots, or video [possible values: events, transcript, screenshots, video]
      --record-retention <DURATION>
          Retain this recording for a duration such as 30m, 24h, or 7d
      --format <FORMAT>
          [possible values: human, json, compact-json, toon, markdown]
      --json
          Alias for --format json
  -h, --help
          Print help
```

## `satelle steer`

```text
Usage: satelle steer [OPTIONS] <SESSION_ID> [PROMPT_OR_DASH]

Arguments:
  <SESSION_ID>
  [PROMPT_OR_DASH]  Prompt text or '-' for stdin. Prompt arguments may be retained by shell history or visible in local process metadata; use stdin or --prompt-file for sensitive input

Options:
      --host <HOST>

      --no-color
          Disable colored human output
      --desktop-binding <ALIAS>

      --log-verbosity <LEVEL>
          Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --model <ALIAS>
          Model alias; must resolve with the effective provider alias as one exact Host Provider Binding pair
      --profile <NAME>
          Apply a named user-level configuration profile
      --error-format <FORMAT>
          Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
      --provider <ALIAS>
          Provider alias; must resolve with the effective model alias as one exact Host Provider Binding pair
      --detach

      --queue
          Persist this Turn until the Host can admit it
      --detach-on-interrupt
          Leave an admitted Turn running when the attached command is interrupted
      --quiet

      --verbose

      --experimental-provider-computer-use
          Experimental: attempt non-OpenAI provider Computer Use; behavior may not work correctly and still requires a live provider Computer Use smoke test
      --refresh-provider-smoke-test
          Refresh the cached provider Computer Use smoke-test result before execution; this does not enable experimental provider Computer Use
      --yolo
          Enable YOLO for this Turn with approval policy never and sandbox policy danger-full-access; native app or operating-system prompts remain human decisions
      --no-yolo
          Disable YOLO mode for this command
      --relay-native-actions
          Relay supported native Computer Use action requests for explicit allow or deny decisions
      --events <EVENTS>
          [default: auto] [possible values: auto, human, json, none]
      --prompt-file <PROMPT_FILE>
          Read prompt text from a local file; recommended for sensitive prompt input
      --image <LOCAL_PATH>
          Attach a local PNG or JPEG image (maximum 2, 5 MiB each, 10 MiB total); accepted media types depend on Host capabilities
      --remote-image <HOST_PATH>
          Attach an absolute PNG or JPEG path on the selected remote Host; shares the limits of --image
      --timeout <DURATION>
          Shorten this Turn's execution timeout (for example 30s, 5m, or 1h; maximum 24h)
      --raw-protocol
          Capture this prospective Turn's redacted Codex app-server JSON protocol
      --output <PATH>
          Write the raw diagnostic artifact to this local path
      --no-input
          Disable raw diagnostic consent input
      --yes
          Confirm this invocation's raw diagnostic warning in noninteractive mode
      --record <MODE>
          Record this prospective Turn as events, transcript, screenshots, or video [possible values: events, transcript, screenshots, video]
      --record-retention <DURATION>
          Retain this recording for a duration such as 30m, 24h, or 7d
      --format <FORMAT>
          [possible values: human, json, compact-json, toon, markdown]
      --json
          Alias for --format json
  -h, --help
          Print help
```

## `satelle action`

```text
Usage: satelle action [OPTIONS] <COMMAND>

Commands:
  respond  Answer a detached native action request
  help     Print this message or the help of the given subcommand(s)

Options:
      --no-color               Disable colored human output
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                   Print help
```

## `satelle queue`

```text
Usage: satelle queue [OPTIONS] <COMMAND>

Commands:
  status  Read the durable state of one queued Turn
  cancel  Cancel one queued Turn before admission
  help    Print this message or the help of the given subcommand(s)

Options:
      --no-color               Disable colored human output
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                   Print help
```

## `satelle status`

```text
Usage: satelle status [OPTIONS] <SESSION_ID>

Arguments:
  <SESSION_ID>

Options:
      --host <HOST>
      --no-color               Disable colored human output
      --format <FORMAT>        [possible values: human, json, compact-json, toon, markdown]
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --json                   Alias for --format json
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                   Print help
```

## `satelle stop`

```text
Usage: satelle stop [OPTIONS] <SESSION_ID>

Arguments:
  <SESSION_ID>

Options:
      --host <HOST>
      --no-color               Disable colored human output
      --format <FORMAT>        [possible values: human, json, compact-json, toon, markdown]
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --json                   Alias for --format json
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                   Print help
```

## `satelle session`

```text
Usage: satelle session [OPTIONS] <COMMAND>

Commands:
  export
  help    Print this message or the help of the given subcommand(s)

Options:
      --no-color               Disable colored human output
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                   Print help
```

## `satelle logs`

```text
Usage: satelle logs [OPTIONS]

Options:
      --host <ALIAS>                 Read logs from this configured Host
      --no-color                     Disable colored human output
      --log-verbosity <LEVEL>        Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --session <SESSION_ID>         Include only entries for this Session
      --profile <NAME>               Apply a named user-level configuration profile
      --tail <COUNT>                 Return the most recent 1 to 10000 matching entries (default: 200)
      --error-format <FORMAT>        Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
      --since <DURATION_OR_RFC3339>  Include entries at or after this duration or RFC 3339 time
      --after <LOG_CURSOR>           Return entries strictly after this opaque cursor; conflicts with --since and --tail
      --source <SOURCE>              Include a source: host_daemon, storage, or codex_adapter; repeat to select multiple
      --level <SEVERITY>             Set minimum severity: info, warn, or error (default: info)
  -f, --follow                       Continue streaming new matching Log Entries
      --no-reconnect                 Fail on transport loss instead of reconnecting
      --format <FORMAT>              [possible values: human, json]
      --json                         Alias for --format json
  -h, --help                         Print help
```

## `satelle skills`

```text
Usage: satelle skills [OPTIONS] <COMMAND>

Commands:
  list
  get
  path
  help  Print this message or the help of the given subcommand(s)

Options:
      --no-color               Disable colored human output
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                   Print help
```

## `satelle mcp`

```text
Usage: satelle mcp [OPTIONS] <COMMAND>

Commands:
  serve
  install
  help     Print this message or the help of the given subcommand(s)

Options:
      --no-color               Disable colored human output
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                   Print help
```

## `satelle support`

```text
Usage: satelle support [OPTIONS] <COMMAND>

Commands:
  bundle  Export a redacted diagnostic bundle
  help    Print this message or the help of the given subcommand(s)

Options:
      --no-color               Disable colored human output
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                   Print help
```

## `satelle config check`

```text
Usage: satelle config check [OPTIONS]

Options:
      --host <HOST>
      --no-color               Disable colored human output
      --all
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --format <FORMAT>        [possible values: human, json, compact-json, toon, markdown]
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
      --json                   Alias for --format json
  -h, --help                   Print help
```

## `satelle config explain`

```text
Usage: satelle config explain [OPTIONS]

Options:
      --host <HOST>
      --no-color                Disable colored human output
      --log-verbosity <LEVEL>   Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --show-secret-references
      --format <FORMAT>         [possible values: human, json, compact-json, toon, markdown]
      --profile <NAME>          Apply a named user-level configuration profile
      --error-format <FORMAT>   Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
      --json                    Alias for --format json
  -h, --help                    Print help
```

## `satelle config repair`

```text
Usage: satelle config repair [OPTIONS]

Options:
      --file <FILE>            Select one loaded local configuration file; project files require this flag
      --no-color               Disable colored human output
      --host <HOST>            Scope Trusted Profile consent to edits inside this Host Binding
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --dry-run
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
      --yes                    Accept the displayed local configuration edits and private backup
      --no-input
      --format <FORMAT>        [possible values: human, json, compact-json, toon, markdown]
      --json                   Alias for --format json
  -h, --help                   Print help
```

## `satelle host start`

```text
Usage: satelle host start [OPTIONS]

Options:
      --bind <BIND>            [default: 127.0.0.1:3001]
      --no-color               Disable colored human output
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --tls-cert <PATH>        PEM certificate chain for Host-terminated HTTPS and WSS
      --profile <NAME>         Apply a named user-level configuration profile
      --tls-key <PATH>         Owner-only PEM private key matching --tls-cert
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
      --tls-client-ca <PATH>   Require client certificates signed by this PEM CA bundle
      --tls-client-crl <PATH>  Signed client revocation lists, including a current list for each issuing CA
      --foreground
      --format <FORMAT>        [possible values: human, json, compact-json, toon, markdown]
      --json                   Alias for --format json
  -h, --help                   Print help
```

## `satelle host trust`

```text
Authenticate a direct Host and pin its stable identity in user configuration

Usage: satelle host trust [OPTIONS] --host <HOST>

Options:
      --host <HOST>                 Host Binding alias to authenticate and trust
      --no-color                    Disable colored human output
      --expected-host-id <HOST_ID>  Exact identity required before a noninteractive trust update
      --log-verbosity <LEVEL>       Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --profile <NAME>              Apply a named user-level configuration profile
      --replace                     Permit replacement when the Host Binding already pins a different identity
      --error-format <FORMAT>       Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
      --yes                         Apply trust without an interactive confirmation
      --no-input                    Reject any path that would prompt for input
      --format <FORMAT>             [possible values: human, json, compact-json, toon, markdown]
      --json                        Alias for --format json
  -h, --help                        Print help
```

## `satelle host status`

```text
Usage: satelle host status [OPTIONS]

Options:
      --host <HOST>
      --no-color               Disable colored human output
      --format <FORMAT>        [possible values: human, json, compact-json, toon, markdown]
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --json                   Alias for --format json
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                   Print help
```

## `satelle host stop`

```text
Usage: satelle host stop [OPTIONS]

Options:
      --host <HOST>
      --no-color               Disable colored human output
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --yes
      --no-input
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
      --format <FORMAT>        [possible values: human, json, compact-json, toon, markdown]
      --json                   Alias for --format json
  -h, --help                   Print help
```

## `satelle host restart`

```text
Usage: satelle host restart [OPTIONS]

Options:
      --host <HOST>
      --no-color               Disable colored human output
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --yes
      --no-input
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
      --format <FORMAT>        [possible values: human, json, compact-json, toon, markdown]
      --json                   Alias for --format json
  -h, --help                   Print help
```

## `satelle host update`

```text
Usage: satelle host update [OPTIONS]

Options:
      --host <HOST>
      --no-color               Disable colored human output
      --component <COMPONENT>
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --profile <NAME>         Apply a named user-level configuration profile
      --version <VERSION>      Select a compatible stable Host release; requires --component host
      --all-remotes
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
      --dry-run
      --yes
      --no-input
      --quiet                  Suppress non-error human plan and progress output; use --json for stable automation data
      --plain                  Emit fixed tab-separated target records; use --json for the complete report
      --format <FORMAT>        [possible values: human, json, compact-json, toon, markdown]
      --json                   Alias for --format json
  -h, --help                   Print help
```

## `satelle host cleanup`

```text
Usage: satelle host cleanup [OPTIONS]

Options:
      --host <HOST>
      --no-color               Disable colored human output
      --format <FORMAT>        [possible values: human, json, compact-json, toon, markdown]
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --json                   Alias for --format json
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                   Print help
```

## `satelle host sessions`

```text
Usage: satelle host sessions [OPTIONS]

Options:
      --host <HOST>
      --no-color               Disable colored human output
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --no-bootstrap
      --format <FORMAT>        [possible values: human, json, compact-json, toon, markdown]
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
      --json                   Alias for --format json
  -h, --help                   Print help
```

## `satelle host storage`

```text
Usage: satelle host storage [OPTIONS] <COMMAND>

Commands:
  migrate
  complete  Confirm a staged migration after a lost completion response
  source
  restore
  backup
  help      Print this message or the help of the given subcommand(s)

Options:
      --no-color               Disable colored human output
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                   Print help
```

## `satelle host store`

```text
Usage: satelle host store [OPTIONS] <COMMAND>

Commands:
  reset
  help   Print this message or the help of the given subcommand(s)

Options:
      --no-color               Disable colored human output
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                   Print help
```

## `satelle desktop snapshot`

```text
Usage: satelle desktop snapshot [OPTIONS] --host <HOST> --output <PATH>

Options:
      --host <HOST>
      --no-color                 Disable colored human output
      --desktop-binding <ALIAS>
      --log-verbosity <LEVEL>    Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --output <PATH>
      --profile <NAME>           Apply a named user-level configuration profile
      --error-format <FORMAT>    Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
      --no-input
      --format <FORMAT>          [possible values: human, json, compact-json, toon, markdown]
      --json                     Alias for --format json
  -h, --help                     Print help
```

## `satelle telemetry status`

```text
Report policy, endpoint origin, queue use, delivery outcome, and exclusions

Usage: satelle telemetry status [OPTIONS]

Options:
      --host <HOST>            Inspect one Host instead of the local Controller
      --no-color               Disable colored human output
      --format <FORMAT>        [possible values: human, json, compact-json, toon, markdown]
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --json                   Alias for --format json
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                   Print help
```

## `satelle self update`

```text
Usage: satelle self update [OPTIONS]

Options:
      --dry-run
      --no-color                   Disable colored human output
      --log-verbosity <LEVEL>      Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --version <VERSION>
      --profile <NAME>             Apply a named user-level configuration profile
      --update-remotes
      --error-format <FORMAT>      Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
      --host <HOST>
      --all-remotes
      --component <COMPONENT>
      --concurrency <CONCURRENCY>  [default: 4]
      --no-input
      --yes
      --quiet                      Suppress non-error human remote-update output; use --json for stable automation data
      --format <FORMAT>            [possible values: human, json, compact-json, toon, markdown]
      --json                       Alias for --format json
  -h, --help                       Print help
```

## `satelle action respond`

```text
Answer a detached native action request

Usage: satelle action respond [OPTIONS] <ACTION_REQUEST_ID>

Arguments:
  <ACTION_REQUEST_ID>

Options:
      --allow
      --no-color               Disable colored human output
      --deny
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                   Print help
```

## `satelle queue status`

```text
Read the durable state of one queued Turn

Usage: satelle queue status [OPTIONS] <QUEUE_REQUEST_ID>

Arguments:
  <QUEUE_REQUEST_ID>

Options:
      --host <HOST>
      --no-color               Disable colored human output
      --format <FORMAT>        [possible values: human, json, compact-json, toon, markdown]
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --json                   Alias for --format json
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                   Print help
```

## `satelle queue cancel`

```text
Cancel one queued Turn before admission

Usage: satelle queue cancel [OPTIONS] <QUEUE_REQUEST_ID>

Arguments:
  <QUEUE_REQUEST_ID>

Options:
      --host <HOST>
      --no-color               Disable colored human output
      --format <FORMAT>        [possible values: human, json, compact-json, toon, markdown]
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --json                   Alias for --format json
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                   Print help
```

## `satelle session export`

```text
Usage: satelle session export [OPTIONS] --host <HOST> --output <OUTPUT> <SESSION_ID>

Arguments:
  <SESSION_ID>

Options:
      --host <HOST>
      --no-color               Disable colored human output
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --output <OUTPUT>
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                   Print help
```

## `satelle skills list`

```text
Usage: satelle skills list [OPTIONS]

Options:
      --format <FORMAT>        [possible values: human, json, compact-json, toon, markdown, csv]
      --no-color               Disable colored human output
      --json                   Alias for --format json
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                   Print help
```

## `satelle skills get`

```text
Usage: satelle skills get [OPTIONS] <NAME>

Arguments:
  <NAME>

Options:
      --format <FORMAT>        [possible values: human, json, compact-json, toon, markdown]
      --no-color               Disable colored human output
      --json                   Alias for --format json
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                   Print help
```

## `satelle skills path`

```text
Usage: satelle skills path [OPTIONS] <NAME>

Arguments:
  <NAME>

Options:
      --format <FORMAT>        [possible values: human, json, compact-json, toon, markdown]
      --no-color               Disable colored human output
      --json                   Alias for --format json
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                   Print help
```

## `satelle mcp serve`

```text
Usage: satelle mcp serve [OPTIONS]

Options:
      --enable-mutations       Expose mutating MCP tools that retain the equivalent CLI consent and safety rules
      --no-color               Disable colored human output
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                   Print help
```

## `satelle mcp install`

```text
Usage: satelle mcp install [OPTIONS]

Options:
      --no-color                     Disable colored human output
      --target <TARGET>              [possible values: claude-code, claude-desktop, codex, cursor, vscode, windsurf, gemini, opencode, cline, roo-code, droid, antigravity]
      --all
      --log-verbosity <LEVEL>        Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --profile <NAME>               Apply a named user-level configuration profile
      --server-name <SERVER_NAME>    [default: satelle]
      --error-format <FORMAT>        Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
      --satelle-path <SATELLE_PATH>
      --dry-run
      --enable-mutations             Configure the MCP server with explicitly enabled mutating tools
      --format <FORMAT>              [possible values: human, json, compact-json, toon, markdown]
      --json                         Alias for --format json
  -h, --help                         Print help
```

## `satelle support bundle`

```text
Export a redacted diagnostic bundle

Usage: satelle support bundle [OPTIONS]

Options:
      --host <HOST>
      --no-color               Disable colored human output
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --output <OUTPUT>
      --no-input
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
      --yes
      --format <FORMAT>        [possible values: human, json, compact-json, toon, markdown]
      --json                   Alias for --format json
  -h, --help                   Print help
```

## `satelle host storage migrate`

```text
Usage: satelle host storage migrate [OPTIONS] --to <TO>

Options:
      --host <HOST>
      --no-color               Disable colored human output
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --to <TO>
      --dry-run
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
      --yes
      --no-input
      --format <FORMAT>        [possible values: human, json, compact-json, toon, markdown]
      --json                   Alias for --format json
  -h, --help                   Print help
```

## `satelle host storage complete`

```text
Confirm a staged migration after a lost completion response

Usage: satelle host storage complete [OPTIONS] --host <HOST> --operation-id <OPERATION_ID>

Options:
      --host <HOST>
      --no-color                     Disable colored human output
      --log-verbosity <LEVEL>        Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --operation-id <OPERATION_ID>
      --profile <NAME>               Apply a named user-level configuration profile
      --yes
      --error-format <FORMAT>        Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
      --format <FORMAT>              [possible values: human, json, compact-json, toon, markdown]
      --json                         Alias for --format json
  -h, --help                         Print help
```

## `satelle host storage source`

```text
Usage: satelle host storage source [OPTIONS] <COMMAND>

Commands:
  cleanup
  help     Print this message or the help of the given subcommand(s)

Options:
      --no-color               Disable colored human output
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                   Print help
```

## `satelle host storage restore`

```text
Usage: satelle host storage restore [OPTIONS] --backup <BACKUP>

Options:
      --host <HOST>
      --no-color               Disable colored human output
      --backup <BACKUP>
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --dry-run
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
      --yes
      --no-input
      --format <FORMAT>        [possible values: human, json, compact-json, toon, markdown]
      --json                   Alias for --format json
  -h, --help                   Print help
```

## `satelle host storage backup`

```text
Usage: satelle host storage backup [OPTIONS] <COMMAND>

Commands:
  cleanup
  help     Print this message or the help of the given subcommand(s)

Options:
      --no-color               Disable colored human output
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
  -h, --help                   Print help
```

## `satelle host store reset`

```text
Usage: satelle host store reset [OPTIONS]

Options:
      --host <HOST>
      --no-color               Disable colored human output
      --delete-recordings
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --dry-run
      --profile <NAME>         Apply a named user-level configuration profile
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
      --yes
      --no-input
      --format <FORMAT>        [possible values: human, json, compact-json, toon, markdown]
      --json                   Alias for --format json
  -h, --help                   Print help
```

## `satelle host storage source cleanup`

```text
Usage: satelle host storage source cleanup [OPTIONS] --host <HOST> --operation-id <OPERATION_ID>

Options:
      --host <HOST>
      --no-color                     Disable colored human output
      --log-verbosity <LEVEL>        Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --operation-id <OPERATION_ID>
      --dry-run
      --profile <NAME>               Apply a named user-level configuration profile
      --error-format <FORMAT>        Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
      --yes
      --no-input
      --format <FORMAT>              [possible values: human, json, compact-json, toon, markdown]
      --json                         Alias for --format json
  -h, --help                         Print help
```

## `satelle host storage backup cleanup`

```text
Usage: satelle host storage backup cleanup [OPTIONS]

Options:
      --host <HOST>
      --no-color               Disable colored human output
      --dry-run
      --log-verbosity <LEVEL>  Set diagnostic log verbosity: off, info, debug, or trace [possible values: off, info, debug, trace]
      --profile <NAME>         Apply a named user-level configuration profile
      --yes
      --error-format <FORMAT>  Format diagnostics as human-readable text or JSON [env: SATELLE_ERROR_FORMAT=] [possible values: human, json]
      --no-input
      --format <FORMAT>        [possible values: human, json, compact-json, toon, markdown]
      --json                   Alias for --format json
  -h, --help                   Print help
```
