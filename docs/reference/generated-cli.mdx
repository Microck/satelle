...
Options:
      ...
      --events <EVENTS>
          [default: auto]
          [possible values: auto, human, json, none]
...
Security: a positional prompt may be retained in shell history or visible in local process metadata. Use standard input with PROMPT_OR_DASH set to '-' or --prompt-file for sensitive prompt text.
```
...
Options:
  ...
  -f, --follow                       Continue streaming matching entries until interrupted or failed
      --no-reconnect                 Fail immediately if a follow stream loses its Host connection
      --format <FORMAT>              [possible values: human, json]
      --json                         Alias for --format json
  -h, --help                         Print help
```
