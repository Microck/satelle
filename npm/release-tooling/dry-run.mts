...
`,
  ...
  const generatedChangelog = readFileSync(
    path.join(fixtureRoot, "crates/core/CHANGELOG.md"),
    "utf8",
  );
  ...
  if (
    !generatedChangelog.includes("1.0.1") ||
    !generatedChangelog.includes("Exercise Cargo release behavior")
  ) {
    throw new Error("Tegami Cargo dry run did not generate the versioned changelog output");
  }
  ...
  const report = {
    ...
    cargoPlugin: {
      ...
      changelogGeneration: "1.0.0 -> 1.0.1",
