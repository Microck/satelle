...
test("npm promotion audit blocks another version until its transaction is terminal", (context) => {
  ...
  writeJson(path.join(records, "other.json"), other);
  ...
  writeJson(path.join(records, "other.json"), other);
  assert.equal(auditRecords(records, "1.2.3"), null);
});
...
test("release workflow gates draft publication on candidate validation and promotion", () => {
  ...
  assert.match(
    workflow,
    /npm-promotion\.cjs advance[\s\S]*npm-promotion\.cjs verify-complete[\s\S]*gh release edit .*--draft=false --latest/,
  );
  assert.match(
    workflow,
  ...
  assert.match(
    workflow,
    /rollback-promotion:[\s\S]*gh api --paginate "repos\/\$GITHUB_REPOSITORY\/releases\?per_page=100"/,
  );
  assert.match(workflow, /startswith\("npm-candidate-"\).*startswith\("npm-promotion-"\)/s);
});
