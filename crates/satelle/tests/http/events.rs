...
use satelle::core::{EventStateSubject, EventType, SatelleEvent};
use satelle::test_contract::assert_privacy_canaries_absent;
use satelle::transport::{EventSubscription, SessionResponse, SubscribeRequest, WsServerControl};
...
async fn expect_subscribed(
    socket: &mut EventSocket,
    host_identity: &str,
) -> satelle::transport::SubscribedResponse {
...
async fn event_socket_streams_only_post_subscription_commits_in_order() {
    ...
    let admitted = running
        ...
        .json(&satelle::transport::TurnRequest::new(&prompt))
        .send()
        .await
...
async fn daemon_event_client_validates_the_subscription_and_event_stream() {
    ...
    let client = satelle::transport::DaemonEventClient::loopback(
        running.server.local_addr(),
        token,
...
async fn daemon_event_client_preserves_typed_handshake_rejections() {
    ...
    assert!(matches!(
        error,
        satelle::transport::DaemonEventError::Handshake { status, error }
