...
async fn expect_control_failure(
    ...
    expected_reason: satelle::transport::WsCloseReason,
) {
...
fn oversized_subscriptions() -> Vec<Value> {
    (0..17)
        .map(|_| {
            serde_json::json!({
                "kind": "session",
                "session_id": satelle::core::SessionId::new()
            })
        })
        .collect()
}
...
async fn inbound_allowance_is_shared_across_token_ids_for_one_principal() {
    ...
    send_subscribe(&mut first, vec![EventSubscription::Host]).await;
    expect_control_failure(&mut first, satelle::transport::WsCloseReason::RateLimited).await;
}
...
async fn custom_inbound_message_allowance_is_enforced() {
    ...
    expect_control_failure(&mut socket, satelle::transport::WsCloseReason::RateLimited).await;
}
...
async fn every_non_close_frame_kind_consumes_inbound_allowance() {
    ...
    expect_control_failure(
        &mut binary,
        satelle::transport::WsCloseReason::InvalidRequest,
    )
    .await;
    ...
    expect_control_failure(
        &mut survivor,
        satelle::transport::WsCloseReason::RateLimited,
    )
    .await;
}
...
async fn failing_sockets_do_not_interrupt_an_unrelated_live_subscriber() {
    ...
    let admitted = running
        ...
        .json(&satelle::transport::TurnRequest::new(
    ...
    expect_control_failure(
        ...
        satelle::transport::WsCloseReason::CapacityExceeded,
    )
    .await;
...
async fn oversized_messages_are_payload_too_large_failures() {
    ...
    expect_control_failure(
        &mut socket,
        satelle::transport::WsCloseReason::PayloadTooLarge,
    )
    .await;
}
