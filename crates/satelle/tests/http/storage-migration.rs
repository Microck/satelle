use super::*;
use satelle::host::SetupRunStatus;
use satelle::transport::StorageMigrationPathsRequest;
...
async fn storage_migration_completion_checks_admin_paths_and_durable_staging_before_release() {
    ...
    satelle::core::open_or_create_owner_only_directory(copied_log.parent().unwrap()).unwrap();
    satelle::core::persist_new_owner_only_config_file(&copied_log, b"original log\n").unwrap();
    let retained_controller =
        std::path::Path::new(&source_paths.state_root).join("controller-only.json");
    satelle::core::persist_new_owner_only_config_file(&retained_controller, b"controller state\n")
        .unwrap();
    ...
    satelle::core::persist_new_owner_only_config_file(&retained_log, b"new local file\n").unwrap();
