use super::*;
use satelle::transport::EventSubscription;
...
fn control_error_fixture(
    request_id: Value,
    host_identity: &str,
    reason: satelle::transport::WsCloseReason,
) -> Value {
    let (code, category, retryable) = match reason {
        satelle::transport::WsCloseReason::SlowConsumer => ("capacity-exceeded", "capacity", false),
        satelle::transport::WsCloseReason::IdleTimeout => ("rate-limited", "rate_limit", true),
        satelle::transport::WsCloseReason::AuthenticationFailed => {
            ("authentication-failed", "authentication", false)
        }
        satelle::transport::WsCloseReason::ServerShutdown => {
            ("host-unreachable", "readiness", true)
        }
...
async fn daemon_event_client_rejects_a_mismatched_acknowledgement_identity() {
    ...
    let client = satelle::transport::DaemonEventClient::loopback(address, token, "host-expected")
    ...
    assert!(matches!(
        error,
        satelle::transport::DaemonEventError::HostIdentityMismatch
    ));
    server.abort();
    let _ = server.await;
}
...
async fn daemon_event_client_validates_post_ack_subscribed_request_id() {
    ...
    assert!(
        matches!(
            &error,
            satelle::transport::DaemonEventError::RequestIdMismatch
        ),
...
async fn daemon_event_client_rejects_context_valid_unsolicited_subscribed_frame() {
    ...
    assert!(matches!(
        error,
        satelle::transport::DaemonEventError::UnexpectedFrame
    ));
    server.abort();
    let _ = server.await;
}
...
async fn daemon_event_client_validates_pre_ack_error_context() {
    for wrong_request_id in [false, true] {
        let (address, server) = spawn_event_protocol_fixture(move |request| {
            ...
            let reason = satelle::transport::WsCloseReason::AuthenticationFailed;
            vec![
                axum::extract::ws::Message::Text(
        ...
        let token = ApiBearerToken::generate().expect("generate fixture token");
        let client =
            satelle::transport::DaemonEventClient::loopback(address, token, "host-expected")
                .expect("construct event client");
        let error = match client.connect_events(vec![EventSubscription::Host]).await {
...
async fn daemon_event_client_preserves_every_ambiguous_close_reason() {
    for reason in [
        satelle::transport::WsCloseReason::SlowConsumer,
        satelle::transport::WsCloseReason::IdleTimeout,
        satelle::transport::WsCloseReason::AuthenticationFailed,
        satelle::transport::WsCloseReason::ServerShutdown,
    ] {
        let (address, server) = spawn_event_protocol_fixture(move |request| {
        ...
        assert!(matches!(
            error,
            satelle::transport::DaemonEventError::Closed {
                code,
                reason: actual,
...
async fn daemon_event_client_validates_post_ack_error_context() {
    for wrong_request_id in [false, true] {
        let reason = satelle::transport::WsCloseReason::ServerShutdown;
        let (address, server) = spawn_event_protocol_fixture(move |request| {
            let request_id = if wrong_request_id {
                serde_json::to_value(RequestId::new()).expect("serialize wrong request ID")
            } else {
...
async fn daemon_event_client_rejects_control_and_close_mismatches() {
    for (control_reason, close_reason, close_code) in [
        (
            satelle::transport::WsCloseReason::SlowConsumer,
            satelle::transport::WsCloseReason::IdleTimeout,
            satelle::transport::WsCloseReason::IdleTimeout.close_code(),
        ),
        (
            satelle::transport::WsCloseReason::ServerShutdown,
            satelle::transport::WsCloseReason::ServerShutdown,
            satelle::transport::WsCloseReason::SlowConsumer.close_code(),
        ),
    ] {
        let (address, server) = spawn_event_protocol_fixture(move |request| {
            vec![
        ...
        assert!(matches!(
            error,
            satelle::transport::DaemonEventError::CloseContractMismatch { .. }
        ));
        server.abort();
        let _ = server.await;
    }
