...
use satelle::host::{ApiBearerToken, test_support::TestStateDir};
#[cfg(target_os = "linux")]
use satelle::host::{ApiScopes, HostService};
use satelle::test_contract::{assert_directory_tree_unchanged, assert_privacy_canaries_absent};
...
fn storage_migration_moves_a_real_local_daemon_and_preserves_its_source_until_cleanup() {
    ...
    let host = config
        .hosts
        .get_mut(satelle::core::LOCAL_DEMO_HOST)
        .unwrap();
    ...
    host.daemon_idle_timeout = satelle::core::ExplicitDuration::parse("2s");
    let service = satelle::host::HostService::production_for_host(host);
...
fn bundled_skills_are_versioned_discoverable_and_materialized_byte_exact() {
    ...
    let source = fs::read(
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("assets/skills/satelle/SKILL.md"),
    )
...
fn support_bundle_writes_a_redacted_archive_and_json_report() {
    ...
    let state = state_dir();
    let service = satelle::host::HostService::local_demo_for_tests_at(state.path()).unwrap();
    let now = time::OffsetDateTime::now_utc();
    let plan = satelle::host::SetupRunPlan::new(
        PRIVATE,
        satelle::host::SetupOperationKind::Repair,
        None,
        now,
        vec![satelle::host::SetupActionPlan::new(PRIVATE, PRIVATE, true).unwrap()],
    )
    .unwrap();
