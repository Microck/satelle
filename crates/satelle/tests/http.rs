...
use satelle::core::session::TurnExecutionMode;
use satelle::core::{
...
async fn host_telemetry_status_is_an_authenticated_typed_read() {
    ...
    assert_eq!(
        telemetry.component,
        satelle::core::telemetry::TelemetryComponent::Host
    );
    assert!(!telemetry.enabled);
    assert_eq!(
        telemetry.excluded_data_categories,
        satelle::core::telemetry::TELEMETRY_EXCLUSIONS
            .iter()
...
async fn bearer_tokens_outside_authorization_are_rejected() {
    ...
    let requests = [
        ...
        running
            ...
            .json(&satelle::transport::TurnRequest::new(format!(
        ...
        running
            ...
            .header("Content-Type", "application/json")
            .body(format!(
                r#"{{"schema_version":"satelle.api.v12","desktop_binding":"local-demo-desktop-v1","model_from_project":false,"provider_from_project":false,"prompt":"{token}","prompt":"safe","execution_mode":"standard"}}"#
            )),
    ];
...
async fn ssh_bootstrap_authentication_rejects_non_loopback_tls_before_listening() {
    ...
    let host_config = SatelleConfig::defaults()
        .hosts
        .remove(satelle::core::LOCAL_DEMO_HOST)
...
async fn persistent_host_service_maintenance_routes_enforce_action_order() {
    ...
    assert!(
        after_unknown
            .actions()
            .iter()
            .all(|action| action.status() == satelle::host::SetupActionStatus::Planned),
    ...
    for (index, action_id) in action_ids.iter().enumerate() {
        ...
        assert!(
            completed.actions()[..=index]
                .iter()
                .all(|action| action.status() == satelle::host::SetupActionStatus::Completed)
        );
        assert!(
    ...
    let finished = service
        ...
        .expect("load persistent service ledger")
        .expect("persistent service ledger exists");
    assert_eq!(finished.status(), satelle::host::SetupRunStatus::Completed);
    assert!(
        finished
            .actions()
            .iter()
            ...
            .all(|(action, id)| {
                action.action_id() == id
                    && action.status() == satelle::host::SetupActionStatus::Completed
            })
    );
    ...
    assert!(
        failed.actions()[2..]
            .iter()
            .all(|action| action.status() == satelle::host::SetupActionStatus::Skipped)
    );
    ...
    assert_eq!(update.status(), satelle::host::SetupRunStatus::Completed);
    assert!(
        update
            .actions()
            .iter()
...
async fn persistent_host_lifecycle_maintenance_uses_exact_single_action_plans() {
    ...
    for (operation_id, expected_action, expected_operation_kind) in [
        ...
        assert_eq!(run.status(), satelle::host::SetupRunStatus::Completed);
