...
use clap::Args;
use satelle::core::{ErrorCode, SatelleError, SessionId};
use satelle::host::{DaemonLogEntry, LogCursor, LogPageQuery, LogSeverity, LogSource, LogSubject};
...
trait FollowConnection: Send {
    ...
    ) -> Result<satelle::core::session::PublicSession, SatelleError>;
    fn logs(&self, query: &LogPageQuery) -> Result<satelle::host::DaemonLogPage, SatelleError>;
}

type FollowConnectionFactory =
    Arc<dyn Fn() -> Result<Box<dyn FollowConnection>, SatelleError> + Send + Sync>;
type FollowReconnectResult =
    Result<(Box<dyn FollowConnection>, satelle::host::DaemonLogPage), SatelleError>;
...
fn visit_forward_snapshot_pages(
    ...
    mut fetch: impl FnMut(
        Option<LogCursor>,
        usize,
    ) -> Result<satelle::host::DaemonLogPage, SatelleError>,
...
mod tests {
    ...
    struct FakeFollowConnection {
        ...
        pages: Mutex<VecDeque<Result<satelle::host::DaemonLogPage, SatelleError>>>,
    ...
    struct SessionScopeMismatchFollowConnection {
        returned_session: satelle::core::session::PublicSession,
    }

    impl FakeFollowConnection {
        fn new(
            host_identity: &str,
            pages: impl IntoIterator<Item = Result<satelle::host::DaemonLogPage, SatelleError>>,
        ) -> Self {
            Self {
    ...
    fn forward_snapshot_visits_every_page_after_the_start_cursor() {
        ...
        let mut pages = VecDeque::from([
            serde_json::from_value::<satelle::host::DaemonLogPage>(serde_json::json!({
                "entries": [],
    ...
    fn public_session(session_id: &SessionId) -> satelle::core::session::PublicSession {
    ...
    fn page(cursor: u64) -> satelle::host::DaemonLogPage {
        serde_json::from_value(serde_json::json!({
            "entries": [{
    ...
    fn reconnect_rejects_a_changed_host_identity_before_resuming() {
        ...
        let factory = queued_connection_factory([
            ...
            Box::new(FakeFollowConnection::new(
                "host-replacement",
                std::iter::empty::<Result<satelle::host::DaemonLogPage, SatelleError>>(),
            )) as Box<dyn FollowConnection>,
        ]);
        let runtime = FakeFollowRuntime::new(None);
