...
use satelle::core::{ErrorCode, SatelleError, resolve_path_set};
use serde::Serialize;
...
mod tests {
    ...
    fn embedded_bytes_match_release_sources() {
        ...
        for skill in &BUNDLED_SKILLS {
            let source = root.join(skill.name).join("SKILL.md");
            assert_eq!(
