...
use crate::core::{ErrorCode, SessionId, TurnId};
...
fn due_retention_failure_blocks_status_and_logs_and_rolls_back() {
    ...
    let logs_error = runtime
        .log_page(&crate::host::LogPageQuery::default())
...
fn expired_session_fixture() -> (crate::host::TestStateDir, Storage, SessionId) {
    ...
    storage
        .commit_lifecycle(
            &session_id,
            &turn_id,
            crate::core::session::ExpectedRevisions::new(
