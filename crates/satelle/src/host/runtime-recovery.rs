...
pub(crate) fn verify_setup_postconditions(
    subject: &crate::host::storage::MaintenanceRecoverySubject,
    ...
    for action in subject.run().actions() {
        if action.status() != crate::host::storage::SetupActionStatus::OutcomeUnknown {
            continue;
        }
...
impl RuntimeEngine {
    ...
    pub(super) fn reconcile_maintenance(
        &self,
        observer: &mut dyn crate::host::SetupPostconditionObserver,
    ) -> Result<Option<crate::host::storage::SetupRunStatus>, SatelleError> {
    ...
    if let Some(freshness) = freshness {
        details.insert(
            ...
            serde_json::Value::String(
                match freshness {
                    crate::host::storage::LeaseFreshness::Fresh => "fresh",
                    crate::host::storage::LeaseFreshness::Stale => "stale",
                }
                .to_string(),
            ),
        );
