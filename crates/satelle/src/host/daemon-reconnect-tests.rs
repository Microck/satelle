...
use crate::host::{LiveEventReceiveError, LogEvent, LogPageQuery, LogSubject};
...
fn daemon_reconnect_restores_current_state_and_cursor_logs_without_event_replay() {
    ...
    assert_eq!(invalid_limit.code, crate::core::ErrorCode::InvalidUsage);
    ...
    let foreign_state = crate::host::TestStateDir::new().expect("second Host state directory");
    let foreign_host =
    ...
    assert_eq!(foreign_error.code, crate::core::ErrorCode::SessionNotFound);
    ...
    assert_eq!(
        ...
        crate::core::session::TurnState::Completed
    );
    assert_eq!(
        restarted
            .session_status(first.session_id())
