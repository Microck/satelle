...
use crate::host::storage::{LeaseOwner, PrivateUpstreamRef, ProbeRecoverySubject};
use crate::host::test_runtime::FakeComputerUseAdapter;
use crate::host::{
...
fn verified_png() -> Vec<crate::host::attachment::VerifiedImageAttachment> {
    ...
    crate::host::attachment::resolve_images(&accepted).expect("resolve the runtime image fixture")
}
...
fn host_config_with_provider_binding(secret_variable: &str) -> crate::core::HostConfig {
    ...
    desktop.provider_auth = std::collections::BTreeMap::from([(
        "host-auth".to_string(),
        crate::core::ProviderSecretSource::Environment {
...
fn host_retention_config_becomes_the_runtime_storage_policy() {
    ...
    config.session_metadata_retention =
        Some(crate::core::RetentionDuration::parse("30d").expect("parse session retention"));
    config.sqlite_log_retention =
        Some(crate::core::RetentionDuration::parse("45d").expect("parse Log retention"));
    config.operator_log_retained_files = Some(12);
    config.platform_log_sink = true;
    ...
    assert!(policy.platform_log_sink);
    assert_eq!(
...
struct ProviderSecretRecoveryFixture {
    operation_id: String,
    binding: crate::core::ResolvedProviderBinding,
...
fn provider_secret_recovery_fixture(
    ...
    let candidate_secret =
        crate::host::provider_auth::ResolvedProviderSecret::for_test("candidate");
    ...
    let candidate_digest = candidate_secret
        .expose_to_provider(|value| {
            crate::core::keyed_secret_comparison_digest(candidate_key.as_bytes(), value.as_bytes())
        })
    ...
    let candidate_hmac = candidate_digest
        ...
        .map(|byte| format!("{byte:02x}"))
        .collect::<String>();
    let prior_secret = crate::core::read_owner_only_secret_file(destination)
    ...
    let prior_digest = crate::core::keyed_owner_only_secret_file_comparison_digest(
        destination,
        prior_key.as_bytes(),
    )
    ...
    let binding = crate::core::ResolvedProviderBinding::from_authorization(
        crate::core::ProviderBindingAuthorization::new("review", "openai", "host-model", "openai")
    ...
    let paths = crate::host::storage::provider_secret_file_paths(destination, operation_id)
...
#[cfg(unix)]
fn recovery_test_runtime(state: &crate::host::TestStateDir) -> RuntimeHandle {
...
#[cfg(unix)]
fn recovery_secret_destination(
    state: &crate::host::TestStateDir,
    name: &str,
) -> std::path::PathBuf {
    let directory = state.path().join("provider-secrets");
    drop(
        crate::core::open_or_create_owner_only_directory(&directory)
...
fn assert_startup_recovery_rolls_back_publish_intent(
    ...
    crate::core::publish_owner_only_secret_file(
        &fixture.paths,
        true,
        true,
        fixture.candidate_key.as_bytes(),
...
fn startup_recovery_preserves_an_unowned_parked_rotation() {
    ...
    crate::core::persist_new_owner_only_secret_file(&destination, "prior")
    ...
    assert_eq!(
        crate::core::read_owner_only_secret_file(&destination)
...
fn runtime_mirrors_only_committed_normalized_log_entries() {
    ...
    let cursor = runtime
        .append_log_for_tests(
            time::OffsetDateTime::now_utc(),
            crate::host::LogSource::Storage,
            crate::host::LogSeverity::Info,
        )
    ...
    assert_eq!(
        runtime
            ...
            .operator_log_health(),
        crate::host::storage::OperatorLogSinkHealth::Healthy
    );
}
...
fn host_config_binding_precedes_colliding_persisted_user_config_without_secret_resolution() {
    ...
    let persisted_user_binding = crate::core::ResolvedProviderBinding::from_authorization(
    ...
    let crate::host::ProviderBindingResolution::Ready(resolved) = resolved else {
...
fn project_selection_requires_exact_host_owned_binding_consent() {
    ...
    assert_eq!(
        error.code,
        crate::core::ErrorCode::ProjectProviderSelectionNotAllowed
    );
    drop(denied_runtime);
...
fn project_selection_requires_exact_persisted_binding_consent() {
    ...
    let authorization = crate::core::ProviderBindingAuthorization::new(
        "review",
        "openai",
...
fn replacing_or_deleting_persisted_binding_prevents_restored_digest_cache_reuse() {
    ...
    let first_binding = crate::core::ResolvedProviderBinding::from_authorization(
        crate::core::ProviderBindingAuthorization::new(
            "review",
            "openai",
...
fn native_probe_ordinary_failure_preserves_only_possible_dispatch() {
    for (behavior, expected_status, expected_leases) in [
        ...
        let state =
            crate::host::TestStateDir::new().expect("temporary state directory should exist");
...
fn setup_provider_binding_failure_occurs_after_returned_native_evidence() {
    ...
    let cancellation = crate::host::AdmissionCancellation::new();
    let error = runtime
...
fn yolo_turn_commits_never_and_danger_full_access_before_adapter_execution() {
    ...
    let adapter = BoundaryInspectingAdapter {
        expected_mode: crate::core::session::TurnExecutionMode::Yolo,
    ...
    runtime
        .run(
            RunCommand::attached(LOCAL_DEMO_HOST, "PRIVATE_YOLO_POLICY_PROMPT")
                .with_execution_mode(crate::core::session::TurnExecutionMode::Yolo),
        )
...
struct BoundaryInspectingAdapter {
    execute_calls: Arc<AtomicUsize>,
    expected_policy: Arc<Mutex<Option<crate::core::session::ExecutionPolicy>>>,
    expected_mode: crate::core::session::TurnExecutionMode,
}
