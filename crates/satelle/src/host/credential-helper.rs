...
use crate::core::CredentialHelper;
use crate::host::codex_capabilities::terminate_group;
use crate::host::provider_auth::{ProviderAuthResolutionError, ProviderHostPlatform};
...
fn read_response(
    ...
    use ProviderAuthResolutionError::{HelperTimeout, Unresolved};
    #[cfg(unix)]
    crate::host::codex_capabilities::set_nonblocking(&stdout).map_err(|_| Unresolved)?;
...
pub(crate) mod tests {
    use super::*;
    use crate::core::{ExplicitDuration, ProviderSecretSource};
...
"#).unwrap();
    ...
    fn helper_uses_one_request_literal_arguments_and_a_private_environment() {
        for mode in ["success", "stderr", "descendant"] {
            ...
            let secret = crate::host::provider_auth::resolve_provider_secret(
