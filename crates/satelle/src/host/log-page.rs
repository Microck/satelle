...
use crate::core::session::{HostIdentityRef, SessionStateRevision, TurnStateRevision};
use crate::core::{SessionId, TurnId};
...
impl LogEvent {
    ...
    pub(crate) const fn matches_queue_status(
        self,
        status: crate::core::queue::QueueRequestStatus,
    ) -> bool {
        matches!(
            (self, status),
            (
                Self::TurnQueued,
                crate::core::queue::QueueRequestStatus::Queued
            ) | (
                Self::QueuePositionChanged,
                crate::core::queue::QueueRequestStatus::Queued
            ) | (
                Self::TurnDequeued,
                crate::core::queue::QueueRequestStatus::Admitted
            ) | (
                Self::TurnQueueCancelled,
                crate::core::queue::QueueRequestStatus::Cancelled
            ) | (
                Self::TurnQueueExpired,
                crate::core::queue::QueueRequestStatus::Expired
            ) | (
                Self::TurnQueueValidationFailed,
                crate::core::queue::QueueRequestStatus::ValidationFailed
            )
        )
    }
...
impl DaemonLogEntry {
    ...
    fn validate(&self) -> Result<(), &'static str> {
        ...
        if self
            .subject
            .desktop_binding()
            .is_some_and(|binding| crate::core::session::DesktopBindingRef::new(binding).is_err())
        {
            ...
            let subject = match event.subject_kind() {
                ...
                LogSubjectKind::Queue => {
                    let status = match event {
                        ...
                        LogEvent::TurnDequeued => crate::core::queue::QueueRequestStatus::Admitted,
                    ...
                    let admitted = status == crate::core::queue::QueueRequestStatus::Admitted;
                    LogSubject::Queue {
                        desktop_binding: "operator".to_string(),
                        queue_status: QueueStatus::new(
                            crate::core::QueueRequestId::parse(
                                "rq_01890a5d-ac96-7b7c-8f89-37c3d0a66e31",
                            )
                            .unwrap(),
                            status,
                            (status == crate::core::queue::QueueRequestStatus::Queued).then_some(1),
                            OffsetDateTime::UNIX_EPOCH,
                            ...
                            (status == crate::core::queue::QueueRequestStatus::ValidationFailed)
                                .then(|| crate::core::queue::QueueFailure {
