use crate::core::queue::{QueueCancelResult, QueueFailure, QueueRequestStatus, QueueStatus};
use crate::core::session::TurnExecutionMode;
use crate::core::{ErrorCode, QueueRequestId, SatelleError, SessionId};
use crate::host::attachment::AttachmentInput;
use crate::host::storage::TurnQueueOperation;
use crate::host::{ApiPrincipal, ApiScopes, HostService, MutationAuthority, TurnIntent};
    ...
    pub(crate) fn capture(
        ...
        Ok(Self {
            ...
            attachments: intent
                .attachments()
                .iter()
                .map(crate::host::attachment::AcceptedImageAttachment::durable_input)
                .collect(),
...
impl QueuePayloadStore {
    pub(crate) fn open(root: PathBuf) -> Result<Self, SatelleError> {
        crate::core::open_or_create_owner_only_directory(&root)
    ...
    pub(crate) fn write(
        ...
        let mut file = crate::core::open_new_owner_only_file(&path)
    ...
    pub(crate) fn read(
        ...
        let bytes = crate::core::read_bounded_regular_file_no_follow(
...
impl HostService {
    ...
    fn next_queued_turn(
        &self,
    ) -> Result<Option<crate::host::storage::StoredQueueRecord>, SatelleError> {
        let mut next = None;
        for binding in self.runtime.configured_desktop_bindings()? {
            ...
            if next
                .as_ref()
                .is_none_or(|current: &crate::host::storage::StoredQueueRecord| {
                    (
...
fn queue_record_matches_payload(
    record: &crate::host::storage::StoredQueueRecord,
