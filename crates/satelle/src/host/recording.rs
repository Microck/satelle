...
use crate::core::sensitive_diagnostics::{DiagnosticRedactor, MAX_RAW_PROTOCOL_BYTES};
use crate::core::{SatelleError, SatelleEvent, SessionId, TurnId};
...
impl RecordingCapture {
    pub(crate) fn begin(
        ...
        crate::core::open_or_create_owner_only_directory(&directory).map_err(|_| {
            recording_error(
    ...
    fn capture_screenshot(&self) {
        ...
        if let Ok(mut state) = self.inner.lock() {
            ...
            match crate::core::persist_new_owner_only_desktop_snapshot(&path, &staging, &png) {
    ...
    pub(crate) fn finish(
        ...
        crate::core::persist_new_owner_only_diagnostic_file(&manifest_path, &staging, &bytes)
            .map_err(|_| {
                recording_error(
...
fn persist_text_artifact(
    ...
    crate::core::persist_new_owner_only_diagnostic_file(&path, &staging, bytes).map_err(|_| {
        recording_error(
...
fn start_video_capture(directory: &Path) -> Result<VideoCapture, SatelleError> {
    ...
    crate::core::open_or_create_owner_only_directory(&staging_root).map_err(|_| {
        recording_error(
...
mod tests {
    ...
    fn event_recording_redacts_before_writing_manifested_artifact() {
        use crate::core::{EventSource, EventType, SatelleEventBody};
        ...
        let event = SatelleEventBody::new(
            ...
            OffsetDateTime::now_utc(),
            "remote-demo",
            Some(crate::core::EventSubject::TurnIdentity {
                session_id,
                turn_id,
            }),
            "progress",
