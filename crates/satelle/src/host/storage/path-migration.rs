...
use crate::core::{DaemonPathOverrides, ErrorCode, SatelleError};
use crate::core::{
...
pub(crate) fn migration_requires_rollback(
    ...
    if journal.operation_id != operation_id
        || journal.action_id != crate::host::STORAGE_MIGRATION_ACTION_ID
    {
...
impl super::Storage {
    ...
    pub(crate) fn verify_storage_migration(
        ...
        super::open::verify_integrity(&self.connection)
            .map_err(crate::host::runtime::storage_failure)?;
        ...
        ).map_err(|source| crate::host::runtime::storage_failure(super::open::sqlite_error(
        ...
        let deletion = (|| {
            ...
            match source.reset_metadata() {
                ...
                Err((names, error)) => {
                    ...
                    return Err(crate::host::runtime::storage_failure(error));
                }
            }
            Ok(())
        })();
...
pub(crate) fn stage(mut plan: StorageMigrationPlan) -> Result<StorageMigrationStage, SatelleError> {
    ...
    let (journal, encoded) =
        super::read_offline_storage_maintenance_journal(&source._state_directory)
            .map_err(crate::host::runtime::storage_failure)?
            .ok_or_else(SatelleError::state_conflict)?;
    if journal.action_id != crate::host::STORAGE_MIGRATION_ACTION_ID {
        return Err(SatelleError::state_conflict());
    }
    plan.items
...
fn prepare_migration_directory(
    path: &Path,
) -> Result<crate::core::OwnerOnlyDirectory, SatelleError> {
    let invalid = || {
        migration_error(
            ErrorCode::StorageMigrationDestinationInvalid,
...
mod tests {
    use super::*;
    use crate::core::open_new_owner_only_file;
    #[cfg(unix)]
    use crate::core::open_or_create_owner_only_directory;
    use crate::test_contract::assert_directory_tree_unchanged;
    ...
    fn source_paths(root: &Path) -> DaemonResolvedPathSet {
        ...
        let mut config = crate::core::SatelleConfig::defaults()
            .hosts
            .remove(crate::core::LOCAL_DEMO_HOST)
            .unwrap();
        ...
        crate::host::HostService::resolved_daemon_paths_for_host(&config).unwrap()
    }
    ...
    fn preview_lists_owned_files_and_preserves_the_entire_directory_tree() {
        let root = crate::host::TestStateDir::new().unwrap();
    ...
    fn staging_preserves_source_identity_and_files_and_invalidates_only_destination_readiness() {
        ...
        let staged =
            crate::host::HostService::stage_storage_migration(&source, &destination, operation_id)
                .unwrap();
        ...
        for (state_root, readiness_count) in [(source_root, 1_i64), (destination_root, 0)] {
            ...
            assert_eq!(journal.action_id, crate::host::STORAGE_MIGRATION_ACTION_ID);
    ...
    fn rollback_recovers_a_committed_lease_when_the_external_journal_was_not_written() {
        ...
        let source = source_paths(root.path());
        let source_root = Path::new(&source.state_root);
        let service = crate::host::HostService::production_for_offline_storage(source_root);
        let operation_id = "migration-journal-write-failed";
        let now = time::OffsetDateTime::now_utc();
        let plan = crate::host::SetupRunPlan::new(
            operation_id,
            crate::host::SetupOperationKind::StorageMigration,
            None,
            now,
            vec![
                crate::host::SetupActionPlan::new(
                    crate::host::STORAGE_MIGRATION_ACTION_ID,
                    crate::host::STORAGE_MIGRATION_ACTION_LABEL,
                    true,
                )
                .unwrap(),
            ],
        ...
        service
            .start_setup_action(&operation, crate::host::STORAGE_MIGRATION_ACTION_ID, now)
            .unwrap();
        drop(operation);
        drop(service);
        assert!(
        ...
        crate::host::HostService::rollback_storage_migration(source_root, operation_id).unwrap();
        crate::host::HostService::rollback_storage_migration(source_root, operation_id).unwrap();
        let store = super::super::Storage::open_without_restart_recovery(source_root).unwrap();
    ...
    fn storage_migration_cleanup_resumes_after_deletion_without_losing_its_original_report() {
        ...
        let staged =
            crate::host::HostService::stage_storage_migration(&source, &destination, operation)
                .unwrap();
        let mut config = crate::core::SatelleConfig::defaults()
            .hosts
            .remove(crate::core::LOCAL_DEMO_HOST)
        ...
        let service = crate::host::HostService::production_for_host(&config);
        ...
        service
            .register_api_token(&token, "cleanup-admin", crate::host::ApiScopes::ADMIN, None)
            .unwrap();
        ...
        .unwrap();
        let canonical = serde_json::to_vec(&crate::host::StorageMigrationRequest::SourceCleanup {
        ...
        let restarted = crate::host::HostService::production_for_host(&config);
    ...
    fn staging_never_overwrites_a_destination_created_after_preview() {
        ...
        crate::host::HostService::start_offline_storage_maintenance(
            Path::new(&source.state_root),
