...
pub(super) fn load_log_page_records(
    ...
    let forward = i64::from(query.mode() == crate::host::LogPageMode::Forward);
    let cursor = i64::try_from(query.cursor().map_or(0, crate::host::LogCursor::position))
    ...
    let all_sources = i64::from(
        query.includes_source(crate::host::LogSource::HostDaemon)
            && query.includes_source(crate::host::LogSource::Storage)
            && query.includes_source(crate::host::LogSource::CodexAdapter),
    );
    let host_daemon = i64::from(query.includes_source(crate::host::LogSource::HostDaemon));
    let storage = i64::from(query.includes_source(crate::host::LogSource::Storage));
    let codex_adapter = i64::from(query.includes_source(crate::host::LogSource::CodexAdapter));
...
pub(super) fn idempotent_operation_token(operation: IdempotentOperation) -> &'static str {
    match operation {
        ...
        IdempotentOperation::ProviderSecretProvisioning => "provider_secret_provisioning",
...
fn parse_log_row(row: RawLogRow) -> Result<StoredLogRecord, StorageError> {
    let subject = match row.queue_status_json {
        Some(queue_status_json) => {
            ...
            let queue_status =
                serde_json::from_str::<crate::core::queue::QueueStatus>(&queue_status_json)
                    .map_err(|_| StorageError::new(StorageErrorKind::InvalidStoredState))?;
