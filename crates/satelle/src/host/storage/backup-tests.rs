...
struct BackupFixture {
    _test_state: crate::host::TestStateDir,
    state_root: PathBuf,
...
impl BackupFixture {
    fn new(backup_count: usize) -> Self {
        let test_state = crate::host::TestStateDir::new().expect("create test state directory");
...
fn migration_backup_restore_preserves_active_and_retired_idempotency_hmac_keys() {
    ...
    let (mut storage, recovery) =
        crate::host::storage::Storage::open(&state_root).expect("open source store");
    ...
    let (restored, recovery) =
        crate::host::storage::Storage::open(&state_root).expect("reopen restored store");
    assert!(recovery.is_empty());
    assert_eq!(
        active_digest,
        restored
...
fn manifest_filename_digest_schema_and_compatibility_are_validated() {
    ...
    wrong_filename.backup_file = "satelle.sqlite3.migration-v10-wrong.backup".to_owned();
...
fn cleanup_ignores_malformed_lookalikes_without_deleting_them() {
    let fixture = BackupFixture::new(3);
    let malformed_name = "satelle.sqlite3.migration-v10-not-a-uuid.backup";
    let invalid_name = "satelle.sqlite3.migration-v10-ffffffff-ffff-7fff-bfff-ffffffffffff.backup";
...
fn offline_store_reset_returns_files_removed_before_a_later_delete_failure() {
    let state = crate::host::TestStateDir::new().expect("create state directory");
