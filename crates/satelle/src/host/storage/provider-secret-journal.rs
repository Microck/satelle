...
use crate::core::session::{DesktopBindingRef, HostIdentityRef};
use crate::core::{
...
use crate::host::{ProviderSmokeEvidence, ReadinessCacheKey, ReadinessEvidence};
...
pub(crate) fn provider_secret_file_paths(
    destination: &Path,
    operation_id: &str,
) -> Result<crate::core::OwnerOnlySecretFilePaths, StorageError> {
    ...
    crate::core::OwnerOnlySecretFilePaths::new(destination, &file_component)
...
fn validate_planned_paths(destination: &Path, staged: &Path) -> Result<(), StorageError> {
    ...
    crate::core::OwnerOnlySecretFilePaths::from_staged_path(destination, staged)
        .map_err(|_| StorageError::new(StorageErrorKind::InvalidInput))?;
...
fn validate_staged_paths(
    ...
    let paths = crate::core::OwnerOnlySecretFilePaths::from_staged_path(destination, staged)
        .map_err(|_| StorageError::new(StorageErrorKind::InvalidInput))?;
