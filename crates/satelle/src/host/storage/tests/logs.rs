use super::*;
use crate::host::DaemonLogEntry;
use crate::host::storage::operator_log::{OperatorLogFailureKind, OperatorLogSinkHealth};
use crate::host::{LogCursor, LogEvent, LogPageQuery, LogSeverity, LogSource};
use std::fs;
use std::path::Path;
...
fn turn_log(at: OffsetDateTime, desktop_binding: &str, suffix: u8) -> SafeLogRecord {
    SafeLogRecord::new(
        ...
        crate::host::LogSubject::Turn {
            desktop_binding: desktop_binding.to_string(),
            session_id: crate::core::SessionId::parse(&format!(
                "rs_01890a5d-ac96-7b7c-8f89-37c3d0a66e{suffix:02x}"
            ))
            .expect("valid Session identifier"),
            turn_id: crate::core::TurnId::parse(&format!(
                "rt_01890a5d-ac96-7b7c-8f89-37c3d0a66e{suffix:02x}"
            ))
            .expect("valid Turn identifier"),
            session_state_revision: crate::core::session::SessionStateRevision::initial(),
            turn_state_revision: crate::core::session::TurnStateRevision::initial(),
        },
    )
...
fn operator_log_mirror_applies_reduced_retention_without_a_new_record() {
    ...
    let log_root = state.path().join("operator-logs");
    drop(
        crate::core::open_or_create_owner_only_directory(&log_root)
