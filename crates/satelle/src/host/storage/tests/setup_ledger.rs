use super::*;
use crate::core::host_update::HostUpdateRecoveryIdentity;
...
fn setup_history_projects_action_states_without_private_ledger_fields() {
    ...
    crate::test_contract::assert_privacy_canaries_absent("setup history", &encoded, &[PRIVATE]);
}
...
fn every_exact_owner_field_and_state_guards_action_normal_and_recovery_release() {
    for (column, wrong_value) in [
        ...
        let subject = match recovery_storage.maintenance_lease_state().unwrap().unwrap() {
            crate::host::storage::MaintenanceLeaseState::RecoveryPending(subject) => subject,
            crate::host::storage::MaintenanceLeaseState::Active { .. } => unreachable!(),
        };
        let verified = crate::host::runtime::verify_setup_postconditions(
            &subject,
...
fn setup_run_plans_require_a_nonempty_unique_action_sequence() {
    ...
    assert_eq!(crate::core::ErrorCode::InvalidUsage, empty.code);
    ...
    assert_eq!(crate::core::ErrorCode::InvalidUsage, duplicate_plan.code);
}
...
fn maintenance_delete_failure_rolls_back_normal_and_recovery_terminal_ledgers() {
    ...
    assert!(matches!(
        ...
        Some(crate::host::storage::MaintenanceLeaseState::Active { operation_id, .. })
    ...
    let verified =
        crate::host::runtime::verify_setup_postconditions(&subject, &mut FixedSetupObserver(true))
            .unwrap();
    recovery
        .connection_for_test()
        .execute_batch(
    ...
    assert!(matches!(
        ...
        Some(crate::host::storage::MaintenanceLeaseState::RecoveryPending(subject))
...
fn startup_preserves_orphaned_maintenance_as_recovery_pending_outcome_unknown() {
    ...
    let unknown_subject = match recovered.maintenance_lease_state().unwrap().unwrap() {
        crate::host::storage::MaintenanceLeaseState::RecoveryPending(subject) => subject,
        crate::host::storage::MaintenanceLeaseState::Active { .. } => {
    ...
    crate::host::runtime::verify_setup_postconditions(&unknown_subject, &mut UnknownSetupObserver)
...
fn restart_reconciliation_commits_verified_unsatisfied_as_failed_before_release() {
    ...
    let verified =
        crate::host::runtime::verify_setup_postconditions(&subject, &mut FixedSetupObserver(false))
            .unwrap();
    assert_eq!(
...
impl crate::host::SetupPostconditionObserver for FixedSetupObserver {
    fn observe(&mut self, _action: &SetupActionRecord) -> Result<bool, crate::core::SatelleError> {
        Ok(self.0)
    }
}
...
impl crate::host::SetupPostconditionObserver for UnknownSetupObserver {
    fn observe(&mut self, _action: &SetupActionRecord) -> Result<bool, crate::core::SatelleError> {
        Err(crate::core::SatelleError::computer_use_not_ready())
    }
}
