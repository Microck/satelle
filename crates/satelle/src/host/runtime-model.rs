...
use crate::core::session::{ExecutionPolicy, ExpectedRevisions, RetainedOwnership, Session};
use crate::core::{
    ErrorCode, LOCAL_DEMO_HOST, SatelleError, SatelleEvent, SessionId, StopResult, TurnId,
};
use crate::host::process_identity::{ProcessIdentity, ProcessIdentityError};
...
pub(super) fn stop_result(
    commit: &crate::host::storage::StopCommit,
    session_id: &SessionId,
) -> Result<StopResult, SatelleError> {
    let turn_id = commit.turn_id().clone();
    match commit.outcome() {
        crate::host::storage::StopCommitOutcome::Stopped(previous_state) => {
            let turn = commit
                .session()
                .turn(&turn_id)
        ...
        crate::host::storage::StopCommitOutcome::AlreadyTerminal(state) => {
            StopResult::already_terminal(session_id.clone(), turn_id, (*state).into())
                .map_err(|_| integrity_failure("invalid terminal stop result"))
        }
        crate::host::storage::StopCommitOutcome::NotConfirmed { ownership, changed } => {
...
fn stop_not_confirmed(
    ...
    session_id: &SessionId,
    turn_id: &TurnId,
    session_state_revision: crate::core::session::SessionStateRevision,
    turn_state_revision: crate::core::session::TurnStateRevision,
) -> SatelleError {
