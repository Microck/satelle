use crate::core::session::{PublicSession, StopObservation, TurnExecutionMode};
use crate::core::{
    ControlPlaneOperation, ErrorCode, LOCAL_DEMO_HOST, SatelleError, SessionId, TurnId,
};
use crate::host::operation_capacity::{OperationCapacity, OperationOutcome, OperationRequest};
...
use crate::host::storage::IdempotentOperation;
...
fn cancellation_registered_before_run_uses_the_same_gate_and_prevents_admission() {
    ...
    assert!(matches!(
        ...
        crate::host::AdmissionCancellationResult::Cancelled
    ));
    assert!(
        service
            .operation_capacity
...
fn cancellation_registered_during_preflight_prevents_durable_turn_admission() {
    ...
    {
        crate::host::AdmissionCancellationResult::Cancelled => {}
        crate::host::AdmissionCancellationResult::RecoveryPending => {
            panic!("pre-dispatch cancellation must not retain recovery ownership")
        }
        crate::host::AdmissionCancellationResult::Admitted { .. } => {
...
fn active_cancellation_requests_before_slow_tombstone_persistence() {
    ...
    let operation = std::thread::spawn(move || {
        operation_capacity.execute_interruptible_durable(
            ...
            || Ok(crate::host::operation_capacity::DurableAdmissionOutcome::Missing),
            |outcome| match outcome {
                crate::host::operation_capacity::AdmissionCancellationOutcome::ReconciledCancelled => {
                    Ok(crate::host::operation_capacity::DurableAdmissionOutcome::Cancelled)
                }
                crate::host::operation_capacity::AdmissionCancellationOutcome::RecoveryPending => {
                    Ok(crate::host::operation_capacity::DurableAdmissionOutcome::RecoveryPending)
                }
                _ => Ok(crate::host::operation_capacity::DurableAdmissionOutcome::Missing),
            },
            |cancellation| {
        ...
        cancellation_capacity.cancel_durable(
            ...
            |outcome| {
                ...
                Ok(match outcome {
                    ...
                    crate::host::operation_capacity::AdmissionCancellationOutcome::Cancelled
                    | crate::host::operation_capacity::AdmissionCancellationOutcome::ReconciledCancelled => {
                        crate::host::operation_capacity::DurableAdmissionOutcome::Cancelled
                    }
                    crate::host::operation_capacity::AdmissionCancellationOutcome::Admitted { .. } => {
        ...
        operation_capacity.execute_interruptible_durable(
            ...
            |outcome| {
                Ok(match outcome {
                    ...
                    _ => crate::host::operation_capacity::DurableAdmissionOutcome::Missing,
                })
            },
            |cancellation| {
        ...
        let operation = std::thread::spawn(move || {
            operation_capacity.execute_interruptible_durable(
                ...
                || match operation_durable_state.load(Ordering::SeqCst) {
                    0 => Ok(crate::host::operation_capacity::DurableAdmissionOutcome::Missing),
                    1 => Ok(crate::host::operation_capacity::DurableAdmissionOutcome::Cancelled),
                    2 => Ok(crate::host::operation_capacity::DurableAdmissionOutcome::RecoveryPending),
                    _ => unreachable!("the synthetic durable state is closed"),
                },
                |outcome| {
                    let state = match outcome {
                        crate::host::operation_capacity::AdmissionCancellationOutcome::Cancelled
                        | crate::host::operation_capacity::AdmissionCancellationOutcome::ReconciledCancelled => 1,
                        crate::host::operation_capacity::AdmissionCancellationOutcome::RecoveryPending => 2,
                        crate::host::operation_capacity::AdmissionCancellationOutcome::Admitted { .. } => {
                            unreachable!("the persistence seam receives only cancellation outcomes")
                        }
                    };
                    ...
                    Ok(match state {
                        1 => crate::host::operation_capacity::DurableAdmissionOutcome::Cancelled,
                        2 => crate::host::operation_capacity::DurableAdmissionOutcome::RecoveryPending,
                        _ => unreachable!("the synthetic durable state is closed"),
                    })
                },
                |cancellation| {
                    ...
                    operation_release_wait.wait();
                    match completion {
                        Completion::KnownPrecommitFailure => Err(crate::host::runtime::integrity_error(
                            "synthetic known precommit failure",
                        )),
                        Completion::PossibleDispatch => {
                            cancellation.finish(
                                crate::host::runtime::AdmissionCancellationState::RecoveryPending,
                            );
                            Err(crate::host::runtime::integrity_error(
    ...
    let operation = std::thread::spawn(move || {
        operation_capacity.execute_interruptible(
            ...
            |cancellation| {
                ...
                cancellation
                    .finish(crate::host::runtime::AdmissionCancellationState::RecoveryPending);
                Err(crate::host::runtime::integrity_error(
...
fn cancellation_replay_returns_the_exact_original_turn_handle() {
    ...
    match cancellation {
        crate::host::AdmissionCancellationResult::Admitted { session, turn_id } => {
...
fn daemon_activity_snapshot_tracks_active_maintenance_only() {
    ...
    let plan = crate::host::storage::SetupRunPlan::new(
        ...
        vec![
            crate::host::storage::SetupActionPlan::new("repair-runtime", "Repair runtime", false)
                ...
                || {
                    ...
                    match expectation {
                        ...
                        StaleProbeExpectation::Conflict => {
                            Err(crate::host::runtime::idempotency_conflict())
                        }
                    }
                },
...
fn coordinator_result_session() -> PublicSession {
    let state = crate::host::TestStateDir::new().expect("temporary coordinator state directory");
    let service = service(state.path(), ControlledAdapter::default());
...
fn assert_same_session_and_turn_handles(
    left: &crate::core::session::PublicSession,
    right: &crate::core::session::PublicSession,
) {
