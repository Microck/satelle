...
pub(crate) struct ProviderProbeSurface {
    page_url: String,
    #[cfg(windows)]
    _native_window: Option<crate::host::windows_native_probe::WindowsNativeProbeWindow>,
    deadline: Instant,
    shutdown: Arc<AtomicBool>,
    observed_actions: Arc<AtomicU8>,
    requirements: ProbeRequirements,
    cancellation: Option<crate::host::runtime::AdmissionCancellation>,
...
struct ProbeServerControl<'a> {
    ...
    cancellation: Option<&'a crate::host::runtime::AdmissionCancellation>,
    requirements: ProbeRequirements,
...
impl ProviderProbeSurface {
    ...
    fn start_with_requirements(
        ...
        let native_window = native_action_evidence
            .as_ref()
            .map(|_| {
                crate::host::windows_native_probe::WindowsNativeProbeWindow::spawn(
                    address,
                    &capability,
                    &nonce,
    ...
    pub(crate) fn wait_for_completion(mut self) -> Result<(), ProviderProbeError> {
        let outcome =
            loop {
                ...
                if self.cancellation.as_ref().is_some_and(
                    crate::host::runtime::AdmissionCancellation::is_requested_or_expired,
                ) {
...
fn probe_stopped(
    ...
    shutdown.load(Ordering::Acquire)
        || cancellation
            .is_some_and(crate::host::runtime::AdmissionCancellation::is_requested_or_expired)
}
...
mod tests {
    ...
    fn native_cancellation_reports_cancellation() {
        let deadline = Instant::now() + Duration::from_secs(2);
        let cancellation = crate::host::runtime::AdmissionCancellation::with_deadline(deadline);
        let probe =
    ...
    fn timeout_cancellation_and_caller_completion_never_leave_a_listener() {
        ...
        let external_cancellation =
            crate::host::runtime::AdmissionCancellation::with_deadline(deadline);
