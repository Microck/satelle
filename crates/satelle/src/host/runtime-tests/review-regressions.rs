...
fn attached_runtime_outcome_includes_revision_bound_live_adapter_events() {
    ...
    let crate::core::EventStateSubject::Turn {
...
fn attached_adapter_failure_retains_already_published_live_events() {
    ...
    assert_eq!(
        failure.phase(),
        crate::core::session::TurnAdmissionPhase::Admitted
    );
    let event = failure
        .events()
        .iter()
...
fn admission_replay_failure_retains_already_published_live_events() {
    ...
    assert_eq!(
        failure.phase(),
        crate::core::session::TurnAdmissionPhase::AdmissionUnknown
    );
    let event = failure
        .events()
        .iter()
...
fn stop_winning_before_running_skips_adapter_execution_and_returns_stopped() {
    ...
    let context = super::super::model::admission(
        crate::host::storage::IdempotentOperation::Run,
        started_at,
        &identity,
        &engine.process_identity,
    )
    .expect("build admission context");
    let crate::host::storage::AdmissionOutcome::Execute {
        session,
        recovery_subject,
    } = engine
        .lock_storage()
    ...
    let plan = super::super::worker::ExecutionPlan {
        ...
        execution_mode: crate::core::session::TurnExecutionMode::Standard,
...
struct SubjectBlockingAdapter {
    session: Arc<Mutex<Option<crate::core::SessionId>>>,
    session_ready: Latch,
...
impl SubjectBlockingAdapter {
    ...
    fn take_session(&self) -> crate::core::SessionId {
