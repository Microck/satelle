...
) -> Result<Vec<VerifiedImageAttachment>, crate::core::SatelleError> {
...
impl AttachmentStore {
    pub(crate) fn open(root: PathBuf) -> Result<Self, crate::core::SatelleError> {
    ...
    pub(crate) fn stage(
    ...
    ) -> Result<StagedAttachments, crate::core::SatelleError> {
        ...
        for (index, attachment) in attachments.into_iter().enumerate() {
            ...
            let mut file = match crate::core::open_new_owner_only_file(&path) {
                Ok(file) => file,
                Err(_) => {
...
fn cleanup_stale_files(root: &Path) -> Result<(), crate::core::SatelleError> {
...
fn attachment_failure(message: &'static str) -> crate::core::SatelleError {
    crate::core::SatelleError::invalid_usage(message)
}
...
mod tests {
    ...
    fn remote_images_share_upload_staging_and_preserve_the_source() {
        let directory =
            crate::host::test_support::TestStateDir::new().expect("private image directory");
    ...
    fn startup_cleanup_removes_only_generated_staging_files() {
        ...
        .unwrap();
        let root = state.path().join("attachments");
        crate::core::open_or_create_owner_only_directory(&root).unwrap();
        let stale = root.join(format!("{FILE_PREFIX}stale.png"));
        crate::core::open_new_owner_only_file(&stale).unwrap();
    ...
    fn populate_stale_files(root: &Path, count: usize) {
        for index in 0..count {
            ...
            drop(crate::core::open_new_owner_only_file(&path).unwrap());
        }
    }
