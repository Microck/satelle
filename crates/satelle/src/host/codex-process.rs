...
use crate::core::sensitive_diagnostics::ProtocolDirection;
...
pub(crate) struct CodexCommand {
    ...
    pub(crate) native_resources: Option<crate::host::codex_capabilities::NativeSessionResources>,
}
...
struct ProcessOwner {
    ...
    _native_resources: Option<crate::host::codex_capabilities::NativeSessionResources>,
    group_stopped: bool,
}
...
impl Drop for ProcessOwner {
    fn drop(&mut self) {
        ...
        if !self.group_stopped
            ...
            && !crate::host::codex_capabilities::terminate_group(child)
        {
            PENDING_CLEANUP
                .lock()
...
pub(super) trait CodexExchange {
    ...
    fn raw_protocol_capture(&self) -> Option<crate::host::raw_diagnostics::RawProtocolCapture> {
        None
    }
}
...
pub(super) fn run_exchange<E: CodexExchange>(
    ...
    let (Some(stdin), Some(stdout)) = (stdin, stdout) else {
        process.group_stopped = crate::host::codex_capabilities::terminate_group(child);
        return Err(CodexSessionFailure::before_turn_dispatch(
    ...
    #[cfg(unix)]
    if crate::host::codex_capabilities::set_nonblocking(&stdout).is_err() {
        process.group_stopped = crate::host::codex_capabilities::terminate_group(child);
        return Err(CodexSessionFailure::before_turn_dispatch(
            CodexSessionError::Spawn,
        ));
    ...
    let writer_stopped = if successful_exchange {
        ...
        if writer_stopped {
            let _ = crate::host::codex_capabilities::wait_for_group_shutdown(
                child,
    ...
    let group_stopped = crate::host::codex_capabilities::terminate_group(child);
...
mod windows_tests {
    ...
    fn windows_unconfirmed_group_retains_staging_and_blocks_admission_until_exit() {
        let (resources, path) =
            crate::host::codex_capabilities::NativeSessionResources::windows_for_test();
        ...
        assert!(crate::host::codex_capabilities::terminate_group(
