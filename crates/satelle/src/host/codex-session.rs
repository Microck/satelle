use crate::core::session::StopObservation;
use crate::core::session::TurnExecutionMode;
use crate::host::provider_auth::ResolvedProviderSecret;
use crate::host::provider_probe::NativeActionEvidence;
...
pub(crate) struct CodexSessionRequest<'a> {
    ...
    pub(crate) image_input_mode: crate::host::codex_capabilities::CodexImageInputMode,
    pub(crate) attachments: &'a [crate::host::attachment::StagedImage],
    pub(crate) raw_protocol_capture: Option<crate::host::raw_diagnostics::RawProtocolCapture>,
    pub(crate) recording_capture: Option<crate::host::recording::RecordingCapture>,
}
...
pub(crate) fn run_codex_session_with_timeout_cancellation(
    ...
    admission_cancellation: Option<crate::host::AdmissionCancellation>,
                ...
                let message: Value = match serde_json::from_slice(&line) {
                    ...
                    Err(_) => {
                        if let Some(capture) = &self.request.raw_protocol_capture {
                            capture.record(
                                crate::core::sensitive_diagnostics::ProtocolDirection::FromCodex,
                                &line,
                            );
                        }
    ...
    fn write_turn_request(&mut self, writer: &ProtocolWriter) -> Result<(), CodexSessionError> {
        ...
        for attachment in self.request.attachments {
            let image = match self.request.image_input_mode {
                ...
                crate::host::codex_capabilities::CodexImageInputMode::Inline => {
                    let encoded =
