use crate::core::session::PublicSession;
use crate::core::{
    ProviderSecretProvisioningResult, PublicResolvedProviderBinding, SatelleError, TurnId,
};
use crate::host::runtime::{AdmissionCancellation, AdmissionCancellationState};
use crate::host::runtime::{RequestIdentity, RuntimeStopOutcome};
...
pub(crate) enum OperationOutcome {
    ...
    ApiToken(crate::host::ApiTokenMutationOutcome),
    ProviderBindingAuthorization(PublicResolvedProviderBinding),
    ProviderSecretProvisioning(ProviderSecretProvisioningResult),
    StorageMigration(crate::host::storage::StorageMigrationReply),
}

impl OperationOutcome {
    pub(crate) fn into_storage_migration(
        self,
    ) -> Result<crate::host::storage::StorageMigrationReply, SatelleError> {
        match self {
            Self::StorageMigration(reply) => Ok(reply),
            _ => Err(crate::host::runtime::integrity_error(
                "the operation-capacity result had the wrong response type",
            )),
        }
    }

    pub(crate) fn into_api_token(
        self,
    ) -> Result<crate::host::ApiTokenMutationOutcome, SatelleError> {
        match self {
    ...
    pub(crate) fn provider_binding_authorization(binding: PublicResolvedProviderBinding) -> Self {
    ...
    pub(crate) fn into_session(self) -> Result<PublicSession, SatelleError> {
        match self {
            ...
            | Self::ProviderSecretProvisioning(_) => Err(crate::host::runtime::integrity_error(
                "the operation-capacity result had the wrong response type",
            )),
        }
    }
    ...
    pub(crate) fn into_provider_binding_authorization(
        self,
    ...
    fn into_admission_cancellation(self) -> Result<AdmissionCancellationOutcome, SatelleError> {
        match self {
            ...
            Self::Stop(_) => Err(crate::host::runtime::integrity_error(
                "an admission cancellation received a stop result",
            )),
            Self::ProviderBindingAuthorization(_) => Err(crate::host::runtime::integrity_error(
            ...
            Self::ApiToken(_) => Err(crate::host::runtime::integrity_error(
                "an admission cancellation received an API token result",
            )),
            Self::StorageMigration(_) => Err(crate::host::runtime::integrity_error(
...
impl OperationCapacity {
    pub(crate) fn lock_identity_read(&self) -> Result<RwLockReadGuard<'_, ()>, SatelleError> {
        self.identity_gate.read().map_err(|_| {
            crate::host::runtime::integrity_error("the request-identity gate was poisoned")
        })
    }

    pub(crate) fn lock_identity_write(&self) -> Result<RwLockWriteGuard<'_, ()>, SatelleError> {
        self.identity_gate.write().map_err(|_| {
            crate::host::runtime::integrity_error("the request-identity gate was poisoned")
        })
    }
    ...
    fn test_cancellation(
        ...
        let cancellations = self.test_cancellations.lock().map_err(|_| {
            crate::host::runtime::integrity_error("the test cancellation lock was poisoned")
        })?;
        ...
        if stored != request {
            return Err(crate::host::runtime::idempotency_conflict());
        }
        Ok(match outcome {
    ...
    fn lock(&self) -> Result<MutexGuard<'_, CapacityState>, SatelleError> {
        self.state.lock().map_err(|_| {
            crate::host::runtime::integrity_error("the operation-capacity lock was poisoned")
        })
    }
    ...
    fn wait(&self) -> SharedResult {
        let mut stored = self.result.lock().map_err(|_| {
            crate::host::runtime::integrity_error(
                "the in-flight operation result lock was poisoned",
            )
        })?;
        loop {
            if let Some(result) = stored.as_ref() {
            ...
            stored = self.completed.wait(stored).map_err(|_| {
                crate::host::runtime::integrity_error(
                    "the in-flight operation wait lock was poisoned",
                )
            })?;
        }
    }
