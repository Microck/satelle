...
use crate::host::{ApiBearerToken, ApiPrincipal};
pub(crate) use crate::host::{LogEvent, LogSeverity, LogSource};
...
mod ssh_identity_commit_tests {
    ...
    fn prepared_state_root(state: &crate::host::TestStateDir) -> PathBuf {
    ...
    fn persist_record(state_root: &Path, record: &SshIdentityCommitRecord) {
        crate::core::persist_new_owner_only_secret_file(
...
pub(crate) enum NativeReadinessInvalidationTarget<'a> {
    Intent(Option<&'a crate::host::ReadinessCacheKey>),
    Host,
}
...
impl Storage {
    ...
    fn mark_interrupted_provider_descriptor_validations_failed(
        ...
        let replay = serde_json::to_string(&serde_json::json!({
            "status": "failed",
            "result": crate::core::SatelleError::state_conflict(),
        }))
    ...
    pub(crate) fn provider_smoke_hmac_key(
        &self,
    ) -> Result<crate::host::provider_auth::ProviderSmokeHmacKey, StorageError> {
