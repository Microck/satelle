...
use satelle::core::daemon_service::WindowsServiceConfigV8;
...
use satelle::core::doctor::{DoctorScope, DoctorScopeSelection, DoctorScopeSelectionError};
...
struct ConfigContext<'a> {
    ...
    select_profile: bool,
    profile_source: Option<satelle::core::ProfileSelectionSource>,
...
impl<'a> ConfigContext<'a> {
    ...
    fn for_profile(profile: &'a str, source: satelle::core::ProfileSelectionSource) -> Self {
        Self {
    ...
    fn mcp_install_profile(&self) -> Result<Option<satelle::core::SelectedProfile>, CliFailure> {
        let cwd = std::env::current_dir().unwrap_or_else(|_| PathBuf::from("."));
        satelle::core::resolve_invocation_profile(&cwd, self.flag_profile).map_err(failure)
    }
...
impl From<RecordingModeArg> for satelle::core::recording::RecordingMode {
...
fn preflight_setup_before_history(
    ...
    if command.expected_host_id.is_some()
        && host.config.transport != satelle::core::TransportKind::Ssh
    {
        return Err(failure(SatelleError::invalid_usage(
    ...
    let ssh_transport = host.config.transport == satelle::core::TransportKind::Ssh;
    ...
    let local_service_decision = (host.config.transport == satelle::core::TransportKind::Local)
        .then(|| {
    ...
    if host.config.transport != satelle::core::TransportKind::Local {
        return Ok(exact_state_requires_identity_discovery);
    }
    ...
    if local_setup_verification_only(
        command,
        host.config.transport == satelle::core::TransportKind::Local,
    ) {
        return Ok(exact_state_requires_identity_discovery);
    }
...
fn telemetry_queue_failure(error: satelle::core::telemetry::TelemetryQueueError) -> CliFailure {
    failure(SatelleError::config_error(
        "the private Controller telemetry queue is unavailable",
        Some(error.to_string()),
    ))
...
fn run_desktop_snapshot(
    ...
    let desktop = satelle::core::resolve_desktop_session(&sessions, &policy).map_err(|error| {
    ...
    eprintln!(
        ...
        satelle::core::sensitive_diagnostics::DESKTOP_SNAPSHOT_REDACTION_POLICY_VERSION
    );
    eprintln!(
        "Unredacted risks: {}",
        satelle::core::sensitive_diagnostics::DESKTOP_SNAPSHOT_RISKS.join(", ")
    );
    ...
    let request = satelle::transport::DesktopSnapshotCaptureRequest::new(
        &selected.alias,
    ...
    if satelle::core::persist_new_owner_only_desktop_snapshot(
        &output_path,
        &staging_path,
        &artifact.png,
    )
    .is_err()
    {
        let _ = transport.acknowledge_desktop_snapshot(
            &snapshot_id,
            satelle::core::sensitive_diagnostics::DesktopSnapshotExportOutcome::Failed,
        );
    ...
    transport
        .acknowledge_desktop_snapshot(
            &snapshot_id,
            satelle::core::sensitive_diagnostics::DesktopSnapshotExportOutcome::Exported,
        )
        .map_err(|mut error| {
    ...
    let report = serde_json::json!({
        "schema_version": satelle::core::sensitive_diagnostics::DESKTOP_SNAPSHOT_SCHEMA_VERSION,
...
fn mcp_install_profile(selected: Option<&satelle::core::SelectedProfile>) -> Option<&str> {
    selected
        .filter(|profile| {
            matches!(
                profile.source,
                satelle::core::ProfileSelectionSource::CliFlag
                    | satelle::core::ProfileSelectionSource::Environment
            )
        })
...
mod mcp_install_cli_tests {
    ...
    fn explicit_profile_reaches_the_installer_request() {
        ...
        let selected = satelle::core::SelectedProfile {
            name: cli.profile.expect("explicit profile"),
            source: satelle::core::ProfileSelectionSource::CliFlag,
        };
    ...
    fn environment_selected_profile_reaches_the_installer_request() {
        let selected = satelle::core::SelectedProfile {
            name: "work".to_string(),
            source: satelle::core::ProfileSelectionSource::Environment,
        };
    ...
    fn project_selected_profile_is_not_pinned_in_the_global_installer_request() {
        let selected = satelle::core::SelectedProfile {
            ...
            source: satelle::core::ProfileSelectionSource::ProjectConfig,
        };
        let command = parse_install(&["satelle", "mcp", "install", "--target", "cursor"]);
...
fn apply_pending_desktop_selection(
    ...
    host.desktop_bindings
        ...
        .or_insert_with(|| satelle::core::DesktopBindingConfig {
...
struct RawSubprocessInvocation {
    output: PathBuf,
    command: satelle::core::sensitive_diagnostics::RawSubprocessCommand,
...
impl RawSubprocessInvocation {
    ...
    fn begin(&mut self, host: &SelectedHost) -> Result<(), CliFailure> {
        ...
        let manifest = transport
            .begin_raw_subprocess_export(&satelle::transport::RawSubprocessBeginRequest::new(
                &host.alias,
                self.command,
                &self.invocation_id,
            ))
        ...
        self.capture = match transport::RawSubprocessCapture::begin(manifest) {
            ...
            Err(_) => {
                let _ = transport.acknowledge_raw_subprocess_export(
                    &self.invocation_id,
                    satelle::core::sensitive_diagnostics::RawDiagnosticExportOutcome::Failed,
                );
                return Err(failure(SatelleError::raw_diagnostics_failure(
                    satelle::core::sensitive_diagnostics::RawDiagnosticFailure::RedactionFailed,
    ...
    fn export_inner(&mut self) -> Result<(), CliFailure> {
        let host = self.host.as_ref().ok_or_else(|| {
            failure(SatelleError::raw_diagnostics_failure(
                satelle::core::sensitive_diagnostics::RawDiagnosticFailure::ExportFailed,
        ...
        let artifact = capture
            .finish(&satelle::core::sensitive_diagnostics::DiagnosticRedactor::default())
            .map_err(|_| {
                failure(SatelleError::raw_diagnostics_failure(
                    satelle::core::sensitive_diagnostics::RawDiagnosticFailure::RedactionFailed,
        ...
        if bytes.len() > satelle::core::sensitive_diagnostics::MAX_RAW_PROTOCOL_BYTES {
            return Err(failure(SatelleError::raw_diagnostics_failure(
                satelle::core::sensitive_diagnostics::RawDiagnosticFailure::RedactionFailed,
        ...
        if let Err(error) = persist_new_owner_only_diagnostic_file(&self.output, &staging, &bytes) {
            let mut export_error = SatelleError::raw_diagnostics_failure(
                satelle::core::sensitive_diagnostics::RawDiagnosticFailure::StagingFailed,
        ...
        transport
            .acknowledge_raw_subprocess_export(
                &self.invocation_id,
                satelle::core::sensitive_diagnostics::RawDiagnosticExportOutcome::Exported,
            )
            .map_err(|mut error| {
                error.details.insert(
                    "output_path".to_string(),
...
fn run_setup_inner(
    ...
    {
        satelle::host::validate_provider_binding_authorization(authorization).map_err(failure)?;
    }
    ...
    let local_transport = host.config.transport == satelle::core::TransportKind::Local;
        ...
        && authenticated_host_available
    {
        let authenticated_bootstrap_user = (host.config.transport
            == satelle::core::TransportKind::Ssh)
        ...
        && report.required_input.is_empty()
    {
        ...
        if !authenticated_host_available || (local_transport && report.mutation_planned) {
            ...
            let validation = transport_for(&host)?.validate_provider_descriptor(
                ...
                provider_alias,
                provider_selection
                    .validation_options(satelle::core::ProviderAuthValidationMode::Cached),
            );
            match validation {
                Ok(validation)
                    if validation.validation.outcome()
                        == satelle::core::ProviderAuthValidationOutcome::UnresolvedHostSecret =>
                {
    ...
    if !command.dry_run && report.required_input.is_empty() && report.mutation_planned {
        ...
        {
            let (model_alias, provider_alias) = provider_selection.validation_aliases();
            let provider_transport = if host.config.transport == satelle::core::TransportKind::Ssh {
                transport_for_with_ssh_bootstrap(&host, Some(SshBootstrapScope::Admin))?
            } else {
                transport_for(&host)?
            };
                ...
                let validation = if authorization_requires_provisioning {
                    ...
                    let cached = provider_transport
                        .validate_provider_descriptor(
                            ...
                            authorization.requested_model_alias(),
                            authorization.requested_provider_alias(),
                            satelle::host::ProviderDescriptorValidationOptions::new(
                                satelle::core::ProviderAuthValidationMode::Cached,
                                provider_selection.model_alias_from_project,
                                provider_selection.provider_alias_from_project,
                    ...
                    if cached.validation.outcome()
                        == satelle::core::ProviderAuthValidationOutcome::ConfiguredDeferred
                    {
                        Some(
                            provider_transport
                                .validate_provider_descriptor(
                                    ...
                                    authorization.requested_provider_alias(),
                                    satelle::host::ProviderDescriptorValidationOptions::new(
                                        satelle::core::ProviderAuthValidationMode::RefreshProviderSmoke,
                                        provider_selection.model_alias_from_project,
                                        provider_selection.provider_alias_from_project,
                                        authorization.experimental_provider_computer_use(),
                                    ),
...
fn accept_setup_provider_auth_validation(
    ...
    use satelle::core::ProviderAuthValidationOutcome::{
    ...
    if let Some(authorization) = provider_selection.authorization.as_ref() {
        let current_binding = satelle::core::ResolvedProviderBinding::from_authorization(
            authorization.clone(),
...
fn native_verification_status(findings: &[satelle::core::DoctorFinding]) -> &'static str {
...
fn provision_provider_secret(
    ...
    command: &SetupCommand,
    format: &OutputFormat,
) -> Result<satelle::transport::ProviderSecretProvisioningResponse, CliFailure> {
    ensure_file_provider_secret_source(authorization)?;
    ...
    let idempotency_key = format!(
        ...
        satelle::transport::RequestId::new()
    );
    let preview_metadata = satelle::transport::ProviderSecretProvisioningMetadata::new(
        desktop_binding,
        authorization.clone(),
        true,
    );
    ...
    let metadata = satelle::transport::ProviderSecretProvisioningMetadata::new(
        desktop_binding,
        authorization.clone(),
...
fn run_repair(
    ...
    let raw_output = prepare_raw_subprocess_capture(
        ...
        command.yes,
        command.dry_run,
        satelle::core::sensitive_diagnostics::RawSubprocessCommand::Repair,
    )?;
    let mut raw = raw_output.map(|output| {
        RawSubprocessInvocation::new(
            output,
...
fn doctor_event_records(
    ...
    for event in &report.probe_schedule_events {
        ...
        match event {
            satelle::core::doctor::DoctorProbeScheduleEvent::Started { timestamp, .. } => {
            ...
            satelle::core::doctor::DoctorProbeScheduleEvent::Finished { timestamp, .. } => {
                let mut record = doctor_event(
                    &mut seq,
...
mod doctor_event_tests {
    use super::*;
    use satelle::core::{DoctorProbeResult, DoctorSchemaVersion, DoctorSummary};
    ...
    fn doctor_events_preserve_completion_order_without_changing_final_report_order() {
        ...
        let report = DoctorReport {
            ...
            probe_schedule_events: vec![
                satelle::core::doctor::DoctorProbeScheduleEvent::Started {
                    probe_id: "provider.smoke.refresh".to_string(),
                    timestamp: "2026-07-29T00:00:10Z".to_string(),
                },
                satelle::core::doctor::DoctorProbeScheduleEvent::Finished {
                    probe_id: "provider.smoke.refresh".to_string(),
...
fn print_doctor_failed_event(
    ...
    seq: u64,
    error: &SatelleError,
    partial_probe_results: &[satelle::core::DoctorProbeResult],
) -> Result<(), SatelleError> {
...
fn post_start_doctor_failure_event_includes_completed_probe_results() {
    ...
    let partial = satelle::core::DoctorProbeResult {
    ...
    match value {
        serde_json::Value::Object(fields) => {
            for (name, field) in fields {
                let name = satelle::core::config_toml_key(name);
...
fn redacted_config_json(
    config: &satelle::core::SatelleConfig,
    show_secret_references: bool,
) -> serde_json::Value {
        ...
        for (binding_alias, binding) in &host.desktop_bindings {
            for (auth_alias, source) in &binding.provider_auth {
                ...
                if host.transport != TransportKind::Local
                    || host.setup_mode == Some(satelle::core::SetupMode::Persistent)
                {
                    descriptor["path"] = json!(null);
                    descriptor["normalization_status"] = json!("remote_home_not_checked");
                    continue;
                }
                let home = home.get_or_init(satelle::core::resolver_account_home);
                match satelle::core::expand_secret_file_path(path, home.as_deref()) {
                    Ok(path) => {
...
fn model_provider_config_json(
    config: &satelle::core::ResolvedConfig,
    selected_host: &str,
...
impl ProviderSelection {
    ...
    fn validation_options(
        &self,
        mode: satelle::core::ProviderAuthValidationMode,
    ) -> satelle::host::ProviderDescriptorValidationOptions {
        satelle::host::ProviderDescriptorValidationOptions::new(
            mode,
...
fn release_ssh_state_owner() -> Result<(), CliFailure> {
    let state_root = satelle::core::state_dir().map_err(failure)?;
...
mod ssh_state_release_tests {
    ...
    fn stale_release_request_is_removed_without_stopping_the_next_daemon() {
        let state = satelle::host::test_support::TestStateDir::new()
...
fn start_host_daemon_with(
    ...
    let forwarded_telemetry = command
        ...
        .as_deref()
        .map(serde_json::from_str::<satelle::core::telemetry::TelemetryConfig>)
        .transpose()
        .map_err(|_| {
            failure(SatelleError::invalid_usage(
    ...
    let forwarded_recording = command
        ...
        .as_deref()
        .map(serde_json::from_str::<satelle::core::recording::RecordingPolicy>)
        .transpose()
        .map_err(|_| {
            failure(SatelleError::invalid_usage(
    ...
    let forwarded_queue = command
        .queue_config_json
        .as_deref()
        .map(serde_json::from_str::<satelle::core::queue::QueueConfig>)
        .transpose()
        .map_err(|_| {
            failure(SatelleError::invalid_usage(
    ...
    let initial_identity = match (
        ...
        (Some(identity), Some(operation_id), Some(encoded_record)) => {
            ...
            let record =
                satelle::core::SshIdentityCommitRecord::parse(encoded_record).map_err(|_| {
                    failure(SatelleError::invalid_usage(
    ...
    let state_release_root = configured_host
        ...
        .map_or_else(satelle::core::state_dir, Ok)
        .map_err(failure)?;
...
) -> Result<Option<satelle::core::TimeoutConfig>, SatelleError> {
    match (native_readiness_timeout_ms, provider_smoke_timeout_ms) {
        (Some(native), Some(provider)) => Ok(Some(satelle::core::TimeoutConfig {
            native_readiness: satelle::core::ExplicitDuration::parse(&format!("{native}ms")),
            provider_smoke_test: satelle::core::ExplicitDuration::parse(&format!("{provider}ms")),
            turn_execution: None,
        })),
        (None, None) => Ok(None),
...
fn read_daemon_tls_config(
    ...
    let client_trust = client_ca
        .as_ref()
        .map(|ca| satelle::transport::DaemonClientTrust {
            ca_pem: ca.as_bytes(),
...
mod daemon_tls_watcher_tests {
    ...
    async fn reconciles_the_startup_gap_and_later_notify_rotation() {
        use satelle::host::{ApiScopes, test_support::TestStateDir};
        ...
        let response = client
            ...
            .header(
                "Satelle-Request-Id",
                satelle::transport::RequestId::new().to_string(),
            )
            .send()
            .await
...
mod bootstrap_startup_tests {
    ...
    async fn failed_start_host_daemon_does_not_strand_maintenance() {
        ...
        tokio::task::spawn_blocking(move || {
            let client = satelle::transport::DaemonClient::loopback(address, token, host_identity)
    ...
    async fn bootstrap_maintenance_starts_only_at_authenticated_begin_and_completes() {
        ...
        tokio::task::spawn_blocking(move || {
            let client = satelle::transport::DaemonClient::loopback(
                address,
                completion_token,
...
mod on_demand_idle_timeout_tests {
    ...
    fn defaults_to_ten_minutes_and_accepts_an_explicit_host_value() {
        ...
        config.daemon_idle_timeout = Some(
            satelle::core::ExplicitDuration::parse("75s")
...
) -> Result<(&'a str, &'a satelle::core::DesktopBindingConfig), SatelleError> {
...
#[cfg(feature = "test-support")]
fn test_support_desktop_binding() -> &'static satelle::core::DesktopBindingConfig {
    static BINDING: std::sync::LazyLock<satelle::core::DesktopBindingConfig> =
        std::sync::LazyLock::new(|| satelle::core::DesktopBindingConfig {
...
fn run_host_update_invocation(
    ...
    if !report.confirmation_required {
        if format.is_structured() {
            ...
            if report.status == satelle::core::host_update::HostUpdateStatus::UpToDate
                && !has_skipped_targets
            {
...
struct PlannedRemoteHostUpdate {
    host: SelectedHost,
    plan: Result<satelle::core::host_update::HostUpdateReport, SatelleError>,
}

fn selected_host_update_components(
    raw_components: &[String],
) -> (Vec<satelle::core::host_update::HostUpdateComponent>, bool) {
    ...
    let components = raw_components
        ...
        .filter_map(|component| match component.as_str() {
            "host" => Some(satelle::core::host_update::HostUpdateComponent::Host),
            "codex" => Some(satelle::core::host_update::HostUpdateComponent::Codex),
            "all" => None,
...
mod host_update_consent_tests {
    ...
    fn declined_consent_skips_only_hosts_that_required_mutation() {
        let unchanged = declined_remote_host_update_outcome(
            satelle::core::host_update::HostUpdateReport::new("current", Vec::new(), Vec::new()),
            None,
            &[],
        );
        let mut changed_report =
            satelle::core::host_update::HostUpdateReport::new("outdated", Vec::new(), Vec::new());
...
fn local_storage_state_root(host: &SelectedHost) -> Result<PathBuf, CliFailure> {
    satelle::host::HostService::resolved_daemon_paths_for_host(&host.config)
...
fn local_storage_state_root_uses_the_selected_hosts_resolved_daemon_home() {
    let state =
        satelle::host::test_support::TestStateDir::new().expect("create local storage path state");
...
fn apply_local_offline_storage_maintenance<T>(
    ...
    satelle::host::HostService::start_offline_storage_maintenance(
        state_root,
        &operation_id,
        action_id,
        action_label,
    ...
    let result = match mutate() {
        Ok(result) => result,
        Err(source) => {
            let _ = satelle::host::HostService::record_failed_offline_storage_maintenance(
                state_root,
                &operation_id,
                action_id,
            );
    ...
    if let Err(source) = satelle::host::HostService::record_completed_offline_storage_maintenance(
        state_root,
        &operation_id,
        action_id,
        action_label,
...
fn preview_local_storage_restore_before_consent(
    ...
    satelle::host::HostService::preview_storage_restore(state_root, backup).map_err(failure)
}
    ...
    match command {
        ...
        HostStorageCommand::Restore(command) => {
            ...
            let activation = if let Some(state_root) = &local_state_root {
                serde_json::to_value(apply_local_offline_storage_maintenance(
                    ...
                    || {
                        satelle::host::HostService::restore_storage_backup_offline(
                            state_root,
                            &command.backup,
                        )
                    },
        ...
        HostStorageCommand::Backup {
            ...
            let (cleanup_plan, service_recovery_required) =
                if let Some(state_root) = &local_state_root {
                    (
                        satelle::host::HostService::plan_storage_backup_cleanup(state_root)
                            .map_err(failure)?,
                        false,
                    )
                } else {
            ...
            let cleanup = if let Some(state_root) = &local_state_root {
                let removed_backup_file_names = apply_local_offline_storage_maintenance(
                    ...
                    || {
                        satelle::host::HostService::cleanup_planned_storage_backups_offline(
                            state_root,
                            &cleanup_plan.eligible_backup_file_names,
                            &recovery_command,
                        )
        ...
        HostStoreCommand::Reset(command) => {
            ...
            let reset = if let Some(state_root) = &local_state_root {
                serde_json::to_value(apply_local_offline_storage_maintenance(
                    ...
                    || {
                        satelle::host::HostService::reset_store_metadata_offline(
                            state_root,
...
fn run_host_storage_completion_recovery(
    ...
    let result = if host.config.transport == TransportKind::Local {
        ...
        satelle::host::HostService::record_completed_offline_storage_maintenance(
            &state_root,
...
fn run_offline_storage_backup_cleanup_plan(
    ...
    let plan = satelle::host::HostService::plan_storage_backup_cleanup(&command.state_root)
        .map_err(failure)?;
...
fn run_offline_storage_restore_preview(
    ...
    satelle::host::HostService::preview_storage_restore(&command.state_root, &command.backup)
        .map_err(failure)?;
            ...
            let remote_choices = resolved_config.map_or_else(Vec::new, |resolved| {
                self_update::remote_host_choices(
                    ...
                    resolved
                        ...
                        .hosts
                        .iter()
                        .filter(|(_, host)| host.transport != satelle::core::TransportKind::Local)
...
fn load_image_attachments(
...
) -> Result<Vec<satelle::transport::ImageAttachment>, CliFailure> {
    use base64::Engine as _;
    use sha2::Digest as _;

    if paths.len() + remote_paths.len() > satelle::transport::MAX_IMAGE_ATTACHMENT_COUNT {
        return Err(failure(SatelleError::invalid_usage(
    ...
    let mut attachments = paths
        ...
        .map(|(index, path)| {
            let number = index + 1;
            let bytes = satelle::core::read_bounded_regular_file_no_follow(
                path,
                satelle::transport::MAX_IMAGE_ATTACHMENT_BYTES,
            )
            .map_err(|error| match error {
                satelle::core::SecureFileError::TooLarge => {
            ...
            if total > satelle::transport::MAX_IMAGE_ATTACHMENT_BYTES_TOTAL {
            ...
            Ok(satelle::transport::ImageAttachment::upload(
                media_type,
                bytes.len() as u64,
                sha256,
                data_base64,
    ...
    attachments.extend(
        remote_paths
            .iter()
            .cloned()
            .map(satelle::transport::ImageAttachment::host_file),
    );
    Ok(attachments)
}

fn validate_remote_image_host(paths: &[String], host: &SelectedHost) -> Result<(), CliFailure> {
    if !paths.is_empty() && host.config.transport == satelle::core::TransportKind::Local {
        return Err(failure(SatelleError::invalid_usage(
...
fn image_loader_accepts_a_relative_png_with_truthful_metadata() {
    ...
    let satelle::transport::ImageAttachment::Upload {
        media_type,
        size_bytes,
        sha256,
        data_base64,
...
fn image_loader_checks_the_count_before_reading_files() {
    let paths = vec![PathBuf::from("not-read"); satelle::transport::MAX_IMAGE_ATTACHMENT_COUNT + 1];
...
fn image_loader_rejects_unsupported_media_and_oversized_files() {
    ...
    file.set_len((satelle::transport::MAX_IMAGE_ATTACHMENT_BYTES + 1) as u64)
...
fn turn_request_construction_carries_provider_aliases_refresh_and_one_shot_opt_in() {
    ...
    let request = build_turn_request(
        ...
        &selection,
        true,
        satelle::core::DEFAULT_TURN_EXECUTION_TIMEOUT_MS,
        None,
    );
    ...
    assert_eq!(
        ...
        json!({
            ...
            "experimental_provider_computer_use": true,
            "turn_execution_timeout_ms": satelle::core::DEFAULT_TURN_EXECUTION_TIMEOUT_MS,
        })
    );
}
...
fn prepare_turn_recording(
    ...
    transport: &dyn transport::TransportClient,
    source_host: &str,
) -> Result<Option<satelle::core::recording::RecordingRequest>, CliFailure> {
    ...
    let retention_ms = recording
        ...
        .as_deref()
        .map(|value| {
            satelle::core::recording::RecordingRetention::parse(value)
    ...
    Ok(Some(satelle::core::recording::RecordingRequest::new(
        preflight.mode,
...
enum RawDiagnosticCaptureKind {
    Protocol,
    Subprocess(satelle::core::sensitive_diagnostics::RawSubprocessCommand),
}
...
impl RawDiagnosticCaptureKind {
    ...
    const fn export_name(self) -> &'static str {
        match self {
            Self::Protocol => "raw protocol",
            Self::Subprocess(satelle::core::sensitive_diagnostics::RawSubprocessCommand::Setup) => {
...
fn raw_protocol_consent_requires_one_exact_noninteractive_invocation() {
    let state = satelle::host::test_support::TestStateDir::new().unwrap();
...
fn run_prompt(
    ...
    let provider_validation = match transport.validate_provider_descriptor(
        ...
        provider_selection
            ...
            .as_deref()
            .unwrap_or("codex-default"),
        provider_selection.validation_options(satelle::core::ProviderAuthValidationMode::Cached),
    ) {
        Ok(validation) => validation,
        Err(error) => {
            let error = if host.config.transport == satelle::core::TransportKind::Direct
...
fn print_queue_status(
    status: &satelle::core::queue::QueueStatus,
    host_alias: &str,
    format: OutputFormat,
) -> Result<(), CliFailure> {
    if format.is_structured() {
...
fn resolve_turn_execution_timeout_ms(
    ...
    let requested = satelle::core::TurnExecutionDuration::parse(command_timeout).ok_or_else(|| {
        failure(SatelleError::invalid_usage(
...
fn configured_turn_execution_timeout_ms(host_config: &satelle::core::HostConfig) -> u64 {
    host_config
        .timeouts
        .as_ref()
        ...
        .map_or(
            satelle::core::DEFAULT_TURN_EXECUTION_TIMEOUT_MS,
            satelle::core::TurnExecutionDuration::milliseconds,
        )
}
...
impl TurnEventOutput {
    ...
    fn emit_command_failed(
        ...
        durable_handles: Option<(&SessionId, &satelle::core::TurnId)>,
    ) -> Result<(), SatelleError> {
...
mod setup_desktop_binding_tests {
    use super::*;
    use satelle::core::{DesktopSessionRecord, HostSessionsSchemaVersion};
    ...
    fn execution_desktop_probe_requires_an_explicit_policy() {
        ...
        binding.desktop_session_native_selector =
            Some(satelle::core::DesktopSessionNativeSelector {
