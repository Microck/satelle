...
use crate::core::{DaemonPathOverrides, SetupMode};
...
pub const WINDOWS_SERVICE_CONFIG_SCHEMA: &str = "satelle.host-service.v8";
...
pub struct DaemonResolvedPathSet {
    ...
    pub sources: crate::core::SatellePathSources,
...
impl DaemonResolvedPathSet {
    ...
    pub fn for_offline_state_root(state_root: &Path) -> Self {
        ...
        let source = crate::core::PathSource::ServiceConfig;
        Self {
            ...
            state_root,
            sources: crate::core::SatellePathSources {
                config_file: source,
                cache_root: source,
                state_root: source,
                sqlite_store: source,
    ...
    pub fn with_service_overrides(&self, overrides: &DaemonPathOverrides) -> Self {
        ...
        if let Some(home) = overrides.home.as_ref() {
            ...
            planned.sources.config_file = crate::core::PathSource::ServiceConfig;
            planned.sources.cache_root = crate::core::PathSource::ServiceConfig;
            planned.sources.state_root = crate::core::PathSource::ServiceConfig;
            planned.sources.operator_log_root = crate::core::PathSource::ServiceConfig;
        }
...
impl From<&crate::core::SatellePathSet> for DaemonResolvedPathSet {
    fn from(paths: &crate::core::SatellePathSet) -> Self {
        Self {
...
pub fn render_launchd_user_plist(
    ...
    render_launchd_user_plist_with_policies(
        binary,
        bind,
        overrides,
        storage_policy,
...
pub fn render_launchd_user_plist_with_policies(
    binary: &Path,
    bind: &str,
    overrides: &DaemonPathOverrides,
    storage_policy: PersistentHostStoragePolicy,
    ...
    let recording_argument = if let Some(policy) = recording {
        policy
            .validate()
            .map_err(|_| WindowsServiceDefinitionError::InvalidServiceConfig)?;
        let encoded = serde_json::to_string(policy)
            .map_err(|_| WindowsServiceDefinitionError::InvalidServiceConfig)?;
        format!(
            "<string>--recording-config-json</string><string>{}</string>",
            xml_escape(&encoded)
        )
    } else {
        String::new()
    ...
    queue
        .validate()
        .map_err(|_| WindowsServiceDefinitionError::InvalidServiceConfig)?;
    let queue_argument = format!(
        "<string>--queue-config-json</string><string>{}</string>",
        xml_escape(
...
impl PersistentHostStoragePolicy {
    pub fn from_host_config(config: &crate::core::HostConfig) -> Self {
        Self {
            setup_ledger_retention_ms: config.setup_ledger_retention.as_ref().map_or(
                DEFAULT_SETUP_LEDGER_RETENTION_MS,
                crate::core::ExplicitDuration::milliseconds,
            ),
            session_metadata_retention_hours: config.session_metadata_retention.as_ref().map_or(
                crate::core::DEFAULT_SESSION_METADATA_RETENTION_HOURS,
                |retention| retention.hours(),
            ),
            sqlite_log_retention_hours: config.sqlite_log_retention.as_ref().map_or(
                crate::core::DEFAULT_SQLITE_LOG_RETENTION_HOURS,
                |retention| retention.hours(),
            ),
    ...
    pub const fn setup_ledger_retention_ms(self) -> u64 {
        self.setup_ledger_retention_ms
    }

    pub const fn session_metadata_retention_hours(self) -> u64 {
        self.session_metadata_retention_hours
    }

    pub const fn operator_log_retained_files(self) -> usize {
        self.operator_log_retained_files
    }
    ...
    fn validate(self) -> Result<(), &'static str> {
        ...
        if !(crate::core::MIN_SESSION_METADATA_RETENTION_HOURS
            ..=crate::core::MAX_SESSION_METADATA_RETENTION_HOURS)
            .contains(&self.session_metadata_retention_hours)
        {
            return Err("invalid session metadata retention");
        }
        if !(crate::core::MIN_SESSION_METADATA_RETENTION_HOURS
            ..=crate::core::MAX_SESSION_METADATA_RETENTION_HOURS)
            .contains(&self.sqlite_log_retention_hours)
        {
            return Err("invalid SQLite Log Entry retention");
        }
        if !(1..=crate::core::MAX_OPERATOR_LOG_RETAINED_FILES)
            .contains(&self.operator_log_retained_files)
        {
            return Err("invalid operator log retention");
        }
        Ok(())
    }
}
...
pub struct WindowsServiceConfigV8 {
    schema: String,
...
impl WindowsServiceConfigV8 {
    ...
    pub fn recording(&self) -> Option<&crate::core::recording::RecordingPolicy> {
        self.recording.as_ref()
    }

    pub const fn queue(&self) -> &crate::core::queue::QueueConfig {
        &self.queue
    }
    ...
    fn validate(&self) -> Result<(), &'static str> {
        ...
        {
            return Err("invalid recording policy");
        }
        self.queue.validate()?;
        Ok(())
    }
...
impl<'de> Deserialize<'de> for WindowsServiceConfigV8 {
...
fn validate_setup_ledger_retention_ms(value: u64) -> Result<(), &'static str> {
    if value == 0 || value > crate::core::MAX_SETUP_LEDGER_RETENTION_MS {
...
mod tests {
    ...
    fn storage_policy() -> PersistentHostStoragePolicy {
        PersistentHostStoragePolicy::new(3_600_000, 30 * 24, 45 * 24, 12)
    ...
    fn windows_service_config_contains_only_owned_launch_configuration() {
        ...
        let config = WindowsServiceConfigV8::new("127.0.0.1:3001", &overrides, enabled_policy)
    ...
    fn windows_service_config_hard_cuts_v5_and_retains_host_telemetry() {
        ...
        assert_eq!(
            serde_json::from_value::<WindowsServiceConfigV8>(encoded.clone())
        ...
        assert!(serde_json::from_value::<WindowsServiceConfigV8>(legacy).is_err());
    }
    ...
    fn windows_service_config_rejects_arbitrary_or_invalid_configuration() {
        ...
        assert!(serde_json::from_value::<WindowsServiceConfigV8>(invalid_arguments).is_err());
        ...
        assert!(serde_json::from_value::<WindowsServiceConfigV8>(invalid_environment).is_err());
        ...
        assert!(serde_json::from_value::<WindowsServiceConfigV8>(invalid_retention).is_err());
        ...
        assert!(
            serde_json::from_value::<WindowsServiceConfigV8>(missing_platform_log_policy).is_err(),
    ...
    fn daemon_path_plan_derives_all_owned_paths_and_restart_directories() {
        let current = DaemonResolvedPathSet {
            ...
            sources: crate::core::SatellePathSources {
                config_file: crate::core::PathSource::OsDefault,
                cache_root: crate::core::PathSource::OsDefault,
                state_root: crate::core::PathSource::OsDefault,
                sqlite_store: crate::core::PathSource::OsDefault,
                operator_log_root: crate::core::PathSource::OsDefault,
                recording_root: crate::core::PathSource::OsDefault,
                project_config_file: crate::core::PathSource::ProjectDiscovery,
                install_receipt: crate::core::PathSource::OsDefault,
            },
            project_config_file: None,
    ...
    fn launchd_definition_is_user_scoped_loopback_and_satelle_owned() {
        ...
        let telemetry_plist = render_launchd_user_plist_with_policies(
    ...
    fn windows_service_config_recovers_persistent_path_overrides() {
        ...
        let config = WindowsServiceConfigV8::new("127.0.0.1:3001", &overrides, storage_policy())
