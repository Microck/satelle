...
mod tests {
    ...
    fn private_state() -> PrivateState {
        ...
        #[cfg(unix)]
        {
            ...
            // Tempfile inherits the process umask. Make the fixture boundary
            // owner-only so the secure-directory check tests telemetry state,
            // independent of the test runner's group-write defaults.
            std::fs::set_permissions(temporary.path(), std::fs::Permissions::from_mode(0o700))
                .expect("make temporary state owner-only");
        }
