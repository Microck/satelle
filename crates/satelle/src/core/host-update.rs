...
pub const HOST_PROTOCOL_VERSION: &str = "22";
pub const HOST_STORAGE_SCHEMA_VERSION: i64 = 24;
...
impl HostUpdateReport {
    pub fn new(
        ...
        let skipped_actions: Vec<String> = targets
            .iter()
            .filter(|target| target.disposition == HostUpdateDisposition::Skipped)
            .flat_map(|target| target.remote_mutations.iter())
            .map(|mutation| mutation.operation.clone())
            .collect();
...
impl RepairUpgradeReport {
    ...
    pub fn partial_failure(
        ...
        self.recovery_command = Some(format!(
            ...
            crate::core::shell_argument(&self.host)
        ));
        self
    }
}
...
mod tests {
    ...
    #[test]
    fn unfinished_postchecks_cannot_report_a_successful_update() {
        let report = HostUpdateReport::new(
            "office",
            vec![HostUpdateComponent::Host],
            vec![update_target(
