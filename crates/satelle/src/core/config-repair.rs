use crate::core::config_includes::{self, ConfigDocuments};
use crate::core::{ConfigFileSource, ConfigSourceKind, ErrorCode, ResolvedConfig, SatelleError};
use serde::Serialize;
use serde_json::json;
use std::path::Path;
...
pub fn plan(
    ...
    let paths = crate::core::resolve_path_set(cwd)?;
    ...
    let resolved = crate::core::load_config_with_profile_selection(
        cwd,
        flag_profile,
        true,
        None,
...
fn validate_document(source: &ConfigFileSource, value: toml::Value) -> Result<(), SatelleError> {
    match source.source {
        ConfigSourceKind::UserConfig => {
            crate::core::parse_user_config_value(&source.path, value).map(|_| ())
        }
        ConfigSourceKind::ProjectConfig => {
            crate::core::project_config::parse(&source.path, value).map(|_| ())
        }
    }
}
