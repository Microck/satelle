use crate::core::ids::{SessionId, TurnId};
...
pub enum TurnAdmissionFailure {
    NotAdmitted(Box<crate::core::SatelleError>),
    AdmissionUnknown {
        error: Box<crate::core::SatelleError>,
        events: Vec<crate::core::SatelleEvent>,
    },
    Admitted {
        error: Box<crate::core::SatelleError>,
...
impl TurnAdmissionFailure {
    ...
    pub fn error(&self) -> &crate::core::SatelleError {
        match self {
    ...
    pub fn into_error(self) -> crate::core::SatelleError {
        match self {
            Self::NotAdmitted(error)
            | Self::AdmissionUnknown { error, .. }
            | Self::Admitted { error, .. } => *error,
    ...
    pub fn events(&self) -> &[crate::core::SatelleEvent] {
        match self {
    ...
    pub fn not_admitted(error: crate::core::SatelleError) -> Self {
        Self::NotAdmitted(Box::new(error))
    }

    pub fn admission_unknown(error: crate::core::SatelleError) -> Self {
...
mod tests {
    ...
    fn turn_admission_failure_preserves_phase_error_and_durable_handles() {
        ...
        let admitted = TurnAdmissionFailure::admitted(
            crate::core::SatelleError::host_unreachable("host-test"),
            session,
            turn_id.clone(),
        );
        ...
        assert_eq!(
            admitted.error().code,
            crate::core::ErrorCode::HostUnreachable
        );

        let not_admitted =
            TurnAdmissionFailure::not_admitted(crate::core::SatelleError::computer_use_not_ready());
        assert_eq!(not_admitted.phase(), TurnAdmissionPhase::NotAdmitted);
        assert!(not_admitted.durable_handles().is_none());

        let event = crate::core::SatelleEventBody::new(
