...
use crate::core::SatelleError;
use crate::host::{ApiScopes, HostService, MutationAuthority, SetupOperationKind};
...
pub(super) async fn plan_setup_repair(
    ...
    let probes = match request
        ...
        .iter()
        .map(|probe| {
            crate::host::SetupRepairProbe::new(
                &probe.action_id,
                &probe.label,
                probe.retry_safe,
                match probe.postcondition {
                    SetupRepairPostcondition::Satisfied => {
                        crate::host::SetupRepairPostcondition::Satisfied
                    }
                    SetupRepairPostcondition::Unsatisfied => {
                        crate::host::SetupRepairPostcondition::Unsatisfied
                    }
                    SetupRepairPostcondition::Unknown => {
                        crate::host::SetupRepairPostcondition::Unknown
                    }
                },
            )
        })
    ...
    let selected_run_status = plan.selected_run_status().map(|status| match status {
        crate::host::SetupRunStatus::Running => SetupRepairRunStatus::Running,
        crate::host::SetupRunStatus::Completed => SetupRepairRunStatus::Completed,
        crate::host::SetupRunStatus::Failed => SetupRepairRunStatus::Failed,
        crate::host::SetupRunStatus::PartialFailure => SetupRepairRunStatus::PartialFailure,
        crate::host::SetupRunStatus::OutcomeUnknown => SetupRepairRunStatus::OutcomeUnknown,
    });
    let actions = plan
        .actions()
        .iter()
        .map(|action| SetupRepairPlanAction {
            ...
            decision: match action.decision() {
                crate::host::SetupRepairDecision::NoActionRequired => {
                    SetupRepairDecision::NoActionRequired
                }
                crate::host::SetupRepairDecision::RetryAutomatically => {
                    SetupRepairDecision::RetryAutomatically
                }
                crate::host::SetupRepairDecision::OperatorActionRequired => {
                    SetupRepairDecision::OperatorActionRequired
                }
                crate::host::SetupRepairDecision::ProbeRequired => {
            ...
            previous_status: action.previous_status().map(|status| match status {
                crate::host::SetupActionStatus::Planned => SetupRepairPreviousStatus::Planned,
                crate::host::SetupActionStatus::Started => SetupRepairPreviousStatus::Started,
                crate::host::SetupActionStatus::Completed => SetupRepairPreviousStatus::Completed,
                crate::host::SetupActionStatus::Failed => SetupRepairPreviousStatus::Failed,
                crate::host::SetupActionStatus::Skipped => SetupRepairPreviousStatus::Skipped,
                crate::host::SetupActionStatus::OutcomeUnknown => {
...
pub(super) async fn validate_provider_descriptor(
    ...
    let options = crate::host::ProviderDescriptorValidationOptions::new(
        request.mode(),
...
pub(super) async fn preview_provider_secret_provisioning(
    ...
    let upload_id = crate::transport::contract::RequestId::new().to_string();
    let keypair = crate::transport::provider_secret_crypto::generate_server_keypair();
    let response = ProviderSecretProvisioningPreviewResponse::new(
        ...
        upload_id.clone(),
        crate::transport::provider_secret_crypto::encode_canonical_base64(&keypair.public_key),
        expires_at_text,
    );
    let mut uploads = match state.provider_secret_uploads.lock() {
        Ok(uploads) => uploads,
...
pub(super) async fn provision_provider_secret(
    ...
    let encapsulated_key = match crate::transport::provider_secret_crypto::decode_canonical_base64(
    ...
    let ciphertext = match crate::transport::provider_secret_crypto::decode_canonical_base64(
        envelope.ciphertext(),
    ) {
        Ok(value) => value,
        Err(_) => {
    ...
    let mut secret_bytes = match crate::transport::provider_secret_crypto::decrypt_provider_secret(
...
pub(super) async fn begin_storage_migration(
    ...
    ApiJson(request): ApiJson<crate::transport::StorageMigrationPathsRequest>,
) -> Response {
    let service = Arc::clone(&state.service);
    ...
    .await
    {
        Ok(Ok(cleanup)) => {
            let mut response = authenticated_json_response(
                StatusCode::OK,
                &crate::transport::StorageMigrationCleanupResponse::new(
                    authorized.request_id().clone(),
                    state.host_identity.clone(),
                    cleanup,
                ),
...
pub(super) async fn begin_bootstrap_maintenance(
    ...
    let plan_kind = match crate::host::BootstrapMaintenancePlanKind::parse(&plan_kind) {
        Ok(plan_kind) => plan_kind,
        Err(error) => return host_error::response(&state, &authorized, &error),
    };
    let authorized_for_plan = match plan_kind {
        crate::host::BootstrapMaintenancePlanKind::OnDemandHandoff => {
            bootstrap_maintenance_principal_is_authorized(&authorized)
        }
        crate::host::BootstrapMaintenancePlanKind::OnDemandHandoffWithManagedSetup
        | crate::host::BootstrapMaintenancePlanKind::PersistentHostService
        | crate::host::BootstrapMaintenancePlanKind::PersistentHostServiceWithManagedSetup
        | crate::host::BootstrapMaintenancePlanKind::PersistentHostStop
        | crate::host::BootstrapMaintenancePlanKind::PersistentHostRestart
        | crate::host::BootstrapMaintenancePlanKind::HostUpdate
        | crate::host::BootstrapMaintenancePlanKind::Repair => {
...
pub(super) async fn apply_managed_setup_action(
    ...
    .await
    {
        Ok(Ok(changed)) => authenticated_json_response(
            StatusCode::OK,
            &crate::transport::contract::ManagedSetupActionResponse::new(
                authorized.request_id().clone(),
                state.host_identity.clone(),
                operation_id,
                changed,
