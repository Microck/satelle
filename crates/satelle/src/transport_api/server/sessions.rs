...
use crate::core::{QueueRequestId, SatelleError, SessionId, TurnId};
use crate::host::{
    ...
    let body = match host_call(&state, &authorized, move || {
        ...
        .map_err(|_| {
            SatelleError::raw_diagnostics_failure(
                crate::core::sensitive_diagnostics::RawDiagnosticFailure::ExportFailed,
...
pub(super) async fn begin_raw_subprocess_export(
    ...
    let manifest = crate::core::sensitive_diagnostics::RawSubprocessManifest::new(
        request.source_host(),
        state.host_identity.clone(),
        request.command(),
...
pub(super) async fn prepare_raw_subprocess_export(
    ...
    if artifact_byte_size > crate::core::sensitive_diagnostics::MAX_RAW_PROTOCOL_BYTES {
        return request_error(
            &state,
            &authorized,
...
fn admission_wire_error(error: SatelleError) -> SatelleError {
    if error.code == crate::core::ErrorCode::Interrupted {
...
fn turn_intent(
    ...
    let attachments = attachments
        ...
        .map(|attachment| match attachment {
            crate::transport::ImageAttachment::Upload {
                media_type,
                size_bytes,
                sha256,
                data_base64,
            } => crate::host::AttachmentInput::upload(media_type, size_bytes, sha256, data_base64),
            crate::transport::ImageAttachment::HostFile { path } => {
                crate::host::AttachmentInput::host_file(path)
            }
        })
        .collect();
...
mod admission_action_tests {
    ...
    fn interrupted_admission_maps_to_wire_safe_conflict() {
        ...
        assert_eq!(mapped.code, crate::core::ErrorCode::StateConflict);
