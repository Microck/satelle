...
use crate::host::{ApiPrincipal, LiveEventReceiveError, LiveEventSubscription};
...
) -> Result<Arc<crate::core::SatelleEventBody>, LiveEventReceiveError> {
    match subscription {
...
async fn event_visible_to_principal(
    state: &Arc<DaemonState>,
    principal: &ApiPrincipal,
    event: &crate::core::SatelleEventBody,
) -> Result<bool, ()> {
    ...
    tokio::task::spawn_blocking(move || {
        service
            ...
            .or_else(|error| match error.code {
                ...
                | crate::core::ErrorCode::SessionNotFound => Ok(false),
                _ => Err(error),
            })
    })
    .await
...
mod tests {
    ...
    use crate::host::test_support::TestStateDir;
    use crate::transport::contract::WsServerControl;
