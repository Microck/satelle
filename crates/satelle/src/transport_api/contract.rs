...
pub(crate) const PROTOCOL_VERSION: &str = crate::core::host_update::HOST_PROTOCOL_VERSION;
...
impl LocalSetupOperationRequest {
    ...
    pub(crate) fn into_parts(
        ...
        String,
        Vec<String>,
        crate::core::DaemonPathOverrides,
    ) {
        (
            self.host,
            self.dry_run,
...
enum LocalSetupOperationOutcome {
    Completed {
        report: Box<crate::core::SetupReport>,
        mutation_planned: bool,
    },
    Failed {
        error: crate::core::SatelleError,
    },
}
...
impl LocalSetupOperationResponse {
    pub(crate) fn new(
        request_id: RequestId,
        host_identity: String,
        result: Result<crate::core::SetupReport, crate::core::SatelleError>,
    ) -> Self {
    ...
    pub(crate) fn into_result(self) -> Result<crate::core::SetupReport, crate::core::SatelleError> {
        match self.outcome {
...
impl LocalDoctorOperationRequest {
    ...
    pub(crate) fn into_inputs(
        ...
        (
            String,
            crate::core::doctor::DoctorScopeSelection,
            crate::core::DoctorOptions,
            crate::host::ProviderComputerUseIntent,
        ),
        crate::core::SatelleError,
    > {
        use crate::core::session::{EffectiveModelRef, ProviderBindingRef};

        let scope_selection = crate::core::doctor::DoctorScopeSelection::parse(&self.scopes)
            .map_err(|_| crate::core::SatelleError::invalid_usage("invalid Doctor scope"))?;
        let options =
            crate::core::DoctorOptions::new(self.refresh, self.probe_timeout.map(Into::into))?
                .with_serial_probes(self.serial_probes);
        let model = self
            .model_alias
            .map(EffectiveModelRef::new)
            .transpose()
            .map_err(|_| crate::core::SatelleError::invalid_usage("invalid model alias"))?;
        let provider = self
            .provider_alias
            .map(ProviderBindingRef::new)
            .transpose()
            .map_err(|_| crate::core::SatelleError::invalid_usage("invalid provider alias"))?;
        if model.is_some() != provider.is_some() {
            return Err(crate::core::SatelleError::invalid_usage(
                "Doctor model and provider aliases must be supplied together",
            ));
        }
        let provider_intent =
            crate::host::ProviderComputerUseIntent::new(model, provider, self.refresh)
...
enum LocalDoctorOperationOutcome {
    Completed {
        report: Box<crate::core::DoctorReport>,
        probe_schedule_events: Box<[crate::core::doctor::DoctorProbeScheduleEvent]>,
    },
    Failed {
        error: crate::core::SatelleError,
        partial_probe_results: Box<[crate::core::DoctorProbeResult]>,
    },
}
...
impl LocalDoctorOperationResponse {
    pub(crate) fn new(
        request_id: RequestId,
        host_identity: String,
        result: crate::host::DoctorExecutionResult,
    ) -> Self {
        let outcome = match result {
    ...
    pub(crate) fn into_result(self) -> crate::host::DoctorExecutionResult {
        match self.outcome {
            ...
            } => Err(crate::host::DoctorExecutionFailure {
                error,
                partial_probe_results,
            }),
        }
...
mod tests {
    ...
    fn local_setup_response_round_trips_success_and_exact_failure() {
        let report = crate::host::local_setup_plan_for_tests(
            "local-demo",
            false,
            "on_demand".to_string(),
            vec!["all".to_string()],
            crate::core::DaemonPathOverrides::default(),
        );
        ...
        let expected = crate::core::SatelleError::state_conflict();
        let response = LocalSetupOperationResponse::new(
            RequestId::new(),
            "host-local-demo".to_string(),
            Err(expected.clone()),
