use satelle::core::daemon_service::DaemonResolvedPathSet;
use satelle::core::{ConfigSourceKind, ResolvedConfig, SatelleError};
...
pub(super) fn run(
    ...
    let trusted = super::trusted_profile_allows_mutation(
        config.load()?,
        alias,
        satelle::core::MutationCommandFamily::HostStorageMigrate,
        command.yes
    ...
    let stage = match host.config.transport {
        satelle::core::TransportKind::Local => transport::storage_migration::apply_local(
            &host,
            &command.to,
            &plan,
            &operation_id,
...
pub(super) fn run_offline(command: OfflineCommand) -> Result<(), super::CliFailure> {
    ...
    let response = match command {
        OfflineCommand::Plan { source_paths, to } => {
            let plan =
                satelle::host::HostService::plan_storage_migration(&parse(&source_paths)?, &to)
                    .map_err(super::failure)?;
        ...
        OfflineCommand::Stage {
            ...
            let stage = satelle::host::HostService::stage_storage_migration(
                &parse(&source_paths)?,
                &to,
                &operation_id,
            )
        ...
        OfflineCommand::Rollback {
            ...
            satelle::host::HostService::rollback_storage_migration(&source_root, &operation_id)
                .map_err(super::failure)?;
...
impl BindingUpdate {
    pub fn plan(
        ...
        let (path, original, mut document) = match selected {
            ...
            None if host_alias == satelle::core::LOCAL_DEMO_HOST => {
                ...
                let defaults = satelle::core::SatelleConfig::defaults();
        ...
        let backup_path = satelle::core::resolve_path_set(&cwd)?
            .state_root
    ...
    pub fn backup(&self) -> Result<(), SatelleError> {
        ...
        satelle::core::persist_new_owner_only_config_file(
            &self.backup_path,
    ...
    pub fn apply(&self) -> Result<(), SatelleError> {
        ...
        if self.original.is_some() {
            ...
            satelle::core::persist_new_owner_only_config_file(&self.path, self.updated.as_bytes())
                .map_err(|_| config_error())
        }
    }
...
fn read_config(path: &Path) -> Result<String, SatelleError> {
    satelle::core::read_owner_controlled_config_file(path).map_err(|_| config_error())
}
...
mod tests {
    ...
    fn binding_backup_and_rollback_preserve_exact_bytes_and_operator_edits() {
        ...
        satelle::core::persist_new_owner_only_config_file(&update.path, original.as_bytes())
            .unwrap();
    ...
    fn concurrent_edit_blocks_backup_and_migration_created_config_can_roll_back() {
        ...
        satelle::core::persist_new_owner_only_config_file(&update.path, b"# new user edit\n")
            .unwrap();
