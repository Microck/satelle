use super::*;
use satelle::core::session::PublicSession;
use satelle::transport::{DaemonEventStream, EventSubscription, WsCloseReason};
...
impl DirectFixture {
    ...
    fn complete_while_server_is_down(&self, prompt: &str) -> PublicSession {
        ...
        let intent = satelle::host::TurnIntent::new(request.prompt(), request.execution_mode())
            .expect("construct local Turn intent");
        self.service
            .run(satelle::core::LOCAL_DEMO_HOST, &intent)
...
fn silent_terminal_event_loss_reconciles_without_waiting_for_socket_failure() {
    ...
    let noise = thread::spawn(move || {
        ...
        while !noise_stopped.load(Ordering::Acquire) {
            let intent = satelle::host::TurnIntent::new(
