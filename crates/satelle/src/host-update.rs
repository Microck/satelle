...
impl HostUpdateOutput {
    ...
    ) -> Result<(), satelle::core::SatelleError> {
        match self {
...
pub(crate) enum PlainUpdateStage<'a> {
    ...
    Completed,
    Cancelled,
    Failed(&'a satelle::core::SatelleError),
}
...
fn plan_host_targets(
    ...
    if let Some((service, disposition)) = request.service_inspection.zip(service_disposition) {
        ...
        let artifact_digest = if remote_mutations
            .iter()
            .any(|mutation| mutation.operation == "install-host-artifact")
        {
            artifact.as_ref().map(|artifact| artifact.digest.clone())
        } else {
            None
...
pub(crate) fn render_host_update_plain(
    ...
    let postcheck_failed = error
        .is_some_and(|error| error.code == satelle::core::ErrorCode::HostUpdatePostcheckFailed);
...
pub fn render_repair_upgrade_plan(
    report: &satelle::core::host_update::RepairUpgradeReport,
) -> String {
...
mod tests {
    ...
    fn plain_update_failure_preserves_successful_unchanged_and_skipped_targets() {
        ...
        let error = satelle::core::SatelleError::host_update_partially_applied(
            &report,
            "publish-host-service",
            "fixture failure",
        );
        ...
        let error = satelle::core::SatelleError::host_update_postcheck_failed(
            &report,
    ...
    fn host_selection_targets_only_exact_cli_host_artifacts() {
        ...
        assert_eq!(
            report.targets[0].artifact_digest.as_deref(),
            Some(artifact_digest.as_str())
        );
        assert_eq!(report.targets[1].artifact_digest, None);
    ...
    fn invalid_artifact_digest_blocks_the_update_plan() {
        ...
        .expect_err("an invalid artifact digest must fail before consent");

        assert!(matches!(error, HostUpdatePlanError::InvalidArtifact { .. }));
    }
    ...
    fn unsafe_owned_codex_update_is_a_skipped_manual_action() {
        ...
        assert_eq!(
            report.status,
            satelle::core::host_update::HostUpdateStatus::ManualActionRequired
        );
        assert!(!report.confirmation_required);
