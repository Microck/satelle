...
use clap::Args;
use satelle::core::config_repair::{KeyRepair, RepairFile, RepairPlan, manual_action};
use satelle::core::{ConfigSourceKind, ErrorCode, MutationCommandFamily, SatelleError};
use serde::Serialize;
use serde_json::json;
...
pub(super) fn run(
    ...
    let plan = satelle::core::config_repair::plan(&cwd, flag_profile, command.file.as_deref())
        .map_err(failure)?;
    let host = plan
        .resolved
    ...
    satelle::core::read_owner_controlled_config_file(&file.source.path)
        .map_err(|_| failure(manual_action(&file.source.path, ErrorCode::ConfigError)))?;
    let repaired = edit_document(file).map_err(failure)?;
    let backup_directory = satelle::core::resolve_path_set(&cwd)
        .map_err(failure)?
        .state_root
        .join("config-repair")
    ...
    satelle::core::persist_new_owner_only_config_file(&item.backup_path, file.original.as_bytes())
    ...
    for (source, original) in &plan.inputs {
        let current = match source.source {
            ConfigSourceKind::UserConfig => {
                satelle::core::read_owner_controlled_config_file(&source.path)
            }
            ConfigSourceKind::ProjectConfig => {
                satelle::core::read_bounded_regular_file_no_follow(&source.path, 1024 * 1024)
                    .and_then(|bytes| {
                        String::from_utf8(bytes)
                            .map_err(|_| satelle::core::SecureFileError::NotUtf8)
                    })
            }
        };
