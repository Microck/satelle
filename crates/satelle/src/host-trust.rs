...
mod tests {
    use super::*;
    #[cfg(unix)]
    use satelle::core::open_or_create_owner_only_directory;
    use satelle::core::{open_owner_only_directory, read_owner_only_secret_config_file};
    #[cfg(unix)]
    ...
    fn credential_helper_persistence_compares_nested_settings_and_literal_argv() {
        ...
        for (argument, account) in [("first argument", "one"), ("second argument", "two")] {
            let descriptor = ProviderSecretSource::ExecutableHelper(
                satelle::core::CredentialHelper::new(
                    vec!["/opt/helper".to_string(), argument.to_string()],
                    satelle::core::ExplicitDuration::parse("2s").unwrap(),
