...
use satelle::core::AdapterKind;
use satelle::core::daemon_service::{
...
use satelle::core::doctor::DoctorScopeSelection;
use satelle::core::session::{HostIdentityRef, PublicSession, TurnAdmissionFailure};
use satelle::core::{
...
pub(crate) struct AttachedTurnOutcome {
    ...
    pub(crate) recording: Option<satelle::core::recording::RecordingManifest>,
}

pub(crate) struct ProviderDescriptorValidationReport {
    pub(crate) resolved_binding: satelle::core::PublicResolvedProviderBinding,
    pub(crate) validation: satelle::core::ProviderAuthValidationResult,
}

fn setup_provider_intent(
    request: &satelle::transport::SetupVerificationRequest,
) -> Result<satelle::host::ProviderComputerUseIntent, SatelleError> {
    let provider_intent = match (request.model_alias(), request.provider_alias()) {
        (None, None) => satelle::host::ProviderComputerUseIntent::host_default(),
        (Some(model_alias), Some(provider_alias)) => satelle::host::ProviderComputerUseIntent::new(
            Some(
                satelle::core::session::EffectiveModelRef::new(model_alias)
                    .map_err(|error| SatelleError::invalid_usage(error.to_string()))?,
            ),
            Some(
                satelle::core::session::ProviderBindingRef::new(provider_alias)
                    .map_err(|error| SatelleError::invalid_usage(error.to_string()))?,
            ),
            true,
        )
...
pub(crate) trait TransportClient: Send {
    ...
    fn plan_setup_repair(
        &self,
        run_id: Option<&str>,
        probes: &[satelle::transport::SetupRepairProbe],
    ...
    fn authorize_provider_binding(
        &self,
        desktop_binding: &str,
        authorization: &satelle::core::ProviderBindingAuthorization,
    ) -> Result<satelle::core::PublicResolvedProviderBinding, SatelleError>;
    fn preview_provider_secret_provisioning(
        &self,
        metadata: &satelle::transport::ProviderSecretProvisioningMetadata,
        idempotency_key: &str,
    ) -> Result<satelle::transport::ProviderSecretProvisioningPreviewResponse, SatelleError>;
    fn provision_provider_secret(
        &self,
        preview: &satelle::transport::ProviderSecretProvisioningPreviewResponse,
        metadata: &satelle::transport::ProviderSecretProvisioningMetadata,
        secret: Zeroizing<Vec<u8>>,
        idempotency_key: &str,
    ) -> Result<satelle::transport::ProviderSecretProvisioningResponse, SatelleError>;
    fn validate_provider_descriptor(
        &self,
        desktop_binding: &str,
        model_alias: &str,
        provider_alias: &str,
        options: satelle::host::ProviderDescriptorValidationOptions,
    ) -> Result<ProviderDescriptorValidationReport, SatelleError>;
    fn host_status(&self) -> Result<HostStatus, SatelleError>;
    fn telemetry_status(&self) -> Result<satelle::core::telemetry::TelemetryStatus, SatelleError>;
    fn recording_preflight(
        &self,
        mode: satelle::core::recording::RecordingMode,
        retention_ms: Option<u64>,
        source_host: &str,
    ) -> Result<satelle::core::recording::RecordingPreflight, SatelleError>;
    fn host_paths(
        &self,
    ) -> Result<satelle::core::daemon_service::DaemonResolvedPathSet, SatelleError>;
    ...
    ) -> Result<satelle::core::queue::QueueStatus, SatelleError> {
    ...
    fn cancel_queue_request(
        &self,
        _queue_request_id: &QueueRequestId,
    ) -> Result<satelle::core::queue::QueueCancelResult, SatelleError> {
        Err(SatelleError::not_implemented(
    ...
    fn raw_protocol_export(
        &self,
        turn_id: &TurnId,
    ) -> Result<satelle::core::sensitive_diagnostics::RawProtocolArtifact, SatelleError>;
    fn acknowledge_raw_protocol_export(
        &self,
        turn_id: &TurnId,
        outcome: satelle::core::sensitive_diagnostics::RawDiagnosticExportOutcome,
    ) -> Result<(), SatelleError>;
    fn begin_raw_subprocess_export(
        &self,
        _request: &satelle::transport::RawSubprocessBeginRequest,
    ) -> Result<satelle::core::sensitive_diagnostics::RawSubprocessManifest, SatelleError> {
        Err(SatelleError::not_implemented(
    ...
    fn acknowledge_raw_subprocess_export(
        &self,
        _invocation_id: &str,
        _outcome: satelle::core::sensitive_diagnostics::RawDiagnosticExportOutcome,
    ) -> Result<(), SatelleError> {
        Err(SatelleError::not_implemented(
    ...
    fn capture_desktop_snapshot(
        &self,
        _request: &satelle::transport::DesktopSnapshotCaptureRequest,
    ...
    fn acknowledge_desktop_snapshot(
        &self,
        _snapshot_id: &str,
        _outcome: satelle::core::sensitive_diagnostics::DesktopSnapshotExportOutcome,
    ) -> Result<(), SatelleError> {
        Err(SatelleError::not_implemented(
    ...
    fn setup_history(&self) -> Result<satelle::host::SetupHistory, SatelleError>;
}
...
impl TaskArtifacts {
    fn from_host(artifacts: satelle::host::TaskArtifactSet) -> Self {
        Self {
    ...
    fn from_response(artifacts: satelle::transport::TaskArtifactsResponse) -> Self {
        Self {
            plan: artifacts.plan().to_string(),
            worklog: artifacts.worklog().to_string(),
            goal: artifacts.goal().to_string(),
...
pub(crate) struct RepairLedgerPlan {
    ...
    pub(crate) selected_operation_kind: Option<satelle::transport::SetupRepairOperationKind>,
    pub(crate) selected_run_status: Option<satelle::transport::SetupRepairRunStatus>,
    pub(crate) host_update_recovery_identity:
        Option<satelle::core::host_update::HostUpdateRecoveryIdentity>,
}

struct LocalTransport {
    alias: String,
    service: HostService,
    desktop_binding: Option<satelle::core::session::DesktopBindingRef>,
...
impl TransportClient for LocalTransport {
    ...
    fn supported_image_media_types(&self) -> Result<Vec<String>, SatelleError> {
        ...
        Ok(if capabilities.image_attachments() {
            satelle::transport::SUPPORTED_IMAGE_MEDIA_TYPES
                .iter()
    ...
    fn plan_setup_repair(
        ...
        let probes = probes
            .iter()
            .map(|probe| {
                satelle::host::SetupRepairProbe::new(
                    &probe.action_id,
                    &probe.label,
                    probe.retry_safe,
                    match probe.postcondition {
                        satelle::transport::SetupRepairPostcondition::Satisfied => {
                            satelle::host::SetupRepairPostcondition::Satisfied
                        }
                        satelle::transport::SetupRepairPostcondition::Unsatisfied => {
                            satelle::host::SetupRepairPostcondition::Unsatisfied
                        }
                        satelle::transport::SetupRepairPostcondition::Unknown => {
                            satelle::host::SetupRepairPostcondition::Unknown
                        }
                    },
                )
            })
        ...
        Ok(RepairLedgerPlan {
            ...
            selected_operation_kind: plan.selected_operation_kind().map(|kind| match kind {
                satelle::host::SetupOperationKind::Setup => {
                    satelle::transport::SetupRepairOperationKind::Setup
                }
                satelle::host::SetupOperationKind::Repair => {
                    satelle::transport::SetupRepairOperationKind::Repair
                }
                satelle::host::SetupOperationKind::HostUpdate => {
                    satelle::transport::SetupRepairOperationKind::HostUpdate
                }
                satelle::host::SetupOperationKind::StorageMigration => {
                    satelle::transport::SetupRepairOperationKind::StorageMigration
                }
                satelle::host::SetupOperationKind::ServiceStop => {
                    satelle::transport::SetupRepairOperationKind::ServiceStop
                }
                satelle::host::SetupOperationKind::ServiceRestart => {
                    satelle::transport::SetupRepairOperationKind::ServiceRestart
                }
            }),
            selected_run_status: plan.selected_run_status().map(|status| match status {
                satelle::host::SetupRunStatus::Running => {
                    satelle::transport::SetupRepairRunStatus::Running
                }
                satelle::host::SetupRunStatus::Completed => {
                    satelle::transport::SetupRepairRunStatus::Completed
                }
                satelle::host::SetupRunStatus::Failed => {
                    satelle::transport::SetupRepairRunStatus::Failed
                }
                satelle::host::SetupRunStatus::PartialFailure => {
                    satelle::transport::SetupRepairRunStatus::PartialFailure
                }
                satelle::host::SetupRunStatus::OutcomeUnknown => {
                    satelle::transport::SetupRepairRunStatus::OutcomeUnknown
                }
            }),
    ...
    fn invalidate_native_readiness(
        &self,
        _request: &satelle::transport::SetupVerificationRequest,
    ) -> Result<u64, SatelleError> {
    ...
    fn validate_provider_descriptor(
        ...
        Ok(ProviderDescriptorValidationReport {
            resolved_binding: satelle::core::PublicResolvedProviderBinding::from(
    ...
    fn authorize_provider_binding(
        ...
        self.service
            ...
            .map(|binding| satelle::core::PublicResolvedProviderBinding::from(&binding))
    }
    ...
    fn run(
        ...
        for event in outcome.events {
            if event.event_type() == satelle::core::EventType::ProviderSmoke {
    ...
    fn acknowledge_desktop_snapshot(
        &self,
        snapshot_id: &str,
        outcome: satelle::core::sensitive_diagnostics::DesktopSnapshotExportOutcome,
    ) -> Result<(), SatelleError> {
        self.service
...
fn local_turn_intent(request: &TurnRequest) -> Result<satelle::host::TurnIntent, SatelleError> {
    let attachments = request
        .attachments()
        .iter()
        .map(|attachment| match attachment {
            satelle::transport::ImageAttachment::Upload {
                media_type,
                size_bytes,
                sha256,
                data_base64,
            } => satelle::host::AttachmentInput::upload(
                media_type.clone(),
                *size_bytes,
                sha256.clone(),
                data_base64.clone(),
            ),
            satelle::transport::ImageAttachment::HostFile { path } => {
                satelle::host::AttachmentInput::host_file(path.clone())
            }
        })
        .collect();
    satelle::host::TurnIntent::new(request.prompt(), request.execution_mode())
        .and_then(|intent| {
...
struct CurrentDaemonArtifactObservation {
    ...
    codex_update_evidence: Option<satelle::core::host_update::CodexUpdateEvidence>,
    #[cfg(test)]
...
trait CurrentDaemonObservationContract {
    ...
    fn codex_update_evidence(&self) -> Option<&satelle::core::host_update::CodexUpdateEvidence>;
}

impl CurrentDaemonObservationContract for satelle::transport::CapabilitiesResponse {
    fn daemon_version(&self) -> &str {
        self.daemon_version()
    }
...
impl CurrentDaemonObservationContract for satelle::transport::MaintenanceUpdateEvidenceResponse {
    fn daemon_version(&self) -> &str {
        self.daemon_version()
    }
    ...
    fn setup_report_for_target(
        ...
        report.host_artifact = Some(
            ...
            .map_err(|error| match error {
                satelle::core::daemon_service::DaemonArtifactPlanError::NewerHostVersion => {
...
struct PersistentSetupInput<'a> {
    target: ssh_bootstrap::RemoteTarget,
    host_config: &'a satelle::core::HostConfig,
    daemon_path_overrides: &'a DaemonPathOverrides,
...
fn resolved_persistent_storage_policy(
    config: &satelle::core::HostConfig,
...
pub(crate) fn plan_ssh_storage_backup_cleanup(
    host: &SelectedHost,
) -> Result<(satelle::host::StorageBackupCleanupPlan, bool), SatelleError> {
    let transport = SshSetupTransport::new(host)?;
    if transport.requires_first_trust {
        return Err(SatelleError::invalid_usage(format!(
            "host '{}' must have a trusted expected Host Identity before storage maintenance can run",
    ...
    .map_err(|error| map_ssh_daemon_bootstrap_error(&transport.alias, error))?;
    Ok((
        satelle::host::StorageBackupCleanupPlan {
...
fn classify_storage_service_recovery(
    alias: &str,
    response: Result<satelle::transport::CapabilitiesResponse, DaemonClientError>,
) -> Result<bool, SatelleError> {
    match response {
        Ok(_) => Ok(false),
...
pub(crate) fn apply_ssh_storage_maintenance(
    ...
    let response = verification_client
        .plan_setup_repair(
            &satelle::transport::SetupRepairPlanRequest::new(
                Some(operation_id.clone()),
                vec![satelle::transport::SetupRepairProbe {
                    action_id: maintenance.action_id().to_string(),
                    label: maintenance.action_id().replace('-', " "),
                    retry_safe: true,
                    postcondition: satelle::transport::SetupRepairPostcondition::Satisfied,
                }],
            ),
...
struct HostMaintenanceInspection {
    ...
    codex_evidence: Option<satelle::core::host_update::CodexUpdateEvidence>,
...
enum HostMaintenancePlanKind {
    CodexOnly,
    HostUpdate(satelle::core::host_update::HostUpdateVersionSource),
    HostUpdateRecovery,
    Repair,
}
...
fn inspects_persistent_service(
    ...
    setup_mode: Option<satelle::core::SetupMode>,
) -> bool {
    matches!(
        kind,
    ...
    ) && setup_mode == Some(satelle::core::SetupMode::Persistent)
}
        ...
        HostMaintenancePlanKind::HostUpdate(source) => {
            let requires_artifact = |relation| {
                host_release_artifact_required(relation)
                    || (source
                        == satelle::core::host_update::HostUpdateVersionSource::ExplicitRelease
    ...
    match host.config.transport {
        TransportKind::Local => {
            ...
            let (_, platform) = canonical_remote_platform(
                &satelle::core::host_update::current_host_artifact_platform(),
            );
        ...
        TransportKind::Ssh => {
            ...
            let host_automation_is_safe = host.config.setup_mode
                == Some(satelle::core::SetupMode::Persistent)
...
fn validate_host_update_recovery_artifact(
    ...
    recovery_identity: &satelle::core::host_update::HostUpdateRecoveryIdentity,
) -> Result<(), SatelleError> {
...
enum DirectMaintenanceEvidence {
    Compatible(Box<satelle::transport::MaintenanceUpdateEvidenceResponse>),
    ProtocolIncompatible { current_version: Option<String> },
}

fn classify_direct_maintenance_evidence(
    host: &str,
    evidence: Result<satelle::transport::MaintenanceUpdateEvidenceResponse, DaemonClientError>,
...
pub(crate) fn plan_host_update(
    host: &SelectedHost,
    target_version: &str,
    components: &[satelle::core::host_update::HostUpdateComponent],
    includes_all: bool,
    version_source: satelle::core::host_update::HostUpdateVersionSource,
) -> Result<satelle::core::host_update::HostUpdateReport, SatelleError> {
...
fn plan_host_update_recovery(
    ...
    plan_host_update_internal(
        host,
        recovery_identity.target_version(),
        &[satelle::core::host_update::HostUpdateComponent::Host],
        false,
        satelle::core::host_update::HostUpdateVersionSource::HostCompatibilityRequirement,
...
fn plan_host_update_internal(
    ...
    includes_all: bool,
    version_source: satelle::core::host_update::HostUpdateVersionSource,
    recovery_identity: Option<&satelle::core::host_update::HostUpdateRecoveryIdentity>,
) -> Result<satelle::core::host_update::HostUpdateReport, SatelleError> {
    use satelle::core::host_update::HostUpdateVersionSource;
    ...
    let needs_host_artifact = includes_all
        || components.is_empty()
        || components.contains(&satelle::core::host_update::HostUpdateComponent::Host);
    ...
    if !host_automation_is_safe {
        for target in &mut report.targets {
            if matches!(
                target.target,
                satelle::core::host_update::HostUpdateTarget::HostDaemon
                    | satelle::core::host_update::HostUpdateTarget::HostDaemonService
            ) && target.requires_mutation()
            {
                target.disposition = satelle::core::host_update::HostUpdateDisposition::Skipped;
            }
        }
        report = satelle::core::host_update::HostUpdateReport::new(
            report.host,
...
pub(crate) fn apply_host_update(
    host: &SelectedHost,
    target_version: &str,
    report: satelle::core::host_update::HostUpdateReport,
    components: &[satelle::core::host_update::HostUpdateComponent],
    includes_all: bool,
) -> Result<satelle::core::host_update::HostUpdateReport, SatelleError> {
    let version_source = report
        .targets
        .iter()
        .find(|target| target.target == satelle::core::host_update::HostUpdateTarget::HostDaemon)
        .map_or(
            satelle::core::host_update::HostUpdateVersionSource::InvokingCliRelease,
...
fn apply_host_update_with_operation(
    host: &SelectedHost,
    target_version: &str,
    mut report: satelle::core::host_update::HostUpdateReport,
    operation: HostUpdateOperation,
    entry: HostReplacementEntry,
    existing_operation_id: Option<&str>,
    revalidate: impl FnOnce() -> Result<satelle::core::host_update::HostUpdateReport, SatelleError>,
) -> Result<satelle::core::host_update::HostUpdateReport, SatelleError> {
    use satelle::core::host_update::{HostUpdatePostcheck, HostUpdateTarget};
    ...
    let invalidation = satelle::transport::NativeReadinessInvalidationRequest::host();
...
fn finish_confirmed_host_update_action(
    report: &mut satelle::core::host_update::HostUpdateReport,
    action_id: &str,
...
fn finish_terminal_failed_host_update_postcheck(
    ...
    report = report.with_postcheck(satelle::core::host_update::HostUpdatePostcheck::failed(
    ...
    report.status = satelle::core::host_update::HostUpdateStatus::PostcheckFailed;
...
fn finish_successful_host_update_postcheck(
    ...
    report = report.with_postcheck(satelle::core::host_update::HostUpdatePostcheck::passed(
        "native-computer-use-ready",
...
fn begin_host_update_maintenance_with_revalidation(
    ...
    operation_id: &str,
    operation: HostUpdateOperation,
    accepted: &satelle::core::host_update::HostUpdateReport,
    revalidate: impl FnOnce() -> Result<satelle::core::host_update::HostUpdateReport, SatelleError>,
) -> Result<satelle::core::host_update::HostUpdateReport, SatelleError> {
...
fn host_update_recovery_identity(
    report: &satelle::core::host_update::HostUpdateReport,
) -> Result<satelle::core::host_update::HostUpdateRecoveryIdentity, SatelleError> {
    let target = report
        .targets
        .iter()
        .find(|target| {
            target.target == satelle::core::host_update::HostUpdateTarget::HostDaemon
                && target.requires_mutation()
        })
        .ok_or_else(|| {
    ...
    Ok(satelle::core::host_update::HostUpdateRecoveryIdentity::new(
...
fn host_update_recovery_pending(
    ...
    report.status = satelle::core::host_update::HostUpdateStatus::PartialFailure;
...
fn selected_host_replacement_operation(
    ...
    match run.selected_operation_kind {
        Some(satelle::transport::SetupRepairOperationKind::HostUpdate) => {
            Some(HostUpdateOperation::Update)
        }
        Some(satelle::transport::SetupRepairOperationKind::Repair) => {
...
fn repair_host_replacement_entry(
    repair: &satelle::core::host_update::RepairUpgradeReport,
    resumes_host_replacement: bool,
) -> HostReplacementEntry {
    use satelle::core::host_update::{
...
pub(crate) fn plan_repair_upgrades(
    ...
    run_id: Option<&str>,
    recovery_authorized: bool,
) -> Result<satelle::core::host_update::RepairUpgradeReport, SatelleError> {
    use satelle::core::host_update::{
    ...
    if repair_plan_reads_daemon_ledger(&report, run_id) {
        let probes = report
            .planned_actions
            .iter()
            .map(|action_id| satelle::transport::SetupRepairProbe {
                action_id: action_id.clone(),
                label: action_id.replace('-', " "),
                retry_safe: true,
                postcondition: satelle::transport::SetupRepairPostcondition::Unsatisfied,
            })
            .collect::<Vec<_>>();
        ...
        if ledger.available {
            report.ledger_status = satelle::core::host_update::RepairLedgerStatus::Available;
            report.plan_source = satelle::core::host_update::RepairPlanSource::SetupLedger;
            ...
            if !unattended_retry_safe {
                for action in &mut report.actions {
                    if action.disposition
                        == satelle::core::host_update::RepairUpgradeDisposition::Required
                    {
                        action.disposition =
                            satelle::core::host_update::RepairUpgradeDisposition::ManualActionRequired;
                    }
                }
                let ledger_status = report.ledger_status;
                let plan_source = report.plan_source;
                report = satelle::core::host_update::RepairUpgradeReport::new(
                    report.host,
                    report.actions,
                );
...
fn append_codex_repair_inspections(
    ...
    codex: Option<satelle::core::host_update::CodexUpdateEvidence>,
) -> Result<(), SatelleError> {
    use satelle::core::host_update::{
    ...
    if codex.availability
        == satelle::core::host_update::CodexUpdateAvailability::UnsupportedHostPlatform
    {
        return Ok(());
    }
...
pub(crate) fn apply_repair_upgrades(
    host: &SelectedHost,
    mut repair: satelle::core::host_update::RepairUpgradeReport,
    recovery_run_id: Option<&str>,
) -> Result<satelle::core::host_update::RepairUpgradeReport, SatelleError> {
    use satelle::core::host_update::HostUpdateComponent;
    ...
    let target_version = recovery_identity.as_ref().map_or(
        env!("CARGO_PKG_VERSION"),
        satelle::core::host_update::HostUpdateRecoveryIdentity::target_version,
    );
...
fn begin_setup_maintenance(
    ...
    let response = reconcile_bootstrap_maintenance_response(
        host,
        match setup_mode {
            satelle::core::SetupMode::Persistent => client.begin_persistent_service_maintenance(
                &operation_id,
                bootstrap_lock.operation_kind().as_str(),
                managed_setup,
            ),
            satelle::core::SetupMode::OnDemand => client.begin_on_demand_setup_maintenance(
                &operation_id,
                bootstrap_lock.operation_kind().as_str(),
                managed_setup,
            ),
...
fn token_file_error(path: &Path, error: satelle::core::SecureFileError) -> SatelleError {
    SatelleError::config_error(
        format!(
...
impl TransportClient for SshSetupTransport {
    ...
    fn doctor(
        ...
        _provider_intent: &satelle::host::ProviderComputerUseIntent,
    ) -> DoctorExecutionResult {
    ...
    fn plan_setup_repair(
        &self,
        _run_id: Option<&str>,
        _probes: &[satelle::transport::SetupRepairProbe],
    ) -> Result<RepairLedgerPlan, SatelleError> {
    ...
    fn validate_provider_descriptor(
        ...
        _model_alias: &str,
        _provider_alias: &str,
        _options: satelle::host::ProviderDescriptorValidationOptions,
    ) -> Result<ProviderDescriptorValidationReport, SatelleError> {
    ...
    fn authorize_provider_binding(
        &self,
        _desktop_binding: &str,
        _authorization: &satelle::core::ProviderBindingAuthorization,
    ) -> Result<satelle::core::PublicResolvedProviderBinding, SatelleError> {
    ...
    fn preview_provider_secret_provisioning(
        &self,
        _metadata: &satelle::transport::ProviderSecretProvisioningMetadata,
    ...
    fn provision_provider_secret(
        &self,
        _preview: &satelle::transport::ProviderSecretProvisioningPreviewResponse,
        _metadata: &satelle::transport::ProviderSecretProvisioningMetadata,
    ...
    fn recording_preflight(
        &self,
        _mode: satelle::core::recording::RecordingMode,
...
impl TransportClient for DirectTransport {
    ...
    fn enqueue_run(
        ...
        self.client
            ...
            .map(satelle::transport::QueueStatusResponse::into_status)
            .map_err(|error| direct_transport_error(&self.alias, error))
    }
    ...
    fn cancel_queue_request(
        ...
        self.client
            ...
            .map(satelle::transport::QueueCancelResponse::into_result)
            .map_err(|error| direct_transport_error(&self.alias, error))
    }
    ...
    fn acknowledge_desktop_snapshot(
        ...
        self.client
            .acknowledge_desktop_snapshot(
                snapshot_id,
                &satelle::transport::DesktopSnapshotAcknowledgeRequest::new(outcome),
                &Self::idempotency_key(),
            )
            .map(|_| ())
            .map_err(|error| direct_transport_error(&self.alias, error))
    ...
    fn plan_setup_repair(
        ...
        Ok(RepairLedgerPlan {
            ...
            automatic_action_ids: response
                ...
                .iter()
                .filter(|action| {
                    action.decision == satelle::transport::SetupRepairDecision::RetryAutomatically
                })
    ...
    fn validate_provider_descriptor(
        ...
        let response = self
            ...
            .validate_provider_descriptor(
                provider_alias,
                model_alias,
                &satelle::transport::ProviderDescriptorValidationRequest::new(
                    desktop_binding,
                    options.mode(),
    ...
    fn authorize_provider_binding(
        ...
        self.client
            .authorize_provider_binding(
                authorization.requested_provider_alias(),
                authorization.requested_model_alias(),
                &satelle::transport::ProviderBindingAuthorizationRequest::new(
                    desktop_binding,
                    authorization.clone(),
                ),
    ...
    fn telemetry_status(&self) -> Result<satelle::core::telemetry::TelemetryStatus, SatelleError> {
        self.client
            ...
            .map(satelle::transport::HostTelemetryStatusResponse::into_status)
            .map_err(|error| direct_transport_error(&self.alias, error))
    }
    ...
    fn recording_preflight(
        ...
        self.client
            .recording_preflight(&satelle::transport::RecordingPreflightRequest::new(
                mode,
                retention_ms,
                source_host,
            ))
            .map(satelle::transport::RecordingPreflightResponse::into_preflight)
            .map_err(|error| direct_transport_error(&self.alias, error))
    }
    ...
    fn raw_protocol_export(
        ...
        self.client
            ...
            .map(satelle::transport::RawProtocolDownloadResponse::into_artifact)
            .map_err(|error| direct_transport_error(&self.alias, error))
    }
    ...
    fn acknowledge_raw_protocol_export(
        ...
        self.client
            .acknowledge_raw_protocol_export(
                turn_id,
                &satelle::transport::RawProtocolAcknowledgeRequest::new(outcome),
                &Self::idempotency_key(),
            )
            .map(|_| ())
            .map_err(|error| direct_transport_error(&self.alias, error))
    ...
    fn begin_raw_subprocess_export(
        &self,
        request: &satelle::transport::RawSubprocessBeginRequest,
    ) -> Result<satelle::core::sensitive_diagnostics::RawSubprocessManifest, SatelleError> {
        self.client
            .begin_raw_subprocess_export(request, &Self::idempotency_key())
            .map(satelle::transport::RawSubprocessBeginResponse::into_manifest)
            .map_err(|error| direct_transport_error(&self.alias, error))
    }
    ...
    fn prepare_raw_subprocess_export(
        ...
        self.client
            .prepare_raw_subprocess_export(
                invocation_id,
                &satelle::transport::RawSubprocessPrepareRequest::new(artifact_byte_size),
                &Self::idempotency_key(),
            )
            .map(drop)
            .map_err(|error| direct_transport_error(&self.alias, error))
    ...
    fn setup_history(&self) -> Result<satelle::host::SetupHistory, SatelleError> {
        self.client
            .setup_history()
            .map(satelle::transport::SetupHistoryResponse::into_history)
            .map_err(|error| direct_transport_error(&self.alias, error))
    }
}
...
fn map_api_error(host: &str, error: &ApiError) -> SatelleError {
    ...
    if let Some((code, message)) = storage_code {
        ...
        let details = if code == ErrorCode::SetupPartiallyApplied {
            let Some(cleanup) = details.get("cleanup").cloned().and_then(|report| {
                serde_json::from_value::<satelle::host::StorageMigrationCleanup>(report).ok()
            }) else {
                return SatelleError::remote_api_error(host, "invalid-daemon-response");
            };
    ...
    if matches!(
        ...
        let Some(details) = error
            .details()
            .and_then(satelle::core::secret_file_error_details)
        else {
            return SatelleError::remote_api_error(host, "invalid-daemon-response");
        };
...
fn map_stop_not_confirmed_api_error(host: &str, error: &ApiError) -> SatelleError {
    ...
    if !matches!(
        ...
        || details
            ...
            .and_then(|value| satelle::core::session::SessionStateRevision::new(value).ok())
            .is_none()
        || details
            .get("turn_state_revision")
            .and_then(serde_json::Value::as_u64)
            .and_then(|value| satelle::core::session::TurnStateRevision::new(value).ok())
            .is_none()
        || details
            .get("retryable")
            .and_then(serde_json::Value::as_bool)
...
fn api_code_error(host: &str, code: ApiErrorCode) -> SatelleError {
    match code {
        ...
        ApiErrorCode::SecretFileTildeFormUnsupported => {
            satelle::core::SecretFilePathError::TildeFormUnsupported.diagnostic(
                None,
                None,
                Some(host),
                None,
        ...
        ApiErrorCode::SecretFileHomeUnavailable => {
            satelle::core::SecretFilePathError::HomeUnavailable.diagnostic(
                None,
                None,
                Some(host),
                None,
...
pub(crate) fn plan_local_setup(
    ...
    if selected_test_support_adapter()?.is_some() {
        return Ok(satelle::host::local_setup_plan_for_tests(
            host,
            dry_run,
            setup_mode,
            setup_components,
...
pub(crate) fn local_setup_host_sessions(host: &str) -> Result<HostSessionsReport, CliFailure> {
    ...
    if selected_test_support_adapter()?.is_some() {
        return Ok(HostSessionsReport {
            ...
            sessions: satelle::host::local_desktop_sessions_for_setup_tests(),
        });
    }
    satelle::host::local_desktop_sessions_for_setup()
    ...
    if let Some(adapter) = selected_test_support_adapter()? {
        ...
        if test_config.desktop_bindings.is_empty() {
            test_config.desktop_bindings.insert(
                ...
                satelle::core::DesktopBindingConfig {
...
fn effective_local_host_config(
    ...
    {
        config.daemon_idle_timeout = satelle::core::ExplicitDuration::parse("250ms");
    }
...
fn launch_local_daemon_process(
    ...
    let provider_environment = host_config
        ...
        .filter_map(|source| match source {
            satelle::core::ProviderSecretSource::Environment { variable } => Some(variable.clone()),
            _ => None,
        })
        .collect::<Vec<_>>();
    #[cfg(not(windows))]
...
pub(crate) fn host_paths_for_inspection(
    host: &SelectedHost,
) -> Result<satelle::core::daemon_service::DaemonResolvedPathSet, CliFailure> {
    let result = match host.config.transport {
        TransportKind::Direct => match direct_transport(host)
...
enum IdentityOperationState {
    Fresh,
    Pending(satelle::core::SshIdentityCommitRecord),
}
...
struct SshTrustFinalization {
    ...
    destination: String,
    target: ssh_bootstrap::RemoteTarget,
    record: satelle::core::SshIdentityCommitRecord,
    client: Arc<DaemonClient>,
    _tunnel: SshTunnel,
...
mod bootstrap_ordering_tests {
    ...
    fn setup_report_uses_older_pre_mutation_daemon_observation() {
        ...
        assert_eq!(
            artifact.action,
            satelle::core::daemon_service::DaemonArtifactAction::UpdateOlder
        );
    }
    ...
    fn setup_reports_newer_host_and_missing_release_artifact_with_typed_errors() {
        ...
        assert_eq!(
            newer_error.code,
            satelle::core::ErrorCode::HostBinaryNewerThanCli
        );
        ...
        assert_eq!(
            missing_error.code,
            satelle::core::ErrorCode::HostArtifactUnavailable
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            satelle::core::ErrorCode::ReleaseVerifierUnavailable
        );
        assert_eq!(
    ...
    fn computer_use_not_ready_reason_token_survives_the_daemon_round_trip() {
        ...
        assert_eq!(mapped.code, satelle::core::ErrorCode::ComputerUseNotReady);
        assert_eq!(
        ...
        assert_eq!(manual.code, satelle::core::ErrorCode::ComputerUseNotReady);
        ...
        for details in [
            ...
            assert_eq!(rejected.code, satelle::core::ErrorCode::RemoteExecution);
        }

        // No details still maps to the bare readiness error.
        let bare = map_api_error("local-demo", &api_error(serde_json::Value::Null));
        assert_eq!(bare.code, satelle::core::ErrorCode::ComputerUseNotReady);
    ...
    fn unsupported_codex_platform_is_a_readiness_error_not_invalid_usage() {
        let error = map_host_update_plan_error(
            crate::host_update::HostUpdatePlanError::UnsupportedCodexTarget {
                target: satelle::core::host_update::HostUpdateTarget::CodexRuntime,
            },
        );

        assert_eq!(error.code, satelle::core::ErrorCode::ComputerUseNotReady);
    ...
    fn host_update_metadata_failures_keep_the_typed_error_and_real_host_alias() {
        ...
        assert_eq!(unreachable.code, satelle::core::ErrorCode::HostUnreachable);
    ...
    fn persistent_repair_inspects_the_service_before_automation() {
        ...
        assert!(!inspects_persistent_service(
            HostMaintenancePlanKind::Repair,
            Some(satelle::core::SetupMode::OnDemand),
        ));
    }
