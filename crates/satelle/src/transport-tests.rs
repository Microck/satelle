...
use satelle::transport::{DaemonServer, DaemonServerConfig};
...
fn storage_maintenance_recovery_commands_and_phase_evidence_are_exact() {
    ...
    assert_eq!(
        ...
        satelle::core::ErrorCode::SetupActionFailed
    );
    assert_eq!(
    ...
    assert_eq!(
        partial.code,
        satelle::core::ErrorCode::SetupPartiallyApplied
    );
    assert_eq!(
...
fn replacement_daemon_adopts_bootstrap_repair_actions_before_postchecks() {
    for (published_service, expected_publish_status) in [
        (true, satelle::host::SetupActionStatus::Completed),
        (false, satelle::host::SetupActionStatus::Skipped),
    ] {
                ...
                assert_eq!(
                    ...
                    [
                        (
                            ...
                            satelle::host::SetupActionStatus::Completed,
                        ),
                        ("publish-host-service", expected_publish_status),
                        (
                            "restart-host-daemon",
                            satelle::host::SetupActionStatus::Started,
                        ),
                        (
                            "invalidate-readiness-caches",
                            satelle::host::SetupActionStatus::Planned,
                        ),
                        (
...
fn host_update_recovery_requires_and_rechecks_the_persisted_artifact() {
    ...
    let mismatch = validate_host_update_recovery_artifact(
        &inspection,
        &satelle::core::host_update::HostUpdateRecoveryIdentity::new(
    ...
    assert_eq!(
        mismatch.code,
        satelle::core::ErrorCode::HostUpdateRecoveryIdentityMismatch
    );
    assert_eq!(
        mismatch
            .details
...
fn test_host_update_recovery_identity() -> satelle::core::host_update::HostUpdateRecoveryIdentity {
    satelle::core::host_update::HostUpdateRecoveryIdentity::new(
...
fn test_host_update_report() -> satelle::core::host_update::HostUpdateReport {
    satelle::core::host_update::HostUpdateReport::new(
        "office",
        vec![satelle::core::host_update::HostUpdateComponent::Host],
        vec![satelle::core::host_update::HostUpdateTargetPlan {
            target: satelle::core::host_update::HostUpdateTarget::HostDaemon,
            current_version: Some("0.0.9".to_string()),
            target_version: env!("CARGO_PKG_VERSION").to_string(),
            version_source: satelle::core::host_update::HostUpdateVersionSource::InvokingCliRelease,
            artifact_digest: Some(
                "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa".to_string(),
            ),
            disposition: satelle::core::host_update::HostUpdateDisposition::Update,
            restart_impact: satelle::core::host_update::HostUpdateRestartImpact::HostDaemon,
            remote_mutations: vec![satelle::core::host_update::HostUpdateMutation {
...
fn setup_selection(mode: satelle::core::SetupMode) -> SetupModeSelection {
...
fn ssh_setup_plan_reaches_explicit_trust_for_an_unpinned_host() {
    ...
    let report = transport
        .setup(
            true,
            setup_selection(satelle::core::SetupMode::OnDemand),
...
fn unpinned_and_unauthenticated_bindings_cannot_open_ordinary_transports() {
    ...
    direct.config.network = Some(satelle::core::NetworkConfig::Tailscale {
...
fn default_missing_ssh_host_plan_skips_unavailable_codex_targets() {
    ...
    assert_eq!(
        ...
        satelle::core::host_update::HostUpdateDisposition::Skipped
    );
...
fn ssh_setup_rerun_reuses_an_existing_secure_token_destination() {
    ...
    drop(
        satelle::core::open_or_create_owner_only_directory(&token_directory)
...
fn uncertain_first_host_update_action_start_closes_the_unmodified_operation() {
    with_bootstrap_handoff_test_context_with_scope(
        ...
        |client, fake_ssh, _, _, ledger, _| {
            ...
            assert_eq!(run.status(), satelle::host::SetupRunStatus::Failed);
            assert_eq!(
                run.actions()
                    .iter()
                    .map(|action| (action.action_id(), action.status()))
                ...
                [
                    (
                        "install-host-artifact",
                        satelle::host::SetupActionStatus::Failed,
                    ),
                    (
                        "publish-host-service",
                        satelle::host::SetupActionStatus::Skipped,
                    ),
                    (
                        "restart-host-daemon",
                        satelle::host::SetupActionStatus::Skipped,
...
fn failed_prechange_action_cleanup_reports_the_recovery_pending_operation() {
    with_bootstrap_handoff_test_context_with_scope(
        ...
        |client, fake_ssh, _, _, service, bootstrap_token_id| {
            ...
            let mut report = satelle::core::host_update::HostUpdateReport::new(
                "office",
                vec![satelle::core::host_update::HostUpdateComponent::Host],
                Vec::new(),
            );
...
fn ssh_setup_rejects_explicit_persistent_linux_before_mutating() {
    ...
    let error = transport
        .setup(
            false,
            setup_selection(satelle::core::SetupMode::Persistent),
            vec!["transport".to_string()],
            DaemonPathOverrides::default(),
        )
    ...
    let report = transport
        .setup(
            ...
            SetupModeSelection::new(
                ...
                satelle::core::daemon_service::SetupModeSource::UserConfig,
            ),
            vec!["transport".to_string()],
            DaemonPathOverrides::default(),
        )
...
fn ssh_setup_plans_windows_task_scheduler_login_session_service() {
    ...
    assert_eq!(
        service.manager,
        satelle::core::daemon_service::DaemonServiceManager::WindowsTaskScheduler
    );
...
fn stored_authorization_is_revalidated_by_alias_before_alias_only_turn_preflight() {
    ...
    let transport = LocalTransport::new(LOCAL_DEMO_HOST.to_string(), service);
    let authorization =
        satelle::core::ProviderBindingAuthorization::new("review", "openai", "gpt-5.2", "openai");
...
fn host_provider_validation_drives_conflicting_controller_projection() {
    ...
    assert_eq!(
        ...
        satelle::core::ProviderBindingSource::UserConfig
    );
    assert!(
        validation
            .resolved_binding
...
fn setup_accepts_matching_host_owned_provider_projection() {
    ...
    let validation = transport::ProviderDescriptorValidationReport {
        resolved_binding: satelle::core::PublicResolvedProviderBinding::from(&resolved_binding),
        validation: satelle::core::ProviderAuthValidationResult::new(
            satelle::core::ProviderAuthValidationOutcome::ConfiguredDeferred,
            satelle::core::ProviderAuthObservationSource::Deferred,
        ),
    };
...
fn setup_rejects_conflicting_host_owned_provider_projection() {
    let controller_authorization = satelle::core::ProviderBindingAuthorization::new(
        "review",
        "openai",
        "controller-model",
        "openai",
...
fn install_replay_admitted_status_failure_peer(
    ...
    session_id: satelle::core::SessionId,
    turn_id: satelle::core::TurnId,
) -> thread::JoinHandle<()> {
    ...
    let subscribed_stream = fixture
        ...
        .block_on(
            fixture
                .transport()
                .event_client
                .connect_events(vec![satelle::transport::EventSubscription::Host]),
        )
...
fn replay_admitted_run_stops_exact_turn_before_failed_status_read() {
    let mut fixture = DirectFixture::start();
    let interrupt = TestInterrupt::default();
    let session_id = satelle::core::SessionId::new();
    let turn_id = satelle::core::TurnId::new();
...
fn direct_run_preserves_typed_recoverable_close_errors() {
    let control = serde_json::from_value(serde_json::json!({
        ...
        "type": "error",
        "request_id": satelle::transport::RequestId::new(),
        "host_identity": "host-direct-test",
    ...
    assert_eq!(
        direct_run_event_error(
            ...
            DaemonEventError::Closed {
                ...
                code: 1008,
                reason: satelle::transport::WsCloseReason::SlowConsumer,
            },
        )
        .code,
...
fn admission_failures_preserve_definitive_and_ambiguous_phases() {
    ...
    let api_error: satelle::transport::ApiError = serde_json::from_value(serde_json::json!({
        "schema_version": "satelle.error.v1",
        "request_id": satelle::transport::RequestId::new().to_string(),
        "host_identity": "host-direct-test",
...
fn stop_not_confirmed_api_details_are_validated_and_preserved() {
    ...
    let api_error = |details: serde_json::Value| {
        serde_json::from_value::<satelle::transport::ApiError>(serde_json::json!({
            "schema_version": "satelle.error.v1",
            "request_id": satelle::transport::RequestId::new().to_string(),
            "host_identity": "host-direct-test",
...
fn managed_setup_api_failures_rerun_the_exact_failed_setup_selection() {
    for (mode, component) in [
        (satelle::core::SetupMode::Persistent, "codex"),
        (satelle::core::SetupMode::OnDemand, "computer-use"),
    ] {
...
fn direct_inspection_fallback_uses_bootstrap_settings_without_durable_authentication() {
    ...
    config.network = Some(satelle::core::NetworkConfig::Tailscale {
    ...
    config.ssh_bootstrap = Some(satelle::core::SshBootstrapConfig {
