...
use satelle::host::{LogPageQuery, LogSubject};
...
impl DirectTransport {
    ...
    fn target_turn<'session>(
        ...
        turn_id: &TurnId,
    ) -> Result<&'session satelle::core::session::PublicTurn, SatelleError> {
        session
            .turns()
            .iter()
    ...
    where
        T: Send + 'static,
        F: FnOnce(Arc<satelle::transport::DaemonClient>) -> Result<T, DaemonClientError>
            + Send
            + 'static,
    {
    ...
    async fn wait_for_recording_manifest(
        &self,
        turn_id: &TurnId,
    ) -> Result<satelle::core::recording::RecordingManifest, SatelleError> {
            ...
            match self
                .blocking_http(move |client| {
                    client
                        ...
                        .map(satelle::transport::RecordingManifestResponse::into_manifest)
                })
                .await
            {
        ...
        loop {
            ...
            match next_event {
                Ok(event) => {
                    ...
                    if matches!(
                        event.event_type(),
                        satelle::core::EventType::ProviderSmoke
                            | satelle::core::EventType::Readiness
                    ) {
                        on_event(event)?;
                        continue;
                    }
    ...
    fn revision_bound_action_event_participates_in_attached_turn_ordering() {
        ...
        let event = SatelleEventBody::new(
            ...
            Some(EventSubject::Turn {
                session_id: session_id.clone(),
                turn_id: turn_id.clone(),
                session_state_revision: satelle::core::session::SessionStateRevision::new(3)
                    .unwrap(),
                turn_state_revision,
            }),
