...
pub(super) fn paths_report(host: Option<&super::SelectedHost>) -> Result<Value, CliFailure> {
    let (selected_host, paths, observation_source) = if let Some(host) = host {
        (
            ...
            Some(satelle::core::PathSource::HostReported),
        )
    } else {
        ...
        (
            ...
            satelle::core::daemon_service::DaemonResolvedPathSet::from(&paths),
            None,
        )
    };
    Ok(json!({
...
pub(super) fn doctor_for_host(
    ...
    transport_for(host)?
        .doctor(
            ...
            options,
            &satelle::host::ProviderComputerUseIntent::host_default(),
        )
...
) -> Result<satelle::host::HostStatus, CliFailure> {
