use std::{io::Result, mem, ops::ControlFlow, process::ExitStatus, ptr};
use tokio::{
...
use crate::command_group::winres::*;
...
impl ChildImp {
    ...
    pub async fn wait(&mut self) -> Result<ExitStatus> {
        ...
        for retry_attempt in 1..=MAX_RETRY_ATTEMPT {
            if Self::wait_imp(completion_port, 0)?.is_break() {
                ...
                let _ = spawn_blocking(move || Self::wait_imp(completion_port, INFINITE)).await??;
            }
        }
    ...
    pub fn try_wait(&mut self) -> Result<Option<ExitStatus>> {
        if Self::wait_imp(ThreadSafeRawHandle(self.handles.completion_port), 0)?.is_break() {
            self.inner.try_wait()
        } else {
            Ok(None)
        }
