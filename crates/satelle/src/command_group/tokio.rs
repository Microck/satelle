...
use crate::command_group::{AsyncGroupChild, builder::CommandGroupBuilder};
...
pub trait AsyncCommandGroup {
    ...
    /// Converts the implementor into a [`CommandGroupBuilder`](crate::command_group::CommandGroupBuilder), which can be used to
    /// set flags that are not available on the `Command` type.
    fn group(
        &mut self,
    ) -> crate::command_group::builder::CommandGroupBuilder<'_, tokio::process::Command>;
