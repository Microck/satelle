-- Rebuild under the migration transaction with foreign keys checked before commit.
CREATE TABLE idempotency_records_v19 (
    ...
    durable_outcome TEXT NOT NULL CHECK (durable_outcome IN (
        ...
        'v1.api_token_issue.completed',
        'v1.api_token_issue.rejected',
        'v1.api_token_rotate.completed',
        'v1.api_token_rotate.rejected',
        'v1.api_token_revoke.completed',
        'v1.api_token_revoke.rejected',
        'v1.storage_migration.pending',
        'v1.storage_migration.completed'
    )),
    ...
    CHECK (
        ...
        OR (
            operation = 'api_token_issue'
            AND status = 'terminal'
            AND durable_outcome IN ('v1.api_token_issue.completed', 'v1.api_token_issue.rejected')
            AND session_id IS NULL
            AND turn_id IS NULL
            AND result_session_state_revision IS NULL
            AND result_session_updated_at IS NULL
        ...
        OR (
            operation = 'api_token_rotate'
            AND status = 'terminal'
            AND durable_outcome IN ('v1.api_token_rotate.completed', 'v1.api_token_rotate.rejected')
            AND session_id IS NULL
            AND turn_id IS NULL
            AND result_session_state_revision IS NULL
            AND result_session_updated_at IS NULL
        ...
        OR (
            operation = 'api_token_revoke'
            AND status = 'terminal'
            AND durable_outcome IN ('v1.api_token_revoke.completed', 'v1.api_token_revoke.rejected')
            AND session_id IS NULL
            AND turn_id IS NULL
            AND result_session_state_revision IS NULL
            AND result_session_updated_at IS NULL
        ...
        OR (
            ...
            AND result_session_updated_at IS NULL
            AND (
                (status = 'in_progress' AND durable_outcome = 'v1.storage_migration.pending')
                OR (status = 'terminal' AND durable_outcome = 'v1.storage_migration.completed'
                    AND result_json IS NOT NULL)
            )
        )
        OR (
...
INSERT INTO idempotency_records_v19 (
    principal_ref,
    operation,
    idempotency_key,
    operation_id,
...
ALTER TABLE idempotency_records_v19 RENAME TO idempotency_records;
